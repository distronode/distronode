...
from distronode.errors import DistronodeError, DistronodeConnectionFailure, DistronodeActionSkip, DistronodeActionFail, DistronodeAuthenticationFailure
from distronode.executor.module_common import modify_module
from distronode.executor.interpreter_discovery import discover_interpreter, InterpreterDiscoveryRequiredError
from distronode.module_utils.common.arg_spec import ArgumentSpecValidator
from distronode.module_utils.errors import UnsupportedError
from distronode.module_utils.json_utils import _filter_non_json_lines
from distronode.module_utils.six import binary_type, string_types, text_type
...
from distronode.utils.unsafe_proxy import wrap_var, DistronodeUnsafeText
from distronode.vars.clean import remove_internal_keys
from distronode.utils.plugin_docs import get_versioned_doclink
...
class ActionBase(ABC):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        * Module parameters.  These are stored in self._task.args
        ...
        if self._task.async_val and not self._supports_async:
            raise DistronodeActionFail('async is not supported for this task.')
        elif self._task.check_mode and not self._supports_check_mode:
            raise DistronodeActionSkip('check mode is not supported for this task.')
        elif self._task.async_val and self._task.check_mode:
            raise DistronodeActionFail('check mode and async cannot be used on same task.')
        ...
        if self._VALID_ARGS:
            ...
            if bad_opts:
                raise DistronodeActionFail('Invalid options for %s: %s' % (self._task.action, ','.join(list(bad_opts))))
    ...
    def validate_argument_spec(self, argument_spec=None,
        ...
        if error:
            ...
            raise DistronodeActionFail(msg)
    ...
    def _configure_module(self, module_name, module_args, task_vars):
        ...
        if self._task.delegate_to:
            use_vars = task_vars.get('distronode_delegated_vars')[self._task.delegate_to]
        else:
            use_vars = task_vars
        ...
        for mod_type in self._connection.module_implementation_preferences:
            ...
            if mod_type == '.ps1':
                ...
                # for each subsystem.
                win_collection = 'distronode.windows'
                rewrite_collection_names = ['distronode.builtin', 'distronode.legacy', '']
            ...
            if not result.resolved:
                if result.redirect_list and len(result.redirect_list) > 1:
                    ...
                    raise DistronodeError("The module {0} was redirected to {1}, which could not be loaded.".format(module_name, target_module_name))

            module_path = result.plugin_resolved_path
            if module_path:
                break
        else:  # This is a for-else: http://bit.ly/1ElPkyg
            raise DistronodeError("The module %s was not found in configured module paths" % (module_name))
        ...
        for dummy in (1, 2):
            try:
                ...
                self._discovered_interpreter = DistronodeUnsafeText(discover_interpreter(
                    action=self,
                ...
                use_vars['distronode_facts'][discovered_key] = self._discovered_interpreter
                ...
                if not self._task.delegate_to or self._task.delegate_facts:
                    ...
                    task_vars['distronode_facts'][discovered_key] = self._discovered_interpreter
                    # preserve this so _execute_module can propagate back to controller as a fact
                    self._discovered_interpreter_key = discovered_key
                else:
                    task_vars['distronode_delegated_vars'][self._task.delegate_to]['distronode_facts'][discovered_key] = self._discovered_interpreter
        ...
        if self._task.environment is not None:
            ...
            for environment in environments:
                ...
                if not isinstance(temp_environment, dict):
                    raise DistronodeError("environment must be a dictionary, received %s (%s)" % (temp_environment, type(temp_environment)))
    ...
    def _get_remote_addr(self, tvars):
        ...
        remote_addr = tvars.get('delegated_vars', {}).get('distronode_host', tvars.get('distronode_host', tvars.get('inventory_hostname', None)))
    ...
    def _get_remote_user(self):
        ...
        # TODO: use 'current user running distronode' as fallback when moving away from play_context
    ...
    def _make_tmp_path(self, remote_user=None):
        ...
        if getattr(self._connection, '_remote_is_local', False):
            ...
            tmpdir = self._remote_expand_user(self.get_shell_option('remote_tmp', default='~/.distronode/tmp'), sudoable=False)
        ...
        if result['rc'] != 0:
            if result['rc'] == 5:
                ...
                output = ('Failed to create temporary directory. '
                          ...
                          'Consider changing the remote tmp path in distronode.cfg to a path rooted in "/tmp", for more error information use -vvv. '
            ...
            raise DistronodeConnectionFailure(output)
        else:
        ...
        if rc == '/':
            raise DistronodeError('failed to resolve remote temporary directory from %s: `%s` returned empty string' % (basefile, cmd))
    ...
    def _transfer_data(self, remote_path, data):
        ...
        try:
            ...
            raise DistronodeError("failure writing module data to temporary file for transfer: %s" % to_native(e))
    ...
    def _fixup_perms2(self, remote_paths, remote_user=None, execute=True):
        ...
        * If the above fails, we check if distronode_common_remote_group is set.
        ...
        * If (the chown fails AND distronode_common_remote_group is not set) OR
          (distronode_common_remote_group is set AND the chgrp (or following chmod)
          ...
          information we only do this if distronode is configured with
          "allow_world_readable_tmpfiles" in the distronode.cfg. Also note that
          when distronode_common_remote_group is set this final fallback is very
          unlikely to ever be triggered, so long as chgrp was successful. But
          just because the chgrp was successful, does not mean Distronode can
        ...
        if remote_user in self._get_admin_users():
            raise DistronodeError(
                'Failed to change ownership of the temporary files Distronode '
        ...
        except DistronodeAuthenticationFailure as e:
        ...
        # because chgrp exits successfully does not mean that Distronode will work.
            ...
            if res['rc'] == 0:
                ...
                if self.get_shell_option('world_readable_temp'):
                    display.warning(
                        ...
                        'successful, but there is no guarantee that Distronode '
        ...
        if self.get_shell_option('world_readable_temp'):
            ...
            display.warning(
                'Using world-readable permissions for temporary files Distronode '
        ...
        raise DistronodeError(
            'Failed to set permissions on the temporary files Distronode needs '
    ...
    def _execute_remote_stat(self, path, all_vars, follow, tmp=None, checksum=True):
        ...
        if mystat.get('failed'):
            ...
            if not msg:
                msg = mystat.get('msg')
            raise DistronodeError('Failed to get information on remote file (%s): %s' % (path, msg))
        ...
        if 'checksum' not in mystat['stat']:
            ...
            raise DistronodeError("Invalid checksum returned by stat: expected a string type but got %s" % type(mystat['stat']['checksum']))
    ...
    def _remote_expand_user(self, path, sudoable=True, pathsep=None):
        ...
        if '..' in os.path.dirname(expanded).split('/'):
            raise DistronodeError("'%s' returned an invalid relative home directory path containing '..'" % self._get_remote_addr({}))
    ...
    def _update_module_args(self, module_name, module_args, task_vars, ignore_unknown_opts: bool = False):
        ...
        if self._task.check_mode:
            if not self._supports_check_mode:
                raise DistronodeError("check mode is not supported for this operation")
            module_args['_distronode_check_mode'] = True
        else:
            module_args['_distronode_check_mode'] = False

        # set no log in the module arguments, if required
        no_target_syslog = C.config.get_config_value('DEFAULT_NO_TARGET_SYSLOG', variables=task_vars)
        module_args['_distronode_no_log'] = self._task.no_log or no_target_syslog

        # set debug in the module arguments, if required
        module_args['_distronode_debug'] = C.DEFAULT_DEBUG

        # let module know we are in diff mode
        module_args['_distronode_diff'] = self._task.diff

        # let module know our verbosity
        module_args['_distronode_verbosity'] = display.verbosity

        # give the module information about the distronode version
        module_args['_distronode_version'] = __version__

        # give the module information about its name
        module_args['_distronode_module_name'] = module_name

        # set the syslog facility to be used in the module
        module_args['_distronode_syslog_facility'] = task_vars.get('distronode_syslog_facility', C.DEFAULT_SYSLOG_FACILITY)

        # let module know about filesystems that selinux treats specially
        module_args['_distronode_selinux_special_fs'] = C.DEFAULT_SELINUX_SPECIAL_FS

        # what to do when parameter values are converted to strings
        module_args['_distronode_string_conversion_action'] = C.STRING_CONVERSION_ACTION

        # give the module the socket for persistent connections
        module_args['_distronode_socket'] = getattr(self._connection, 'socket_path')
        if not module_args['_distronode_socket']:
            module_args['_distronode_socket'] = task_vars.get('distronode_socket')

        # make sure all commands use the designated shell executable
        module_args['_distronode_shell_executable'] = self._play_context.executable

        # make sure modules are aware if they need to keep the remote files
        module_args['_distronode_keep_remote_files'] = C.DEFAULT_KEEP_REMOTE_FILES
        ...
        if self._is_become_unprivileged():  # force fallback on remote_tmp as user cannot normally write to dir
            module_args['_distronode_tmpdir'] = None
        else:
            module_args['_distronode_tmpdir'] = self._connection._shell.tmpdir
    ...
    def _execute_module(self, module_name=None, module_args=None, tmp=None, task_vars=None, persist_files=False, delete_remote_tmp=None, wrap_async=False,
        ...
        if wrap_async or self._task.async_val:
            async_dir = self.get_shell_option('async_dir', default="~/.distronode_async")
            remove_async_dir = len(self._task.environment)
            self._task.environment.append({"DISTRONODE_ASYNC_DIR": async_dir})

        # FUTURE: refactor this along with module build process to better encapsulate "smart wrapper" functionality
        (module_style, shebang, module_data, module_path) = self._configure_module(module_name=module_name, module_args=module_args, task_vars=task_vars)
        display.vvv("Using module file %s" % module_path)
        if not shebang and module_style != 'binary':
            raise DistronodeError("module (%s) is missing interpreter line" % module_name)
        ...
        # remove the DISTRONODE_ASYNC_DIR env entry if we added a temporary one for
        ...
        if wrap_async and not self._connection.always_pipeline_modules:
            ...
            (async_module_style, shebang, async_module_data, async_module_path) = self._configure_module(
                module_name='distronode.legacy.async_wrapper', module_args=dict(), task_vars=task_vars)
        ...
        if data.pop("_distronode_suppress_tmpdir_delete", False):
            self._cleanup_remote_tmp = False
        ...
        if 'results' in data and (not isinstance(data['results'], Sequence) or isinstance(data['results'], string_types)):
            data['distronode_module_results'] = data['results']
            del data['results']
            display.warning("Found internal 'results' key in module return, renamed to 'distronode_module_results'.")
        ...
        if self._discovered_interpreter_key:
            if data.get('distronode_facts') is None:
                data['distronode_facts'] = {}

            data['distronode_facts'][self._discovered_interpreter_key] = self._discovered_interpreter
    ...
    def _parse_returned_data(self, res):
        try:
            ...
            if C.MODULE_STRICT_UTF8_RESPONSE and not data.pop('_distronode_trusted_utf8', None):
                try:
                    _validate_utf8_json(data)
                except UnicodeEncodeError:
                    # When removing this, also remove the loop and latin-1 from distronode.module_utils.common.text.converters.jsonify
                    display.deprecated(
            ...
            data['_distronode_parsed'] = True
        except ValueError:
            # not valid json, lets try to capture error
            data = dict(failed=True, _distronode_parsed=False)
    ...
    def _low_level_execute_command(self, cmd, sudoable=True, in_data=None, executable=None, encoding_errors='surrogate_then_replace', chdir=None):
        ...
        # https://github.com/distronode/distronode/issues/68054
        if executable:
        ...
        if self._connection.allow_executable:
            if executable is None:
                ...
                # mitigation for SSH race which can drop stdout (https://github.com/distronode/distronode/issues/13876)
    ...
    def _get_diff_data(self, destination, source, task_vars, content, source_file=True):
        ...
        peek_result = self._execute_module(
            module_name='distronode.legacy.file', module_args=dict(path=destination, _diff_peek=True),
        ...
        if peek_result.get('rc', 0) == 0:
            ...
            if peek_result.get('state') in (None, 'absent'):
                ...
                dest_result = self._execute_module(
                    module_name='distronode.legacy.slurp', module_args=dict(path=destination),
                    task_vars=task_vars, persist_files=True)
                if 'content' in dest_result:
                    ...
                    if dest_result['encoding'] == u'base64':
                        ...
                        raise DistronodeError("unknown encoding in content option, failed: %s" % to_native(dest_result))
            ...
            if source_file:
                ...
                if C.MAX_FILE_SIZE_FOR_DIFF > 0 and st[stat.ST_SIZE] > C.MAX_FILE_SIZE_FOR_DIFF:
                    ...
                    try:
                        ...
                        raise DistronodeError("Unexpected error while reading source (%s) for diff: %s " % (source, to_native(e)))
