...
from distronode.errors import DistronodeError, DistronodeAction, _DistronodeActionDone, DistronodeActionFail
...
from distronode.utils.hashing import checksum_s
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            if src is None or dest is None:
                raise DistronodeActionFail("src and dest are required")

            if boolean(remote_src, strict=False):
                # call assemble via distronode.legacy to allow library/ overrides of the module without collection search
                result.update(self._execute_module(module_name='distronode.legacy.assemble', task_vars=task_vars))
                raise _DistronodeActionDone()
            else:
                try:
                    src = self._find_needle('files', src)
                except DistronodeError as e:
                    raise DistronodeActionFail(to_native(e))

            if not os.path.isdir(src):
                raise DistronodeActionFail(u"Source (%s) is not a directory" % src)
            ...
            if path_checksum != dest_stat['checksum']:
                ...
                res = self._execute_module(module_name='distronode.legacy.copy', module_args=new_module_args, task_vars=task_vars)
                if diff:
                    res['diff'] = diff
                result.update(res)
            else:
                result.update(self._execute_module(module_name='distronode.legacy.file', module_args=new_module_args, task_vars=task_vars))
