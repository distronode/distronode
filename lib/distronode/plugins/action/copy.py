...
from distronode.module_utils.basic import FILE_COMMON_ARGUMENTS
...
from distronode.utils.hashing import checksum
...
class ActionModule(ActionBase):
    ...
    def _copy_file(self, source_full, source_rel, content, content_tempfile,
        ...
        # If the local file does not exist, get_real_file() raises DistronodeFileNotFound
        try:
            source_full = self._loader.get_real_file(source_full, decrypt=decrypt)
        except DistronodeFileNotFound as e:
            result['failed'] = True
            result['msg'] = "could not find src=%s, %s" % (source_full, to_text(e))
            return result

        # Get the local mode and set if user wanted it preserved
        # https://github.com/distronode/distronode-modules-core/issues/1124
        lmode = None
        ...
        if local_checksum != dest_status['checksum']:
            ...
            module_return = self._execute_module(module_name='distronode.legacy.copy', module_args=new_module_args, task_vars=task_vars)
            ...
            # Fix for https://github.com/distronode/distronode-modules-core/issues/1568.
            ...
            module_return = self._execute_module(module_name='distronode.legacy.file', module_args=new_module_args, task_vars=task_vars)
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        # look up the files and use the first one we find as src
            ...
            result.update(self._execute_module(module_name='distronode.legacy.copy', task_vars=task_vars))
            return self._ensure_invocation(result)
        else:
