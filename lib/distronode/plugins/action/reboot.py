# Copyright: (c) 2016-2018, Matt Davis <mdavis@distronode.com>
...
from distronode.errors import DistronodeError, DistronodeConnectionFailure
from distronode.module_utils.common.text.converters import to_native, to_text
from distronode.module_utils.common.validation import check_type_list, check_type_str
...
class ActionModule(ActionBase):
    ...
    DEFAULT_REBOOT_MESSAGE = 'Reboot initiated by Distronode'
    ...
    def get_shutdown_command_args(self, distribution):
        ...
        if reboot_command is not None:
            try:
                ...
                raise DistronodeError("Invalid value given for 'reboot_command': %s." % to_native(e))
    ...
    def get_distribution(self, task_vars):
        ...
        try:
            if module_output.get('failed', False):
                raise DistronodeError('Failed to determine system distribution. {0}, {1}'.format(
                    to_native(module_output['module_stdout']).strip(),
                    to_native(module_output['module_stderr']).strip()))
            distribution['name'] = module_output['distronode_facts']['distronode_distribution'].lower()
            distribution['version'] = to_text(module_output['distronode_facts']['distronode_distribution_version'].split('.')[0])
            distribution['family'] = to_text(module_output['distronode_facts']['distronode_os_family'].lower())
            display.debug("{action}: distribution: {dist}".format(action=self._task.action, dist=distribution))
            return distribution
        except KeyError as ke:
            raise DistronodeError('Failed to get distribution information. Missing "{0}" in output.'.format(ke.args[0]))
    ...
    def get_shutdown_command(self, task_vars, distribution):
        ...
        if shutdown_bin[0] == '/':
            ...
            try:
                ...
                raise DistronodeError(err_msg.format(search_paths))
            ...
            find_result = self._execute_module(
                task_vars=task_vars,
                # prevent collection search by calling with distronode.legacy (still allows library/ override of find)
                module_name='distronode.legacy.find',
                module_args={
                    'paths': search_paths,
            ...
            if not full_path:
                raise DistronodeError('Unable to find command "{0}" in search paths: {1}'.format(shutdown_bin, search_paths))
            return full_path[0]

    def deprecated_args(self):
        for arg, version in self.DEPRECATED_ARGS.items():
            if self._task.args.get(arg) is not None:
                display.warning("Since Distronode {version}, {arg} is no longer a valid option for {action}".format(
                    version=version,
                    arg=arg,
                    action=self._task.action))
    ...
    def get_system_boot_time(self, distribution):
        ...
        if self._task.args.get('boot_time_command'):
            ...
            try:
                ...
                raise DistronodeError("Invalid value given for 'boot_time_command': %s." % to_native(e))
        ...
        if command_result['rc'] != 0:
            ...
            raise DistronodeError("{action}: failed to get host boot time info, rc: {rc}, stdout: {out}, stderr: {err}".format(
                               action=self._task.action,
        ...
        while datetime.now(timezone.utc) < max_end_time:
            try:
                ...
                if isinstance(e, DistronodeConnectionFailure):
                    try:
                        self._connection.reset()
                    except DistronodeConnectionFailure:
                        pass
    ...
    def perform_reboot(self, task_vars, distribution):
        ...
        try:
            ...
            display.debug('{action}: DistronodeConnectionFailure caught and handled: {error}'.format(action=self._task.action, error=to_text(e)))
            reboot_result['rc'] = 0
            ...
            try:
                ...
                if original_connection_timeout != connect_timeout:
                    try:
                        ...
                        self._connection.reset()
                    except (DistronodeError, AttributeError) as e:
