...
from distronode.module_utils.common.text import formatters
...
def unique(environment, a, case_sensitive=None, attribute=None):
    ...
    def _do_fail(e):
        if case_sensitive is False or attribute:
            raise DistronodeFilterError("Jinja2's unique filter failed and we cannot fall back to Distronode's version "
    ...
    try:
        ...
        error = e
        _do_fail(e)
        display.warning('Falling back to Distronode unique filter as Jinja2 one failed: %s' % to_text(e))

    if not HAS_UNIQUE or error:

        # handle Jinja2 specific attributes when using Distronode's version
        if case_sensitive is False or attribute:
            raise DistronodeFilterError("Distronode's unique filter does not support case_sensitive=False nor attribute parameters, "
...
def logarithm(x, base=math.e):
    try:
        ...
        raise DistronodeFilterTypeError('log() can only be used on numbers: %s' % to_native(e))
...
def power(x, y):
    try:
        ...
        raise DistronodeFilterTypeError('pow() can only be used on numbers: %s' % to_native(e))
...
def inversepower(x, base=2):
    try:
        ...
        raise DistronodeFilterTypeError('root() can only be used on numbers: %s' % to_native(e))
...
def human_readable(size, isbits=False, unit=None):
    ...
    try:
        ...
        raise DistronodeFilterTypeError("human_readable() failed on bad input: %s" % to_native(e))
    except Exception:
        raise DistronodeFilterError("human_readable() can't interpret following string: %s" % size)
...
def human_to_bytes(size, default_unit=None, isbits=False):
    ...
    try:
        ...
        raise DistronodeFilterTypeError("human_to_bytes() failed on bad input: %s" % to_native(e))
    except Exception:
        raise DistronodeFilterError("human_to_bytes() can't interpret following string: %s" % size)
...
def rekey_on_member(data, key, duplicates='error'):
    ...
    if duplicates not in ('error', 'overwrite'):
        raise DistronodeFilterError("duplicates parameter to rekey_on_member has unknown value: {0}".format(duplicates))
    ...
    if isinstance(data, Mapping):
        ...
        raise DistronodeFilterTypeError("Type is not a valid list, set, or dict")

    for item in iterate_over:
        if not isinstance(item, Mapping):
            raise DistronodeFilterTypeError("List item is not a valid dict")

        try:
            key_elem = item[key]
        except KeyError:
            raise DistronodeFilterError("Key {0} was not found".format(key))
        except TypeError as e:
            raise DistronodeFilterTypeError(to_native(e))
        except Exception as e:
            raise DistronodeFilterError(to_native(e))

        # Note: if new_obj[key_elem] exists it will always be a non-empty dict (it will at
        # minimum contain {key: key_elem}
        if new_obj.get(key_elem, None):
            if duplicates == 'error':
                raise DistronodeFilterError("Key {0} is not unique, cannot correctly turn into dict".format(key_elem))
...
class FilterModule(object):
    ''' Distronode math jinja2 filters '''
