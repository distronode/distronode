...
from distronode.inventory.group import to_safe_group_name as original_safe
from distronode.parsing.utils.addresses import parse_address
from distronode.plugins import DistronodePlugin
from distronode.plugins.cache import CachePluginAdjudicator as CacheObject
...
from distronode.utils.vars import combine_vars, load_extra_vars
...
def expand_hostname_range(line=None):
    ...
    References: https://docs.distronode.com/distronode/latest/user_guide/intro_inventory.html#hosts-and-groups
    '''
    all_hosts = []
    if line:
        ...
        if len(bounds) != 2 and len(bounds) != 3:
            raise DistronodeError("host range must be begin:end or begin:end:step")
        beg = bounds[0]
        end = bounds[1]
        if len(bounds) == 2:
            step = 1
        ...
        if not beg:
            beg = "0"
        if not end:
            raise DistronodeError("host range must specify end value")
        if beg[0] == '0' and len(beg) > 1:
            rlen = len(beg)  # range length formatting hint
            if rlen != len(end):
                raise DistronodeError("host range must specify equal-length begin and end formats")
        ...
        try:
            ...
            if i_beg > i_end:
                raise DistronodeError("host range must have begin <= end")
...
def get_cache_plugin(plugin_name, **kwargs):
    try:
        ...
        if 'fact_caching_connection' in to_native(e):
            raise DistronodeError("error, '%s' inventory cache plugin requires the one of the following to be set "
                               "to a writeable directory path:\ndistronode.cfg:\n[default]: fact_caching_connection,\n"
                               "[inventory]: cache_connection;\nEnvironment:\nDISTRONODE_INVENTORY_CACHE_CONNECTION,\n"
                               "DISTRONODE_CACHE_PLUGIN_CONNECTION." % plugin_name)
        else:
            raise e

    if plugin_name != 'memory' and kwargs and not getattr(cache._plugin, '_options', None):
        raise DistronodeError('Unable to use cache plugin {0} for inventory. Cache options were provided but may not reconcile '
                           'correctly unless set via set_options. Refer to the porting guide if the plugin derives user settings '
                           'from distronode.constants.'.format(plugin_name))
    return cache


class BaseInventoryPlugin(DistronodePlugin):
    ...
    def _populate_host_vars(self, hosts, variables, group=None, port=None):
        if not isinstance(variables, Mapping):
            raise DistronodeParserError("Invalid data from file, expected dictionary and got:\n\n%s" % to_native(variables))
    ...
    def _read_config_data(self, path):
        ...
        try:
            ...
            raise DistronodeParserError(to_native(e))
        ...
        if not config:
            # no data
            raise DistronodeParserError("%s is empty" % (to_native(path)))
        elif config.get('plugin') not in valid_names:
            # this is not my config file
            raise DistronodeParserError("Incorrect plugin name in file: %s" % config.get('plugin', 'none found'))
        elif not isinstance(config, Mapping):
            # configs are dictionaries
            raise DistronodeParserError('inventory source has invalid structure, it should be a dictionary, got: %s' % type(config))
    ...
    def _consume_options(self, data):
        ''' update existing options from alternate configuration sources not normally used by Distronode.
...
class Constructable(object):
    ...
    def _compose(self, template, variables, disable_lookups=True):
        ''' helper method for plugins to compose variables for Distronode based on jinja2 expression and inventory vars'''
        t = self.templar
        ...
        if compose and isinstance(compose, dict):
            for varname in compose:
                try:
                    ...
                    if strict:
                        raise DistronodeError("Could not set %s for host %s: %s" % (varname, host, to_native(e)))
                    continue
        ...
        if groups and isinstance(groups, dict):
            ...
            for group_name in groups:
                ...
                try:
                    ...
                    if strict:
                        raise DistronodeParserError("Could not add host %s to group %s: %s" % (host, group_name, to_native(e)))
                    continue
            ...
            for keyed in keys:
                if keyed and isinstance(keyed, dict):
                    ...
                    try:
                        ...
                        if strict:
                            raise DistronodeParserError("Could not generate group for host %s from %s entry: %s" % (host, keyed.get('key'), to_native(e)))
                        continue
                    ...
                    if trailing_separator is not None and default_value_name is not None:
                        raise DistronodeParserError("parameters are mutually exclusive for keyed groups: default_value|trailing_separator")
                    if key or (key == '' and default_value_name is not None):
                        ...
                        if raw_parent_name:
                            try:
                                ...
                                if strict:
                                    raise DistronodeParserError("Could not generate parent group %s for group %s: %s" % (raw_parent_name, key, to_native(e)))
                                continue
                        ...
                        if isinstance(key, string_types):
                            ...
                            raise DistronodeParserError("Invalid group name format, expected a string or a list of them or dictionary, got: %s" % type(key))
                        ...
                        if strict and key not in ([], {}):
                            raise DistronodeParserError("No key or key resulted empty for %s in host %s, invalid entry" % (keyed.get('key'), host))
                else:
                    raise DistronodeParserError("Invalid keyed group entry, it must be a dictionary: %s " % keyed)
