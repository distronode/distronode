...
class DistronodePlugin(ABC):
    ...
    def matches_name(self, possible_names):
        ...
        for name in possible_names:
            if '.' not in name:
                possible_fqcns.add(f"distronode.builtin.{name}")
            elif name.startswith("distronode.legacy."):
                possible_fqcns.add(name.removeprefix("distronode.legacy."))
            possible_fqcns.add(name)
        return bool(possible_fqcns.intersection(set(self.distronode_aliases)))
...
class DistronodeJinja2Plugin(DistronodePlugin):

    def __init__(self, function):

        super(DistronodeJinja2Plugin, self).__init__()
        self._function = function

    @property
    def plugin_type(self):
        return self.__class__.__name__.lower().replace('distronodejinja2', '')
