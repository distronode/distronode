...
# (c) 2015, 2017 Toshio Kuratomi <tkuratomi@distronode.com>
...
DOCUMENTATION = '''
    ...
    description:
        - This connection plugin allows distronode to execute tasks on the Distronode 'controller' instead of on a remote host.
...
class Connection(ConnectionBase):
    ...
    def exec_command(self, cmd: str, in_data: bytes | None = None, sudoable: bool = True) -> tuple[int, bytes, bytes]:
        ...
        if not os.path.exists(to_bytes(executable, errors='surrogate_or_strict')):
            raise DistronodeError("failed to find the executable specified %s."
            ...
            try:
                while not self.become.check_success(become_output) and not self.become.check_password_prompt(become_output):
                    ...
                    if not events:
                        ...
                        raise DistronodeError('timeout waiting for privilege escalation password prompt:\n' + to_native(become_output))
                    ...
                    if not chunk:
                        stdout, stderr = p.communicate()
                        raise DistronodeError('privilege output closed while waiting for password prompt:\n' + to_native(become_output))
                    become_output += chunk
            finally:
                selector.close()
    ...
    def put_file(self, in_path: str, out_path: str) -> None:
        ...
        try:
            ...
            raise DistronodeError("failed to copy: {0} and {1} are the same".format(to_native(in_path), to_native(out_path)))
        except IOError as e:
            raise DistronodeError("failed to transfer file to {0}: {1}".format(to_native(out_path), to_native(e)))
