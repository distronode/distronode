...
- This is similar to the P(distronode.builtin.winrm#connection) connection plugin which uses the same
...
options:
  ...
  remote_password:
    ...
    - name: distronode_winrm_pass
    - name: distronode_winrm_password
    aliases:
  ...
  protocol:
    ...
    type: str
    vars:
    - name: distronode_psrp_protocol
  path:
    description:
    - The URI path to connect to.
    type: str
    vars:
    - name: distronode_psrp_path
    default: 'wsman'
  auth:
    description:
  ...
  cert_validation:
    ...
    type: str
    vars:
    - name: distronode_psrp_cert_validation
  ca_cert:
    description:
    ...
    type: path
    vars:
    - name: distronode_psrp_cert_trust_path
    - name: distronode_psrp_ca_cert
  ...
  connection_timeout:
    ...
    type: int
    vars:
    - name: distronode_psrp_connection_timeout
    default: 30
  read_timeout:
    description:
    ...
    type: int
    vars:
    - name: distronode_psrp_read_timeout
    default: 30
    version_added: '2.8'
  reconnection_retries:
    ...
    type: int
    vars:
    - name: distronode_psrp_reconnection_retries
    default: 0
    version_added: '2.8'
  reconnection_backoff:
    ...
    - This is measured in seconds.
    - The C(distronode_psrp_reconnection_backoff) variable was added in Distronode
      2.9.
    type: int
    vars:
    - name: distronode_psrp_connection_backoff
    - name: distronode_psrp_reconnection_backoff
    default: 2
    version_added: '2.8'
  message_encryption:
    description:
    ...
    type: str
    vars:
    - name: distronode_psrp_message_encryption
    choices:
    - auto
    - always
    - never
  ...
  ignore_proxy:
    ...
    vars:
    - name: distronode_psrp_ignore_proxy
    type: bool
    default: false
  ...
  certificate_key_pem:
    ...
    type: path
    vars:
    - name: distronode_psrp_certificate_key_pem
  certificate_pem:
    description:
    - The local path to an X509 certificate to use with certificate auth.
    type: path
    vars:
    - name: distronode_psrp_certificate_pem
  credssp_auth_mechanism:
    ...
    default: auto
    vars:
    - name: distronode_psrp_credssp_auth_mechanism
  credssp_disable_tlsv1_2:
    ...
    type: bool
    vars:
    - name: distronode_psrp_credssp_disable_tlsv1_2
  credssp_minimum_version:
    ...
    type: int
    vars:
    - name: distronode_psrp_credssp_minimum_version
  negotiate_delegate:
    description:
    ...
    type: bool
    vars:
    - name: distronode_psrp_negotiate_delegate
  negotiate_hostname_override:
    description:
    - Override the remote hostname when searching for the host in the Kerberos
      lookup.
    - This allows Distronode to connect over IP but authenticate with the remote
    ...
    type: str
    vars:
    - name: distronode_psrp_negotiate_hostname_override
  negotiate_send_cbt:
    description:
    ...
    type: bool
    vars:
    - name: distronode_psrp_negotiate_send_cbt
  negotiate_service:
    description:
    ...
    type: str
    vars:
    - name: distronode_psrp_negotiate_service
  ...
  operation_timeout:
    ...
    type: int
    vars:
    - name: distronode_psrp_operation_timeout
    default: 20
  max_envelope_size:
    description:
    ...
    type: int
    vars:
    - name: distronode_psrp_max_envelope_size
    default: 153600
  configuration_name:
    description:
    ...
    type: str
    vars:
    - name: distronode_psrp_configuration_name
...
from distronode.errors import DistronodeConnectionFailure, DistronodeError
...
from distronode.plugins.shell.powershell import ShellModule as PowerShellPlugin
from distronode.plugins.shell.powershell import _common_args
from distronode.utils.display import Display
from distronode.utils.hashing import sha1
...
class Connection(ConnectionBase):
    ...
    def _connect(self) -> Connection:
        if not HAS_PYPSRP:
            raise DistronodeError("pypsrp or dependencies are not installed: %s"
        ...
        if not self.runspace:
            ...
            self.host = PSHost(None, None, False, "Distronode PSRP Host", None,
                               host_ui, None)
            ...
            try:
                ...
                raise DistronodeConnectionFailure("failed to authenticate with "
    ...
    def exec_command(self, cmd: str, in_data: bytes | None = None, sudoable: bool = True) -> tuple[int, bytes, bytes]:
        ...
        if cmd.startswith(" ".join(_common_args) + " -EncodedCommand"):
            ...
            if pwsh_in_data and isinstance(pwsh_in_data, str) and pwsh_in_data.startswith("#!"):
                ...
                raise DistronodeError("cannot run the interpreter '%s' on the psrp "
...
'''
        ...
        if not os.path.exists(b_in_path):
            raise DistronodeFileNotFound('file or module does not exist: "%s"' % to_native(in_path))
        ...
        if rc != 0:
            raise DistronodeError(to_native(stderr))
        ...
        if not remote_sha1:
            raise DistronodeError("Remote sha1 was not returned, stdout: '%s', stderr: '%s'"
                               % (to_native(stdout), to_native(stderr)))

        if not remote_sha1 == local_sha1:
            raise DistronodeError("Remote sha1 hash %s does not match local hash %s"
...
if ($bytes_read -gt 0) {
    ...
    Write-Output -InputObject ([System.Convert]::ToBase64String($bytes))
        ...
        if rc != 0:
            raise DistronodeError("failed to setup file stream for fetch '%s': %s"
    ...
    def _build_kwargs(self) -> None:
        ...
        extra_args = {v.replace('distronode_psrp_', '') for v in self.get_option('_extras')}
        unsupported_args = extra_args.difference(supported_args)

        for arg in unsupported_args:
            display.warning("distronode_psrp_%s is unsupported by the current "
        ...
        if hasattr(pypsrp, 'FEATURES') and 'wsman_read_timeout' in pypsrp.FEATURES:
            ...
            display.warning("distronode_psrp_read_timeout is unsupported by the current psrp version installed, "
                            "using distronode_psrp_connection_timeout value for read_timeout instead.")
        ...
        if hasattr(pypsrp, 'FEATURES') and 'wsman_reconnections' in pypsrp.FEATURES:
            ...
            if self._psrp_reconnection_retries is not None:
                display.warning("distronode_psrp_reconnection_retries is unsupported by the current psrp version installed.")
            if self._psrp_reconnection_backoff is not None:
                display.warning("distronode_psrp_reconnection_backoff is unsupported by the current psrp version installed.")

        # add in the extra args that were set
        for arg in extra_args.intersection(supported_args):
            option = self.get_option('_extras')['distronode_psrp_%s' % arg]
    ...
    def _parse_pipeline_result(self, pipeline: PowerShell) -> tuple[int, bytes, bytes]:
        ...
        output into the format that Distronode understands.
