...
DOCUMENTATION = '''
    ...
    description:
        - This connection plugin allows Distronode to communicate to the target machines through normal SSH command line.
        - Distronode does not expose a channel to allow communication between the user and the SSH process to accept
    ...
    options:
      host:
          ...
          vars:
               ...
               - name: distronode_host
               - name: distronode_ssh_host
      ...
      host_key_checking:
          ...
          ini:
              ...
              - section: ssh_connection
                ...
                version_added: '2.5'
          env:
              - name: DISTRONODE_HOST_KEY_CHECKING
              - name: DISTRONODE_SSH_HOST_KEY_CHECKING
                version_added: '2.5'
          vars:
              - name: distronode_host_key_checking
                version_added: '2.5'
      ...
      password:
          ...
          vars:
              ...
              - name: distronode_ssh_pass
              - name: distronode_ssh_password
      sshpass_prompt:
          description:
          ...
          ini:
              - section: 'ssh_connection'
                key: 'sshpass_prompt'
          env:
              - name: DISTRONODE_SSHPASS_PROMPT
          vars:
              - name: distronode_sshpass_prompt
          version_added: '2.10'
      ssh_args:
      ...
      ssh_common_args:
          ...
          ini:
              - section: 'ssh_connection'
                ...
                version_added: '2.7'
          env:
              - name: DISTRONODE_SSH_COMMON_ARGS
                version_added: '2.7'
          vars:
              - name: distronode_ssh_common_args
          cli:
      ...
      ssh_executable:
          ...
          type: string
          env: [{name: DISTRONODE_SSH_EXECUTABLE}]
          ini:
          - {key: ssh_executable, section: ssh_connection}
          #const: DISTRONODE_SSH_EXECUTABLE
          version_added: "2.2"
          vars:
              - name: distronode_ssh_executable
                version_added: '2.7'
      sftp_executable:
          default: sftp
          description:
          ...
          type: string
          env: [{name: DISTRONODE_SFTP_EXECUTABLE}]
          ini:
          - {key: sftp_executable, section: ssh_connection}
          version_added: "2.6"
          vars:
              - name: distronode_sftp_executable
                version_added: '2.7'
      scp_executable:
          default: scp
          description:
          ...
          type: string
          env: [{name: DISTRONODE_SCP_EXECUTABLE}]
          ini:
          - {key: scp_executable, section: ssh_connection}
          version_added: "2.6"
          vars:
              - name: distronode_scp_executable
                version_added: '2.7'
      scp_extra_args:
          description: Extra exclusive to the C(scp) CLI
          type: string
          vars:
              - name: distronode_scp_extra_args
          env:
            - name: DISTRONODE_SCP_EXTRA_ARGS
              version_added: '2.7'
          ini:
      ...
      sftp_extra_args:
          ...
          type: string
          vars:
              - name: distronode_sftp_extra_args
          env:
            - name: DISTRONODE_SFTP_EXTRA_ARGS
              version_added: '2.7'
          ini:
      ...
      ssh_extra_args:
          ...
          type: string
          vars:
              - name: distronode_ssh_extra_args
          env:
            - name: DISTRONODE_SSH_EXTRA_ARGS
              version_added: '2.7'
          ini:
      ...
      reconnection_retries:
          description:
            ...
            - Distronode retries connections only if it gets an SSH error with a return code of 255.
            - Any errors with return codes other than 255 indicate an issue with program execution.
          default: 0
          type: integer
          env:
            - name: DISTRONODE_SSH_RETRIES
          ini:
            - section: connection
              key: retries
            - section: ssh_connection
      ...
      port:
          ...
          ini:
            - section: defaults
              key: remote_port
          env:
            - name: DISTRONODE_REMOTE_PORT
          vars:
            - name: distronode_port
            - name: distronode_ssh_port
          keyword:
            - name: port
      remote_user:
          description:
              - User name with which to login to the remote server, normally set by the remote_user keyword.
              - If no user is supplied, Distronode will let the SSH client binary choose the user as it normally.
          type: string
          ini:
            - section: defaults
              key: remote_user
          env:
            - name: DISTRONODE_REMOTE_USER
          vars:
            - name: distronode_user
            - name: distronode_ssh_user
          cli:
            - name: user
          keyword:
            - name: remote_user
      pipelining:
          env:
            - name: DISTRONODE_PIPELINING
            - name: DISTRONODE_SSH_PIPELINING
          ini:
            - section: defaults
              key: pipelining
            - section: connection
      ...
      private_key_file:
          ...
          env:
            - name: DISTRONODE_PRIVATE_KEY_FILE
          vars:
            - name: distronode_private_key_file
            - name: distronode_ssh_private_key_file
          cli:
      ...
      control_path:
        description:
          ...
          - Since 2.3, if null (default), distronode will generate a unique hash. Use ``%(directory)s`` to indicate where to use the control dir path setting.
          - Before 2.3 it defaulted to ``control_path=%(directory)s/distronode-ssh-%%h-%%p-%%r``.
          - Be aware that this setting is ignored if C(-o ControlPath) is set in ssh args.
        type: string
        env:
          - name: DISTRONODE_SSH_CONTROL_PATH
        ini:
          - key: control_path
            section: ssh_connection
        vars:
          - name: distronode_control_path
            version_added: '2.7'
      control_path_dir:
        default: ~/.distronode/cp
        description:
          - This sets the directory to use for ssh control path if the control path setting is null.
          - Also, provides the ``%(directory)s`` variable for the control path setting.
        type: string
        env:
          - name: DISTRONODE_SSH_CONTROL_PATH_DIR
        ini:
          - section: ssh_connection
            key: control_path_dir
        vars:
          - name: distronode_control_path_dir
            version_added: '2.7'
      sftp_batch_mode:
        default: true
        description: 'TODO: write it'
        env: [{name: DISTRONODE_SFTP_BATCH_MODE}]
        ini:
        - {key: sftp_batch_mode, section: ssh_connection}
        type: bool
        vars:
          - name: distronode_sftp_batch_mode
            version_added: '2.7'
      ssh_transfer_method:
        description:
        ...
        type: string
        env: [{name: DISTRONODE_SSH_TRANSFER_METHOD}]
        ini:
            - {key: transfer_method, section: ssh_connection}
        vars:
            - name: distronode_ssh_transfer_method
              version_added: '2.12'
      scp_if_ssh:
        deprecated:
        ...
        description:
          - "Preferred method to use when transferring files over SSH."
          - When set to V(smart), Distronode will try them until one succeeds or they all fail.
        ...
        env: [{name: DISTRONODE_SCP_IF_SSH}]
        ini:
        - {key: scp_if_ssh, section: ssh_connection}
        vars:
          - name: distronode_scp_if_ssh
            version_added: '2.7'
      use_tty:
        version_added: '2.5'
        default: true
        description: add -tt to ssh commands to force tty allocation.
        env: [{name: DISTRONODE_SSH_USETTY}]
        ini:
        - {key: usetty, section: ssh_connection}
        type: bool
        vars:
          - name: distronode_ssh_use_tty
            version_added: '2.7'
      timeout:
        default: 10
        description:
        ...
        env:
            - name: DISTRONODE_TIMEOUT
            - name: DISTRONODE_SSH_TIMEOUT
              version_added: '2.11'
        ini:
            - key: timeout
              section: defaults
      ...
      pkcs11_provider:
        ...
        env: [{name: DISTRONODE_PKCS11_PROVIDER}]
        ini:
          - {key: pkcs11_provider, section: ssh_connection}
        vars:
          - name: distronode_ssh_pkcs11_provider
'''
...
from distronode.errors import (
    DistronodeAuthenticationFailure,
    DistronodeConnectionFailure,
    DistronodeError,
...
from distronode.plugins.connection import ConnectionBase, BUFSIZE
from distronode.plugins.shell.powershell import _parse_clixml
from distronode.utils.display import Display
from distronode.utils.path import unfrackpath, makedirs_safe
...
class DistronodeControlPersistBrokenPipeError(DistronodeError):
...
def _handle_error(
    ...
    if command == b'sshpass':
        ...
        if return_tuple[0] == 5:
            ...
            raise DistronodeAuthenticationFailure(msg)
        ...
        # sshpass_prompt with an sshpass that won't let us pass -P, in which case we fail loudly.
            ...
            if no_log:
                ...
                if "sshpass: invalid option -- 'P'" in details:
                    ...
                    raise DistronodeError('{0} {1}'.format(msg, details))
    ...
    if return_tuple[0] == 255:
        ...
        if SSH_ERROR:
            ...
            if no_log:
                ...
                msg = '{0} {1}'.format(msg, to_native(return_tuple[2]).rstrip())
            raise DistronodeConnectionFailure(msg)
    ...
    def wrapped(self: Connection, *args: P.args, **kwargs: P.kwargs) -> tuple[int, bytes, bytes]:
        ...
        for attempt in range(remaining_tries):
            ...
            try:
                ...
                except (DistronodeControlPersistBrokenPipeError):
            ...
            # 5 = Invalid/incorrect password from sshpass
            except DistronodeAuthenticationFailure:
                # Raising this exception, which is subclassed from DistronodeConnectionFailure, prevents further retries
                raise

            except (DistronodeConnectionFailure, Exception) as e:
                ...
                if attempt == remaining_tries - 1:
                    ...
                    if isinstance(e, DistronodeConnectionFailure):
...
class Connection(ConnectionBase):
    ...
    def _build_command(self, binary: str, subsystem: str, *other_args: bytes | str) -> list[bytes]:
        ...
        if conn_password or pkcs11_provider:
            if not self._sshpass_available():
                raise DistronodeError("to use the 'ssh' connection type with passwords or pkcs11_provider, you must install the sshpass program")
            if not conn_password and pkcs11_provider:
                raise DistronodeError("to use pkcs11_provider you must specify a password/pin")
        ...
        if ssh_args:
            ...
            self._add_args(b_command, b_args, u"distronode.cfg set ssh_args")
        ...
        if self.get_option('host_key_checking') is False:
            ...
            self._add_args(b_command, b_args, u"DISTRONODE_HOST_KEY_CHECKING/host_key_checking disabled")

        self.port = self.get_option('port')
        if self.port is not None:
            b_args = (b"-o", b"Port=" + to_bytes(self.port, nonstring='simplerepr', errors='surrogate_or_strict'))
            self._add_args(b_command, b_args, u"DISTRONODE_REMOTE_PORT/remote_port/distronode_port set")

        key = self.get_option('private_key_file')
        if key:
            b_args = (b"-o", b'IdentityFile="' + to_bytes(os.path.expanduser(key), errors='surrogate_or_strict') + b'"')
            self._add_args(b_command, b_args, u"DISTRONODE_PRIVATE_KEY_FILE/private_key_file/distronode_ssh_private_key_file set")
        ...
        if not conn_password:
            self._add_args(
                ...
                u"distronode_password/distronode_ssh_password not set"
            )
        ...
        if self.user:
            self._add_args(
                ...
                u"DISTRONODE_REMOTE_USER/remote_user/distronode_user/user/-u set"
            )
        ...
        self._add_args(
            ...
            u"DISTRONODE_TIMEOUT/timeout set"
        )
        ...
        if controlpersist:
            ...
            if not controlpath:
                ...
                if not os.access(b_cpdir, os.W_OK):
                    raise DistronodeError("Cannot write to ControlPath %s" % to_native(cpdir))
    ...
    def _send_initial_data(self, fh: io.IOBase, in_data: bytes, ssh_process: subprocess.Popen) -> None:
        ...
        try:
            ...
            # Only raise DistronodeConnectionFailure if the ssh process is still alive
            time.sleep(0.001)
            ssh_process.poll()
    ...
    def _bare_run(self, cmd: list[bytes], in_data: bytes | None, sudoable: bool = True, checkrc: bool = True) -> tuple[int, bytes, bytes]:
        ...
        if not p:
            try:
                ...
                raise DistronodeError('Unable to execute ssh command line on a controller due to: %s' % to_native(e))
            ...
            while True:
                ...
                if not events:
                    ...
                    if state <= states.index('awaiting_escalation'):
                        ...
                        raise DistronodeError('Timeout (%ds) waiting for privilege escalation prompt: %s' % (timeout, to_native(b_stdout)))
                ...
                if states[state] == 'awaiting_escalation':
                    if self._flags['become_success']:
                        ...
                        raise DistronodeError('Incorrect %s password' % self.become.name)
                    elif self._flags['become_nopasswd_error']:
                        display.vvv(u'Escalation requires password')
                        self._terminate_process(p)
                        self._flags['become_nopasswd_error'] = False
                        raise DistronodeError('Missing %s password' % self.become.name)
        ...
        if self.get_option('host_key_checking'):
            if cmd[0] == b"sshpass" and p.returncode == 6:
                raise DistronodeError('Using a SSH password instead of a key is not possible because Host Key checking is enabled and sshpass does not support '
                                   'this.  Please add this host\'s fingerprint to your known_hosts file to manage this host.')

        controlpersisterror = b'Bad configuration option: ControlPersist' in b_stderr or b'unknown configuration option: ControlPersist' in b_stderr
        if p.returncode != 0 and controlpersisterror:
            raise DistronodeError('using -c ssh on certain older ssh versions may not support ControlPersist, set DISTRONODE_SSH_ARGS="" '
        ...
        if p.returncode == 255:
            ...
            if controlpersist_broken_pipe:
                raise DistronodeControlPersistBrokenPipeError('Data could not be sent because of ControlPersist broken pipe: %s' % additional)

            elif in_data and checkrc:
                raise DistronodeConnectionFailure('Data could not be sent to remote host "%s". Make sure this host can be reached over ssh: %s'
    ...
    def _file_transport_command(self, in_path: str, out_path: str, sftp_action: str) -> tuple[int, bytes, bytes]:
        ...
        if ssh_transfer_method is not None:
            ...
            if not isinstance(scp_if_ssh, bool):
                ...
                if scp_if_ssh in BOOLEANS:
                    ...
                    raise DistronodeOptionsError('scp_if_ssh needs to be one of [smart|True|False]')
            if scp_if_ssh == 'smart':
                methods = smart_methods
        ...
        for method in methods:
            ...
            if returncode == 0:
                ...
                if len(methods) > 1:
                    display.warning(u'%s transfer mechanism failed on %s. Use DISTRONODE_DEBUG=1 to see detailed information' % (method, host))
                    display.debug(u'%s' % to_text(stdout))
                    display.debug(u'%s' % to_text(stderr))

        if returncode == 255:
            raise DistronodeConnectionFailure("Failed to connect to the host via %s: %s" % (method, to_native(stderr)))
        else:
            raise DistronodeError("failed to transfer file to %s %s:\n%s\n%s" %
    ...
    def put_file(self, in_path: str, out_path: str) -> tuple[int, bytes, bytes]:  # type: ignore[override]  # Used by tests and would break API
        ...
        if not os.path.exists(to_bytes(in_path, errors='surrogate_or_strict')):
            raise DistronodeFileNotFound("file or module does not exist: {0}".format(to_native(in_path)))
