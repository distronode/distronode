...
from distronode.plugins.shell import ShellBase
from distronode.utils.display import Display
from distronode.plugins.loader import connection_loader, get_shell_plugin
...
class ConnectionBase(DistronodePlugin):
    '''
    ...
    def exec_command(self, cmd: str, in_data: bytes | None = None, sudoable: bool = True) -> tuple[int, bytes, bytes]:
        ...
        there.  It looks approximately like this::
            ...
            [LocalShell] ConnectionCommand [UsersLoginShell (*)] DISTRONODE_SHELL_EXECUTABLE [(BecomeCommand DISTRONODE_SHELL_EXECUTABLE)] Command
        ...
        :ConnectionCommand: This is the command that connects us to the remote
            machine to run the rest of the command.  ``distronode_user``,
            ``distronode_ssh_host`` and so forth are fed to this piece of the
        ...
        :UsersLoginShell: This shell may or may not be created depending on
            ...
            shell that the ``distronode_user`` has configured as their login
            ...
            connects to a remote machine.  ``distronode_shell_type`` may be set
            to inform distronode of differences in how the ``UsersLoginShell``
            handles things like quoting if a shell has different semantics
            than the Bourne shell.
        :DISTRONODE_SHELL_EXECUTABLE: This is the shell set via the inventory var
            ``distronode_shell_executable`` or via
            ``constants.DEFAULT_EXECUTABLE`` if the inventory var is not set.
            We explicitly invoke this shell so that we have predictable
            quoting rules at this point.  ``DISTRONODE_SHELL_EXECUTABLE`` is only
            ...
            that the ``DISTRONODE_SHELL_EXECUTABLE`` is being invoked directly.
        :BecomeComand DISTRONODE_SHELL_EXECUTABLE: Is the command that performs
            ...
            invoke a DISTRONODE_SHELL_EXECUTABLE shell inside of it so that we
            have a consistent view of quoting.
        :Command: Is the command we're actually trying to run remotely.
            (Examples: mkdir -p $HOME/.distronode, python $HOME/.distronode/tmp-script-file)
        """
        pass
    ...
    def update_vars(self, variables: dict[str, t.Any]) -> None:
        ...
        for varname in C.COMMON_CONNECTION_VARS:
            ...
            if varname in variables:
                ...
                # no secrets!
                continue
            elif varname == 'distronode_connection':
                # its me mom!
                value = self._load_name
            elif varname == 'distronode_shell_type' and self._shell:
                # its my cousin ...
...
class NetworkConnectionBase(ConnectionBase):
    ...
    def __init__(
        ...
        **kwargs: t.Any,
        ...
        self._distronode_playbook_pid = kwargs.get('distronode_playbook_pid')
