...
from .filter import DistronodeJinja2Filter
from .test import DistronodeJinja2Test

from distronode import __version__ as distronode_version
...
from distronode.plugins import get_plugin_class, MODULE_CACHE, PATH_CACHE, PLUGIN_PATH_CACHE
from distronode.utils.collection_loader import DistronodeCollectionConfig, DistronodeCollectionRef
from distronode.utils.collection_loader._collection_finder import _DistronodeCollectionFinder, _get_collection_metadata
from distronode.utils.display import Display
from distronode.utils.plugin_docs import add_fragments
...
def get_shell_plugin(shell_type=None, executable=None):
    ...
    if not shell_type:
        ...
        if executable:
            ...
            raise DistronodeError("Either a shell type or a shell executable must be provided ")

    shell = shell_loader.get(shell_type)
    if not shell:
        raise DistronodeError("Could not find the shell plugin required (%s)." % shell_type)
...
class PluginLoadContext(object):
    ...
    def resolved_fqcn(self):
        ...
        if not self._resolved_fqcn:
            ...
            if DistronodeCollectionRef.is_valid_fqcr(final_plugin) and final_plugin.startswith('distronode.legacy.'):
                final_plugin = final_plugin.split('distronode.legacy.')[-1]
            if self.plugin_resolved_collection and not DistronodeCollectionRef.is_valid_fqcr(final_plugin):
...
class PluginLoader:
    ...
    @property
    def type(self):
        return DistronodeCollectionRef.legacy_plugin_dir_to_plugin_type(self.subdir)
    ...
    def _get_paths_with_context(self, subdirs=True):
        ...
        # In current usage, everything calls this with subdirs=True except for module_utils_loader and distronode-doc
    ...
    def _load_config_defs(self, name, module, path):
        ...
        if self.class_name:
            ...
            if type_name in C.CONFIGURABLE_PLUGINS and not C.config.has_configuration_definition(type_name, name):
                dstring = DistronodeLoader(getattr(module, 'DOCUMENTATION', ''), file_name=path).get_single_data()
                ...
                #     # TODO: dstring = DistronodeLoader(, file_name=path).get_single_data()
    ...
    def _find_fq_plugin(self, fq_name, extension, plugin_load_context, ignore_deprecated=False):
        ...
        plugin_type = DistronodeCollectionRef.legacy_plugin_dir_to_plugin_type(self.subdir)

        acr = DistronodeCollectionRef.from_fqcr(fq_name, plugin_type)
        ...
        if routing_metadata:
            ...
            if tombstone:
                ...
                raise DistronodePluginRemovedError(removed_msg, plugin_load_context=plugin_load_context)

            redirect = routing_metadata.get('redirect', None)

            if redirect:
                # Prevent mystery redirects that would be determined by the collections keyword
                if not DistronodeCollectionRef.is_valid_fqcr(redirect):
        ...
        while True:
            ...
            if result.pending_redirect:
                if result.pending_redirect in result.redirect_list:
                    raise DistronodePluginCircularRedirect('plugin redirect loop resolving {0} (path: {1})'.format(result.original_name, result.redirect_list))
    ...
    def _resolve_plugin_step(self, name, mod_type='', ignore_deprecated=False,
        ...
        if mod_type:
            ...
            # Distronode plugins that run in the controller process (most plugins)
            suffix = '.py'
        else:
            # Only Distronode Modules.  Distronode modules can be any executable so
            # they can have any suffix
            suffix = ''

        # FIXME: need this right now so we can still load shipped PS module_utils- come up with a more robust solution
        if (DistronodeCollectionRef.is_valid_fqcr(name) or collection_list) and not name.startswith('Distronode'):
            ...
            for candidate_name in candidates:
                try:
                    ...
                    if candidate_name.startswith('distronode.legacy'):
                        # 'distronode.legacy' refers to the plugin finding behavior used before collections existed.
                        # They need to search 'library' and the various '*_plugins' directories in order to find the file.
                        plugin_load_context = self._find_plugin_legacy(name.removeprefix('distronode.legacy.'),
                                                                       plugin_load_context, ignore_deprecated, check_aliases, suffix)
                    else:
                        # 'distronode.builtin' should be handled here. This means only internal, or builtin, paths are searched.
                    ...
                    if plugin_load_context.resolved or plugin_load_context.pending_redirect:  # if we got an answer or need to chase down a redirect, return
                        return plugin_load_context
                except (DistronodePluginRemovedError, DistronodePluginCircularRedirect, DistronodeCollectionUnsupportedVersionError):
    ...
    def _find_plugin_legacy(self, name, plugin_load_context, ignore_deprecated=False, check_aliases=False, suffix=None):
        ...
        try:
            ...
            plugin_load_context.plugin_resolved_collection = 'distronode.builtin' if path_with_context.internal else ''
            plugin_load_context._resolved_fqcn = ('distronode.builtin.' + name if path_with_context.internal else name)
            plugin_load_context.resolved = True
            return plugin_load_context
        except KeyError:
        ...
        for path_with_context in (p for p in self._get_paths_with_context() if p.path not in self._searched_paths and os.path.isdir(to_bytes(p.path))):
            ...
            for full_path in (to_native(f) for f in full_paths if os.path.isfile(f) and not f.endswith(b'__init__.py')):
                ...
                # HACK: We have no way of executing python byte compiled files as distronode modules so specifically exclude them
        ...
        if not name.startswith('_'):
            ...
            if alias_name in pull_cache:
                ...
                if not ignore_deprecated and not os.path.islink(path_with_context.path):
                    ...
                    display.deprecated('%s is kept for backwards compatibility but usage is discouraged. '  # pylint: disable=distronode-deprecated-no-version
                                       'The module documentation details page may explain more about this rationale.' % name.lstrip('_'))
                plugin_load_context.plugin_resolved_path = path_with_context.path
                plugin_load_context.plugin_resolved_name = alias_name
                plugin_load_context.plugin_resolved_collection = 'distronode.builtin' if path_with_context.internal else ''
                plugin_load_context._resolved_fqcn = 'distronode.builtin.' + alias_name if path_with_context.internal else alias_name
                plugin_load_context.resolved = True
                return plugin_load_context

        # last ditch, if it's something that can be redirected, look for a builtin redirect before giving up
        candidate_fqcr = 'distronode.builtin.{0}'.format(name)
        if '.' not in name and DistronodeCollectionRef.is_valid_fqcr(candidate_fqcr):
    ...
    def has_plugin(self, name, collection_list=None):
        ...
        try:
            ...
            if isinstance(ex, DistronodeError):
                raise
    ...
    def _load_module_source(self, name, path):
        ...
        if name.startswith('distronode_collections.'):
            full_name = name
        else:
            full_name = '.'.join([self.package, name])

        if full_name in sys.modules:
            # Avoids double loading, See https://github.com/distronode/distronode/issues/13110
    ...
    def _update_object(self, obj, name, path, redirected_names=None, resolved=None):
        ...
        if not names:
            raise DistronodeError(f"Missing FQCN for plugin source {name}")

        setattr(obj, 'distronode_aliases', names)
        setattr(obj, 'distronode_name', names[0])
    ...
    def all(self, *args, **kwargs):
        ...
        if path_only and class_only:
            raise DistronodeError('Do not set both path_only and class_only when calling PluginLoader.all()')
        ...
        for path in all_matches:
            ...
            if path in legacy_excluding_builtin:
                ...
                fqcn = f"distronode.builtin.{basename}"
...
class Jinja2Loader(PluginLoader):
    ...
    def get_contained_plugins(self, collection, plugin_path, name):
        ...
        full_name = '.'.join(['distronode_collections', collection, 'plugins', self.type, name])
        try:
    ...
    def get_with_context(self, name, *args, **kwargs):
        ...
        name = name.removeprefix('distronode.legacy.')
        ...
        if known_plugin := self._cached_non_collection_wrappers.get(name):
            ...
            context.plugin_resolved_collection = 'distronode.builtin' if known_plugin.distronode_name.startswith('distronode.builtin.') else ''
            context._resolved_fqcn = known_plugin.distronode_name
        ...
        while True:
            ...
            if key in seen:
                raise DistronodeError('recursive collection redirect found for %r' % name, 0)
            seen.add(key)

            acr = DistronodeCollectionRef.try_parse_fqcr(key, self.type)
            if not acr:
            ...
            if tombstone_entry:
                ...
                raise DistronodePluginRemovedError(exc_msg)
        ...
        try:
        ...
        except DistronodePluginRemovedError as apre:
            raise DistronodeError(to_native(apre), 0, orig_exc=apre)
        except (DistronodeError, KeyError):
            raise
        except Exception as ex:
            ...
            raise DistronodeError(to_native(ex), 0, orig_exc=ex)
    ...
    def _ensure_non_collection_wrappers(self, *args, **kwargs):
        ...
        for p_map in super(Jinja2Loader, self).all(*args, **kwargs):
            is_builtin = p_map.distronode_name.startswith('distronode.builtin.')
            ...
            for plugin_name in plugins.keys():
                ...
                fqcn = plugin_name
                collection = '.'.join(p_map.distronode_name.split('.')[:2]) if p_map.distronode_name.count('.') >= 2 else ''
                ...
                if is_builtin:
                    target_names.add(f'distronode.builtin.{plugin_name}')
...
def get_fqcr_and_name(resource, collection='distronode.builtin'):
    if '.' not in resource:
        name = resource
...
def _load_plugin_filter():
    ...
    if C.PLUGIN_FILTERS_CFG is None:
        filter_cfg = '/etc/distronode/plugin_filters.yml'
    else:
    ...
    if os.path.exists(filter_cfg):
        ...
        if version == u'1.0':
            ...
            try:
                filters['distronode.modules'] = frozenset(filter_data['module_rejectlist'])
            except TypeError:
                display.warning(u'Unable to parse the plugin filter file {0} as'
                                u' module_rejectlist is not a list.'
                                u' Skipping.'.format(filter_cfg))
                return filters
            filters['distronode.plugins.action'] = filters['distronode.modules']
        else:
            display.warning(u'The plugin filter file, {0} was a version not recognized by this'
                            u' version of Distronode. Skipping.'.format(filter_cfg))
    else:
        if user_set:
...
def _on_collection_load_handler(collection_name, collection_path):
    ...
    try:
        if not _does_collection_support_distronode_version(collection_meta.get('requires_distronode', ''), distronode_version):
            mismatch_behavior = C.config.get_config_value('COLLECTIONS_ON_DISTRONODE_VERSION_MISMATCH')
            message = 'Collection {0} does not support Distronode version {1}'.format(collection_name, distronode_version)
            if mismatch_behavior == 'warning':
                display.warning(message)
            elif mismatch_behavior == 'error':
                raise DistronodeCollectionUnsupportedVersionError(message)
    except DistronodeError:
        raise
    except Exception as ex:
        display.warning('Error parsing collection metadata requires_distronode value from collection {0}: {1}'.format(collection_name, ex))


def _does_collection_support_distronode_version(requirement_string, distronode_version):
    if not requirement_string:
        return True

    if not SpecifierSet:
        display.warning('packaging Python module unavailable; unable to validate collection Distronode version requirements')
        return True

    ss = SpecifierSet(requirement_string)

    # ignore prerelease/postrelease/beta/dev flags for simplicity
    base_distronode_version = Version(distronode_version).base_version

    return ss.contains(base_distronode_version)


def _configure_collection_loader(prefix_collections_path=None):
    if DistronodeCollectionConfig.collection_finder:
        # this must be a Python warning so that it can be filtered out by the import sanity test
        warnings.warn('DistronodeCollectionFinder has already been configured')
        return
    ...
    finder = _DistronodeCollectionFinder(paths, C.COLLECTIONS_SCAN_SYS_PATH)
    finder._install()

    # this should succeed now
    DistronodeCollectionConfig.on_collection_load += _on_collection_load_handler
