...
def scm_archive_resource(src, scm='git', name=None, version='HEAD', keep_scm_meta=False):
    ...
    def run_scm_cmd(cmd, tempdir):
        try:
            ...
            raise DistronodeError("when executing %s: %s" % (ran, to_native(e)))
        if popen.returncode != 0:
            raise DistronodeError("- command %s failed in directory %s (rc=%s) - %s" % (' '.join(cmd), tempdir, popen.returncode, to_native(stderr)))

    if scm not in ['hg', 'git']:
        raise DistronodeError("- scm %s is not currently supported" % scm)

    try:
        scm_path = get_bin_path(scm)
    except (ValueError, OSError, IOError):
        raise DistronodeError("could not find/use %s, it is required to continue with installing %s" % (scm, src))
