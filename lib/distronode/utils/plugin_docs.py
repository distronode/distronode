...
from distronode.errors import DistronodeError, DistronodeParserError, DistronodePluginNotFound
from distronode.module_utils.six import string_types
from distronode.module_utils.common.text.converters import to_native
from distronode.parsing.plugin_docs import read_docstring
...
def add_fragments(doc, filename, fragment_loader, is_module=False):
    ...
    for fragment_slug in fragments:
        ...
        fragment = DistronodeLoader(fragment_yaml, file_name=filename).get_single_data()

        real_fragment_name = getattr(fragment_class, 'distronode_name')
            ...
            if doc_key in fragment:
                if doc_key in doc:
                    try:
                        ...
                        raise DistronodeError("%s %s (%s) of unknown type: %s" % (to_native(e), doc_key, fragment_name, filename))
                else:
        ...
        try:
            ...
            raise DistronodeError("%s (%s) of unknown type: %s" % (to_native(e), fragment_name, filename))

    if unknown_fragments:
        raise DistronodeError('unknown doc_fragment(s) in file {0}: {1}'.format(filename, to_native(', '.join(unknown_fragments))))
...
def get_versioned_doclink(path):
    ...
    try:
        ...
        if path.startswith('/'):
            path = path[1:]
        split_ver = distronode_version.split('.')
        if len(split_ver) < 3:
            raise RuntimeError('invalid version ({0})'.format(distronode_version))
...
def find_plugin_docfile(plugin, plugin_type, loader):
    ...
    if not context or not context.resolved:
        raise DistronodePluginNotFound('%s was not found' % (plugin), plugin_load_context=context)
    ...
    if filename is None:
        raise DistronodeError('%s cannot contain DOCUMENTATION nor does it have a companion documentation file' % (plugin))
...
def get_plugin_docs(plugin, plugin_type, loader, fragment_loader, verbose):
    ...
    try:
        ...
        raise DistronodeParserError('%s did not contain a DOCUMENTATION attribute (%s)' % (plugin, filename), orig_exc=e)
    ...
    if not docs[0]:
        for newfile in _find_adjacent(filename, plugin, C.DOC_EXTENSIONS):
            try:
                ...
                raise DistronodeParserError('Adjacent file %s did not contain a DOCUMENTATION attribute (%s)' % (plugin, filename), orig_exc=e)

    # add extra data to docs[0] (aka 'DOCUMENTATION')
    if docs[0] is None:
        raise DistronodeParserError('No documentation available for %s (%s)' % (plugin, filename))
    else:
