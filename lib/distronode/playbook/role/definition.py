...
from distronode.playbook.collectionsearch import CollectionSearch
...
from distronode.utils.collection_loader._collection_finder import _get_collection_role_path
...
class RoleDefinition(Base, Conditional, Taggable, CollectionSearch):
    ...
    def load(data, variable_manager=None, loader=None):
        raise DistronodeError("not implemented")
    ...
    def preprocess_data(self, ds):
        ...
        if not isinstance(ds, dict) and not isinstance(ds, string_types) and not isinstance(ds, DistronodeBaseYAMLObject):
    ...
    def _load_role_name(self, ds):
        ...
        if not role_name or not isinstance(role_name, string_types):
            raise DistronodeError('role definitions must contain a role name', obj=ds)
    ...
    def _load_role_path(self, role_name):
        ...
        if self._collection_list or DistronodeCollectionRef.is_valid_fqcr(role_name):
        ...
        # distronode.legacy searches with the collections keyword
        ...
        raise DistronodeError("the role '%s' was not found in %s" % (role_name, ":".join(searches)), obj=self._ds)
