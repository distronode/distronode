...
from distronode.errors import DistronodeParserError, DistronodeError
...
from distronode.playbook.helpers import load_list_of_roles
...
class RoleMetadata(Base, CollectionSearch):
    ...
    def load(data, owner, variable_manager=None, loader=None):
        ...
        if not isinstance(data, dict):
            raise DistronodeParserError("the 'meta/main.yml' for role %s is not a dictionary" % owner.get_name())
    ...
    def _load_dependencies(self, attr, ds):
        ...
        if ds:
            if not isinstance(ds, list):
                raise DistronodeParserError("Expected role dependencies to be a list.", obj=self._ds)

            for role_def in ds:
                # FIXME: consolidate with distronode-galaxy to keep this in sync
                ...
                try:
                    ...
                    raise DistronodeParserError(to_native(exc), obj=role_def, orig_exc=exc)
        ...
        if self._owner:
            ...
            if 'distronode.legacy' not in collection_search_list:
                collection_search_list.append('distronode.legacy')
        ...
        try:
            ...
            raise DistronodeParserError("A malformed list of role dependencies was encountered.", obj=self._ds, orig_exc=e)
