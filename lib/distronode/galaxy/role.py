...
class GalaxyRole(object):
    ...
    def install(self):
        ...
        if self.scm:
            ...
            if os.path.isfile(self.src):
                ...
                if not role_data:
                    raise DistronodeError("- sorry, %s was not found on %s." % (self.src, self.api.api_server))

                if role_data.get('role_type') == 'APP':
                    # Container Role
                    display.warning("%s is a Container App role, and should only be installed using Distronode "
                                    "Container" % self.name)
                ...
                if not self.version:
                    ...
                    if role_versions and to_text(self.version) not in [a.get('name', None) for a in role_versions]:
                        raise DistronodeError("- the specified version (%s) of %s was not found in the list of available versions (%s)." % (self.version,
                                                                                                                                         self.name,
                                                                                                                                         role_versions))
            ...
            raise DistronodeError("No valid role data found")
        ...
        if tmp_file:
            ...
            if not tarfile.is_tarfile(tmp_file):
                raise DistronodeError("the downloaded file does not appear to be a valid tar archive.")
            else:
                        ...
                        if meta_main in member.name:
                            ...
                            if not meta_file:
                                ...
                                if len(meta_parent_dir) < len(archive_parent_dir):
                                    ...
                                    meta_file = member
                if not meta_file:
                    raise DistronodeError("this role does not appear to have a meta/main.yml file.")
                else:
                    try:
                        self._metadata = yaml_load(role_tar_file.extractfile(meta_file))
                    except Exception:
                        raise DistronodeError("this role does not appear to have a valid meta/main.yml file.")
                    ...
                    try:
                        if os.path.exists(self.path):
                            if not os.path.isdir(self.path):
                                raise DistronodeError("the specified roles path exists and is not a directory.")
                            elif not context.CLIARGS.get("force", False):
                                raise DistronodeError("the specified role %s appears to already exist. Use --force to replace it." % self.name)
                            else:
                                # using --force, remove the old path
                                if not self.remove():
                                    raise DistronodeError("%s doesn't appear to contain a role.\n  please remove this directory manually if you really "
                        ...
                        if e.errno == errno.EACCES and idx < paths_len - 1:
                            continue
                        raise DistronodeError("Could not update files in %s: %s" % (self.path, to_native(e)))
    ...
    def requirements(self):
        ...
        if not isinstance(self._requirements, MutableSequence):
            raise DistronodeParserError(f"Expected role dependencies to be a list. Role {self} has meta/requirements.yml {self._requirements}")
