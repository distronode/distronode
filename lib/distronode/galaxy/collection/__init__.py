...
from distronode.galaxy.collection.galaxy_api_proxy import MultiGalaxyAPIProxy
from distronode.galaxy.collection.gpg import (
    run_gpg_verify,
    parse_gpg_errors,
...
try:
    from distronode.galaxy.dependency_resolution import (
        build_collection_dependency_resolver,
    )
    from distronode.galaxy.dependency_resolution.errors import (
        CollectionDependencyResolutionImpossible,
        CollectionDependencyInconsistentCandidate,
    )
    from distronode.galaxy.dependency_resolution.providers import (
        RESOLVELIB_VERSION,
...
from distronode.galaxy.dependency_resolution.dataclasses import (
    Candidate, Requirement, _is_installed_collection_dir,
)
from distronode.galaxy.dependency_resolution.versioning import meets_requirements
from distronode.plugins.loader import get_all_plugin_loaders
...
from distronode.utils.collection_loader import DistronodeCollectionRef
from distronode.utils.display import Display
from distronode.utils.hashing import secure_hash, secure_hash_s
...
def verify_local_collection(local_collection, remote_collection, artifacts_manager):
    ...
    if not keyring_configured and signatures:
        display.warning(
            ...
            "Configure a keyring for distronode-galaxy to verify "
    ...
    if verify_local_only:
        ...
        # fetch remote
        # NOTE: DistronodeError is raised on URLError
...
def build_collection(u_collection_path, u_output_path, force):
    ...
    """Creates the Distronode collection artifact in a .tar.gz file.
    ...
    try:
        ...
        raise DistronodeError(to_native(lookup_err)) from lookup_err
    ...
    if os.path.exists(b_collection_output):
        if os.path.isdir(b_collection_output):
            raise DistronodeError("The output collection artifact '%s' already exists, "
                               "but is a directory - aborting" % to_native(b_collection_output))
        elif not force:
            raise DistronodeError("The file '%s' already exists. You can use --force to re-create "
...
def download_collections(
    ...
    """Download Distronode collections as their tarball from a Galaxy server to the path specified and creates a requirements
...
def publish_collection(collection_path, api, wait, timeout):
    """Publish an Distronode collection tarball into an Distronode Galaxy server.
    ...
    if wait:
        ...
        if not task_id:
            raise DistronodeError("Publishing the collection did not return valid task info. Cannot wait for task status. Returned task info: '%s'" % import_uri)
...
def install_collections(
    ...
    """Install Distronode collections to the path specified.
        ...
        for fqcn, concrete_coll_pin in dependency_map.items():
            ...
            if not disable_gpg_verify and concrete_coll_pin.signatures and not keyring_exists:
                ...
                display.warning(
                    ...
                    "Configure a keyring for distronode-galaxy to use "
...
# NOTE: imported in distronode.cli.galaxy
def validate_collection_name(name):  # type: (str) -> str
    ...
    if DistronodeCollectionRef.is_valid_collection_name(collection):
        return name

    raise DistronodeError("Invalid collection name '%s', "
...
def validate_collection_path(collection_path):  # type: (str) -> str
    """Ensure a given path ends with 'distronode_collections'

    :param collection_path: The path that should end in 'distronode_collections'
    :return: collection_path ending in 'distronode_collections' if it does not already.
    """

    if os.path.split(collection_path)[1] != 'distronode_collections':
        return os.path.join(collection_path, 'distronode_collections')
        ...
        for collection in collections:
            try:
                ...
                for search_path in search_paths:
                    ...
                    if not _is_installed_collection_dir(b_search_path):
                        default_err = (
                            ...
                            "and installed via distronode-galaxy" % collection.fqcn
                        )
                        continue
                    ...
                    raise DistronodeError(message=default_err)
                ...
                if local_verify_only:
                    ...
                    except DistronodeError as e:  # FIXME: does this actually emit any errors?
...
def _build_files_manifest(b_collection_path, namespace, name, ignore_patterns,
    ...
    if ignore_patterns and manifest_control is not Sentinel:
        raise DistronodeError('"build_ignore" and "manifest" are mutually exclusive')
...
def _build_files_manifest_distlib(b_collection_path, namespace, name, manifest_control,
    ...
    if not HAS_DISTLIB:
        raise DistronodeError('Use of "manifest" requires the python "distlib" library')
    ...
    try:
        ...
        raise DistronodeError(f'Invalid "manifest" provided: {ex}')

    if not is_sequence(control.directives):
        raise DistronodeError(f'"manifest.directives" must be a list, got: {control.directives.__class__.__name__}')
    ...
    for directive in directives:
        try:
            ...
            raise DistronodeError(f'Invalid manifest directive: {e}')
        except Exception as e:
            raise DistronodeError(f'Unknown error processing manifest directive: {e}')
...
def _build_files_manifest_walk(b_collection_path, namespace, name, ignore_patterns):
    ...
    b_ignore_patterns = [
        ...
        b'*.pyc',
        b'*.retry',
        b'tests/output',  # Ignore distronode-test result output directory.
            ...
            for file_info in file_manifest['files']:  # type: ignore[union-attr]
                ...
                if os.path.islink(b_src_path):
                    ...
                    if not os.path.exists(b_link_target):
                        raise DistronodeError(f"Failed to find the target path '{to_native(b_link_target)}' for the symlink '{to_native(b_src_path)}'.")
...
def find_existing_collections(path_filter, artifacts_manager, namespace_filter=None, collection_filter=None, dedupe=True):
    ...
    """
    if files is None:
        raise DistronodeError('importlib_resources is not installed and is required')
    ...
    paths = set()
    for path in files('distronode_collections').glob('*/*/'):
    ...
    for path in paths:
        ...
        if dedupe:
            try:
                collection_path = files(f'distronode_collections.{namespace}.{name}')
            except ImportError:
                continue
...
def verify_artifact_manifest(manifest_file, signatures, keyring, required_signature_count, ignore_signature_errors):
    ...
    if not verify_file_signatures(collection_name, manifest_file, signatures, keyring, required_signature_count, ignore_signature_errors):
        raise DistronodeError(f"Not installing {collection_name} because GnuPG signature verification failed.")
...
def install_artifact(b_coll_targz_path, b_collection_path, b_temp_path, signatures, keyring, required_signature_count, ignore_signature_errors):
    ...
    try:
        with tarfile.open(b_coll_targz_path, mode='r') as collection_tar:
            ...
            collection_tar._distronode_normalized_cache = {
...
def install_src(collection, b_collection_path, b_collection_output_path, artifacts_manager):
    ...
    Generates the Distronode collection artifact data from a galaxy.yml and
    ...
    :raises DistronodeError: If no collection metadata found.
    """
...
def _extract_tar_dir(tar, dirname, b_dest):
    ...
    try:
        tar_member = tar._distronode_normalized_cache[dirname]
    except KeyError:
        raise DistronodeError("Unable to extract '%s' from collection" % dirname)
    ...
    if tar_member.type == tarfile.SYMTYPE:
        ...
        if not _is_child_path(b_link_path, b_dest, link_name=b_dir_path):
            raise DistronodeError("Cannot extract symlink '%s' in collection: path points to location outside of "
...
def _extract_tar_file(tar, filename, b_dest, b_temp_path, expected_hash=None):
    ...
    with _get_tar_file_member(tar, filename) as (tar_member, tar_obj):
        ...
        if expected_hash and actual_hash != expected_hash:
            raise DistronodeError("Checksum mismatch for '%s' inside collection at '%s'"
                               % (to_native(filename, errors='surrogate_or_strict'), to_native(tar.name)))

        b_dest_filepath = os.path.abspath(os.path.join(b_dest, to_bytes(filename, errors='surrogate_or_strict')))
        b_parent_dir = os.path.dirname(b_dest_filepath)
        if not _is_child_path(b_parent_dir, b_dest):
            raise DistronodeError("Cannot extract tar entry '%s' as it will be placed outside the collection directory"
...
def _get_tar_file_member(tar, filename):
    ...
    try:
        ...
        raise DistronodeError("Collection tar at '%s' does not contain the expected file '%s'." % (
            to_native(tar.name),
            n_filename))
...
def _resolve_depenency_map(
    ...
    if not HAS_RESOLVELIB:
        raise DistronodeError("Failed to import resolvelib, check that a supported version is installed")
    if not HAS_PACKAGING:
        raise DistronodeError("Failed to import packaging, check that a supported version is installed")

    req = None

    try:
        dist = distribution('distronode-core')
    except Exception:
        pass
    else:
        ...
        if req is None:
            ...
            # display.warning("Unable to find 'distronode-core' distribution requirements to verify the resolvelib version is supported.")
            if not RESOLVELIB_LOWERBOUND <= RESOLVELIB_VERSION < RESOLVELIB_UPPERBOUND:
                raise DistronodeError(
                    f"distronode-galaxy requires resolvelib<{RESOLVELIB_UPPERBOUND.vstring},>={RESOLVELIB_LOWERBOUND.vstring}"
                )
        elif not req.specifier.contains(RESOLVELIB_VERSION.vstring):
            raise DistronodeError(f"distronode-galaxy requires {req.name}{req.specifier}")
    ...
    try:
        ...
        raise DistronodeError(to_native(exc)) from exc
