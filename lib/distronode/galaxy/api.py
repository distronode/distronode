# (C) 2013, James Cammarata <jcammarata@distronode.com>
...
from distronode.module_utils.api import retry_with_delays_and_condition
from distronode.module_utils.api import generate_jittered_backoff
...
from distronode.module_utils.urls import open_url, prepare_multipart
from distronode.utils.display import Display
from distronode.utils.hashing import secure_hash_s
from distronode.utils.path import makedirs_safe
...
def should_retry_error(exception):
    ...
    if isinstance(exception, DistronodeError) and (orig_exc := getattr(exception, 'orig_exc', None)):
    ...
    def decorator(method):
        def wrapped(self, *args, **kwargs):
            if not self._available_api_versions:
                ...
                if self.api_server == 'https://galaxy.distronode.com' or self.api_server == 'https://galaxy.distronode.com/':
                    n_url = 'https://galaxy.distronode.com/api/'

                try:
                    data = self._call_galaxy(n_url, method='GET', error_context_msg=error_context_msg, cache=True)
                except (DistronodeError, GalaxyError, ValueError, KeyError) as err:
                ...
                if 'available_versions' not in data:
                    raise DistronodeError("Tried to find galaxy API root at %s but no 'available_versions' are available "
            ...
            if not common_versions:
                raise DistronodeError("Galaxy action %s requires API versions '%s' but only '%s' are available on %s %s"
...
class GalaxyError(DistronodeError):
...
class GalaxyAPI:
    """ This class is meant to be used as a API client for an Distronode Galaxy server """
    ...
    def _call_galaxy(self, url, args=None, headers=None, method=None, auth_required=False, error_context_msg=None,
        ...
        try:
            ...
            raise DistronodeError("Failed to parse Galaxy response from '%s' as JSON:\n%s"
    ...
    def _add_auth_token(self, headers, url, token_type=None, required=False):
        ...
        if not self.token and required:
            raise DistronodeError("No access token or username set. A token can be set with --api-key "
    ...
    def authenticate(self, github_token):
        ...
        try:
            ...
            raise DistronodeError('Unable to authenticate to galaxy: %s' % to_native(e), orig_exc=e)
    ...
    def create_import_task(self, github_user, github_repo, reference=None, role_name=None):
        ...
        elif github_repo.startswith('distronode-role'):
            args['alternate_role_name'] = github_repo[len('distronode-role') + 1:]
    ...
    def get_import_task(self, task_id=None, github_user=None, github_repo=None):
        ...
        if task_id is not None:
            ...
            raise DistronodeError("Expected task_id or github_user and github_repo")
    ...
    def lookup_role_by_name(self, role_name, notify=True):
        ...
        try:
            ...
            raise DistronodeError("Invalid role name (%s). Specify role as format: username.rolename" % role_name)
    ...
    def fetch_role_related(self, related, role_id):
        ...
        try:
            ...
            # https://github.com/distronode/distronode/issues/64355
    ...
    def get_list(self, what):
        ...
        try:
            ...
            raise DistronodeError("Failed to download the %s list: %s" % (what, to_native(error)))
    ...
    def publish_collection(self, collection_path):
        ...
        if not os.path.exists(b_collection_path):
            raise DistronodeError("The collection path specified '%s' does not exist." % to_native(collection_path))
        elif not tarfile.is_tarfile(b_collection_path):
            raise DistronodeError("The collection path specified '%s' is not a tarball, use 'distronode-galaxy collection "
    ...
    def wait_import_task(self, task_id, timeout=0):
        ...
        if state == 'waiting':
            raise DistronodeError("Timeout while waiting for the Galaxy import process to finish, check progress at '%s'"
                               % to_native(full_url))
        ...
        if state == 'failed':
            ...
            raise DistronodeError("Galaxy import process failed: %s (Code: %s)" % (description, code))
    ...
    def get_collection_versions(self, namespace, name):
        ...
        if 'data' in data:
            ...
            # since v3 pulp_distronode does not, we cannot rely on version
