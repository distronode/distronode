...
from distronode.errors import DistronodeOptionsError, DistronodeError
from distronode.module_utils.common.text.converters import to_text, to_bytes, to_native
from distronode.module_utils.common.yaml import yaml_load
...
from distronode.utils.path import cleanup_tmp_file, makedirs_safe, unfrackpath
    ...
    if value is not None:
        if value_type in ('boolean', 'bool'):
            ...
            if isinstance(value, string_types):
                ...
                prefix = 'distronode-local-%s' % os.getpid()
            ...
            if isinstance(value, (string_types, DistronodeVaultEncryptedUnicode, bool, int, float, complex)):
        ...
        elif isinstance(value, (string_types, DistronodeVaultEncryptedUnicode)):
            value = to_text(value, errors='surrogate_or_strict')
            if origin == 'ini':
                value = unquote(value)
...
def get_config_type(cfile):
    ...
    if cfile is not None:
        ...
        if ext in ('.ini', '.cfg'):
            ...
            raise DistronodeOptionsError("Unsupported configuration file extension for %s: %s" % (cfile, to_native(ext)))
...
def find_ini_config_file(warnings=None):
    ''' Load INI Config File order(first found is used): ENV, CWD, HOME, /etc/distronode '''
    ...
    # A value that can never be a valid path so that we can tell if DISTRONODE_CONFIG was set later
    ...
    # Environment setting
    path_from_env = os.getenv("DISTRONODE_CONFIG", SENTINEL)
    if path_from_env is not SENTINEL:
        path_from_env = unfrackpath(path_from_env, follow=False)
        if os.path.isdir(to_bytes(path_from_env)):
            path_from_env = os.path.join(path_from_env, "distronode.cfg")
    ...
    try:
        cwd = os.getcwd()
        perms = os.stat(cwd)
        cwd_cfg = os.path.join(cwd, "distronode.cfg")
    ...
    # Per user location
    potential_paths.append(unfrackpath("~/.distronode.cfg", follow=False))

    # System location
    potential_paths.append("/etc/distronode/distronode.cfg")
    ...
    # * We did not use a config from DISTRONODE_CONFIG
    # * There's an distronode.cfg in the current working directory that we skipped
    if path_from_env != path and warn_cmd_public:
        warnings.add(u"Distronode is being run in a world writable directory (%s),"
                     ...
                     u" https://docs.distronode.com/distronode/devel/reference_appendices/config.html#cfg-in-world-writable-dir"
                     % to_text(cwd))
...
def _add_base_defs_deprecations(base_defs):
    '''Add deprecation source 'distronode.builtin' to deprecations in base.yml'''
    def process(entry):
        if 'deprecated' in entry:
            entry['deprecated']['collection_name'] = 'distronode.builtin'
...
class ConfigManager(object):
    ...
    def _read_config_yaml_file(self, yml_file):
        ...
        # Currently this is only used with absolute paths to the `distronode/config` directory
            ...
            if ftype == 'ini':
                ...
                with open(to_bytes(cfile), 'rb') as f:
                    try:
                        ...
                        raise DistronodeOptionsError("Error reading config file(%s) because the config file was not utf8 encoded: %s" % (cfile, to_native(e)))
                try:
                    self._parsers[cfile].read_string(cfg_text)
                except configparser.Error as e:
                    raise DistronodeOptionsError("Error reading config file (%s): %s" % (cfile, to_native(e)))
            # FIXME: this should eventually handle yaml config files
            # elif ftype == 'yaml':
            #     with open(cfile, 'rb') as config_stream:
            #         self._parsers[cfile] = yaml_load(config_stream)
            else:
                raise DistronodeOptionsError("Unsupported configuration file type: %s" % to_native(ftype))
    ...
    def _loop_entries(self, container, entry_list):
        ...
        for entry in entry_list:
            ...
            if temp_value is not None:  # only set if entry is defined in container
                ...
                if isinstance(temp_value, DistronodeVaultEncryptedUnicode):
    ...
    def get_config_value(self, config, cfile=None, plugin_type=None, plugin_name=None, keys=None, variables=None, direct=None):
        ...
        try:
            ...
            raise DistronodeError("Unhandled exception when retrieving %s:\n%s" % (config, to_native(e)), orig_exc=e)
        return value
        ...
        if config in defs:
            ...
            if value is None:
                if defs[config].get('required', False):
                    if not plugin_type or config not in INTERNAL_DEFS.get(plugin_type, {}):
                        raise DistronodeError("No setting was provided for required configuration %s" %
            ...
            try:
                ...
                if origin.startswith('env:') and value == '':
                    ...
                    raise DistronodeOptionsError('Invalid type for configuration option %s (from %s): %s' %
            ...
            if value is not None and 'choices' in defs[config] and defs[config]['choices'] is not None:
                ...
                if invalid_choices:
                    ...
                    raise DistronodeOptionsError('Invalid value "%s" for configuration option "%s", valid values are: %s' %
            ...
            raise DistronodeError('Requested entry (%s) was not defined in configuration.' % to_native(_get_entry(plugin_type, plugin_name, config)))
