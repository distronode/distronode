#!/usr/bin/env python
# Copyright: (c) 2017, Brian Coca <bcoca@distronode.com>
...
from distronode.vars.plugins import get_vars_from_inventory_sources, get_vars_from_path

display = Display()

INTERNAL_VARS = frozenset(['distronode_diff_mode',
...
class InventoryCLI(CLI):
    ''' used to display or dump the configured inventory as Distronode sees it '''

    name = 'distronode-inventory'
    ...
    def init_parser(self):
        super(InventoryCLI, self).init_parser(
            ...
            desc='Show Distronode inventory information, by default it uses the inventory script JSON format')
        ...
        self.parser.add_argument("--export", action="store_true", default=C.INVENTORY_EXPORT, dest='export',
                                 help="When doing an --list, represent in a way that is optimized for export,"
                                      "not as an accurate representation of how Distronode has processed it")
    ...
    def post_process_args(self, options):
        ...
        for opt in (options.list, options.host, options.graph):
            if opt:
                used += 1
        if used == 0:
            raise DistronodeOptionsError("No action selected, at least one of --host, --graph or --list needs to be specified.")
        elif used > 1:
            raise DistronodeOptionsError("Conflicting options used, only one of --host, --graph or --list can be used at the same time.")
    ...
    def run(self):
        ...
        if context.CLIARGS['host']:
            ...
            if len(hosts) != 1:
                raise DistronodeOptionsError("You must pass a single valid host to --host parameter")
        ...
        if results:
            ...
            if outfile is None:
                ...
                try:
                    ...
                    raise DistronodeError('Unable to write to destination file (%s): %s' % (to_native(outfile), to_native(e)))
            sys.exit(0)
    ...
    def dump(stuff):
        ...
        if context.CLIARGS['yaml']:
            ...
            results = to_text(yaml.dump(stuff, Dumper=DistronodeDumper, default_flow_style=False, allow_unicode=True))
        elif context.CLIARGS['toml']:
            from distronode.plugins.inventory.toml import toml_dumps
            try:
            ...
            import json
            from distronode.parsing.ajson import DistronodeJSONEncoder
            try:
                results = json.dumps(stuff, cls=DistronodeJSONEncoder, sort_keys=True, indent=4, preprocess_unsafe=True, ensure_ascii=False)
            except TypeError as e:
                results = json.dumps(stuff, cls=DistronodeJSONEncoder, sort_keys=False, indent=4, preprocess_unsafe=True, ensure_ascii=False)
    ...
    def _get_group_variables(self, group):
        ...
        if group.priority != 1:
            res['distronode_group_priority'] = group.priority
    ...
    def inventory_graph(self):
        ...
        if start_at:
            ...
            raise DistronodeOptionsError("Pattern must be valid group name when using --graph")
