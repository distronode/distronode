...
from distronode.plugins.loader import add_all_plugin_dirs
...
class PlaybookCLI(CLI):
    ...
    def init_parser(self):
        ...
        super(PlaybookCLI, self).init_parser(
            ...
            desc="Runs Distronode playbooks, executing the defined tasks on the targeted hosts.")
        ...
        # distronode playbook specific opts
    ...
    def run(self):
        ...
        for playbook in context.CLIARGS['args']:
            ...
            if resource is not None:
                ...
                if not os.path.exists(playbook):
                    raise DistronodeError("the playbook: %s could not be found" % playbook)
                if not (os.path.isfile(playbook) or stat.S_ISFIFO(os.stat(playbook).st_mode)):
                    raise DistronodeError("the playbook: %s does not appear to be a file" % playbook)
        ...
        if b_playbook_dirs:
            ...
            DistronodeCollectionConfig.playbook_paths = b_playbook_dirs
