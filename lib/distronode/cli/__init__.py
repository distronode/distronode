...
if sys.version_info < (3, 10):
    raise SystemExit(
        'ERROR: Distronode requires Python 3.10 or newer on the controller. '
...
def check_blocking_io():
    ...
    if handles:
        raise SystemExit('ERROR: Distronode requires blocking IO on stdin/stdout/stderr. '
...
def initialize_locale():
    ...
    try:
        ...
        raise SystemExit(
            'ERROR: Distronode could not initialize the preferred locale: %s' % e
        )

    if not encoding or encoding.lower() not in ('utf-8', 'utf8'):
        raise SystemExit('ERROR: Distronode requires the locale encoding to be UTF-8; Detected %s.' % encoding)

    fs_enc = sys.getfilesystemencoding()
    if fs_enc.lower() != 'utf-8':
        raise SystemExit('ERROR: Distronode requires the filesystem encoding to be UTF-8; Detected %s.' % fs_enc)
...
if jinja2_version < LooseVersion('3.0'):
    raise SystemExit(
        'ERROR: Distronode requires Jinja2 3.0 or newer on the controller. '
...
from distronode.errors import DistronodeError, DistronodeOptionsError, DistronodeParserError
from distronode.inventory.manager import InventoryManager
from distronode.module_utils.six import string_types
from distronode.module_utils.common.text.converters import to_bytes, to_text
from distronode.module_utils.common.collections import is_sequence
from distronode.module_utils.common.file import is_executable
from distronode.parsing.dataloader import DataLoader
from distronode.parsing.vault import PromptVaultSecret, get_file_vault_secret
from distronode.plugins.loader import add_all_plugin_dirs, init_plugin_loader
from distronode.release import __version__
from distronode.utils.collection_loader import DistronodeCollectionConfig
from distronode.utils.collection_loader._collection_finder import _get_collection_name_from_path
from distronode.utils.path import unfrackpath
from distronode.utils.unsafe_proxy import to_unsafe_text
...
class CLI(ABC):
    ''' code behind bin/distronode* programs '''
    ...
    def __init__(self, args, callback=None):
        ...
        if C.DEVEL_WARNING and __version__.endswith('dev0'):
            display.warning(
                'You are running the development version of Distronode. You should only run Distronode from "devel" if '
                'you are modifying the Distronode engine, or trying out features under development. This is a rapidly '
                'changing source of code and can become unstable at any point.'
            )

    @abstractmethod
    def run(self):
        """Run the distronode command

        Subclasses must implement this method.  It does the actual work of
        running an Distronode command.
        """
        self.parse()
    ...
    def init_parser(self, usage="", desc=None, epilog=None):
        """
        Create an options parser for most distronode scripts
        ...
        An implementation will look something like this::
            ...
            def init_parser(self):
                super(MyCLI, self).init_parser(usage="My Distronode CLI", inventory_opts=True)
                distronode.arguments.option_helpers.add_runas_options(self.parser)
        ...
        An implementation will look something like this::
            ...
            def post_process_args(self, options):
                ...
                if options.addition and options.subtraction:
                    raise DistronodeOptionsError('Only one of --addition and --subtraction can be specified')
    ...
    def version_info(gitinfo=False):
        ''' return full distronode version info '''
        if gitinfo:
            # expensive call, user with care
            distronode_version_string = opt_help.version()
        else:
            distronode_version_string = __version__
        distronode_version = distronode_version_string.split()[0]
        distronode_versions = distronode_version.split('.')
        for counter in range(len(distronode_versions)):
            if distronode_versions[counter] == "":
                distronode_versions[counter] = 0
            try:
                distronode_versions[counter] = int(distronode_versions[counter])
            except Exception:
                pass
        if len(distronode_versions) < 3:
            for counter in range(len(distronode_versions), 3):
                distronode_versions.append(0)
        return {'string': distronode_version_string.strip(),
                'full': distronode_version,
                'major': distronode_versions[0],
                'minor': distronode_versions[1],
                'revision': distronode_versions[2]}
    ...
    @staticmethod
    def _play_prereqs():
        # TODO: evaluate moving all of the code that touches ``DistronodeCollectionConfig``
        # into ``init_plugin_loader`` so that we can specifically remove
        # ``DistronodeCollectionConfig.playbook_paths`` to make it immutable after instantiation
        ...
        if basedir:
            ...
            DistronodeCollectionConfig.playbook_paths = basedir
            ...
            if default_collection:
                ...
                DistronodeCollectionConfig.default_collection = default_collection
    ...
    def get_host_list(inventory, subset, pattern='all'):
        ...
        if not hosts and no_hosts is False:
            raise DistronodeError("Specified inventory, host pattern and/or --limit leaves us with no hosts to target.")
    ...
    def get_password_from_file(pwd_file):
        ...
        if b_pwd_file == b'-':
            ...
            raise DistronodeError("The password file %s was not found" % pwd_file)
            ...
            try:
                ...
                raise DistronodeError("Problem occured when trying to run the password script %s (%s)."
                                   " If this is not a script, remove the executable bit from the file." % (pwd_file, e))

            stdout, stderr = p.communicate()
            if p.returncode != 0:
                raise DistronodeError("The password script %s returned an error (rc=%s): %s" % (pwd_file, p.returncode, stderr))
            secret = stdout
            ...
            try:
                ...
                raise DistronodeError("Could not read password file %s: %s" % (pwd_file, e))

        secret = secret.strip(b'\r\n')

        if not secret:
            raise DistronodeError('Empty password was provided from file (%s)' % pwd_file)
    ...
    def cli_executor(cls, args=None):
        ...
        try:
            ...
            distronode_dir = Path(C.DISTRONODE_HOME).expanduser()
            try:
                distronode_dir.mkdir(mode=0o700)
            except OSError as exc:
                if exc.errno != errno.EEXIST:
                    display.warning(
                        "Failed to create the directory '%s': %s" % (distronode_dir, to_text(exc, errors='surrogate_or_replace'))
                    )
            else:
                display.debug("Created the '%s' directory" % distronode_dir)

            try:
                args = [to_text(a, errors='surrogate_or_strict') for a in args]
            except UnicodeError:
                display.error('Command line args are not in utf-8, unable to continue.  Distronode currently only understands utf-8')
    ...
    #    except DistronodeHostUnreachable as e:
    #        display.error(str(e))
    #        exit_code = 3
    #    except DistronodeHostFailed as e:
    #        display.error(str(e))
    #        exit_code = 2
