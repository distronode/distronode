#!/usr/bin/env python
# Copyright: (c) 2013, James Cammarata <jcammarata@distronode.com>
...
from distronode.errors import DistronodeError, DistronodeOptionsError
from distronode.galaxy import Galaxy, get_collections_galaxy_meta_info
from distronode.galaxy.api import GalaxyAPI, GalaxyError
from distronode.galaxy.collection import (
    build_collection,
...
from distronode.galaxy.collection.gpg import GPG_ERROR_MAP
from distronode.galaxy.dependency_resolution.dataclasses import Requirement

from distronode.galaxy.role import GalaxyRole
from distronode.galaxy.token import BasicAuthToken, GalaxyToken, KeycloakToken, NoTokenSentinel
...
from distronode.module_utils.common.yaml import yaml_dump, yaml_load
...
from distronode.playbook.role.requirement import RoleRequirement
...
class GalaxyCLI(CLI):
    '''Command to manage Distronode roles and collections.
        ...
        if len(args) > 1:
            ...
            if args[1:3] == ['role', 'login']:
                display.error(
                    ...
                    "to Galaxy. The key can be found at https://galaxy.distronode.com/me/preferences, and passed to the "
    ...
    def init_parser(self):
        ''' create an options parser for bin/distronode '''
        ...
        common.add_argument('--token', '--api-key', dest='api_key',
                            help='The Distronode Galaxy API key which can be found at '
                                 'https://galaxy.distronode.com/me/preferences.')
        ...
        collection = type_parser.add_parser('collection', help='Manage an Distronode Galaxy collection.')
        ...
        role = type_parser.add_parser('role', help='Manage an Distronode Galaxy role.')
    ...
    def add_verify_options(self, parser, parents=None):
        ...
        verify_parser.add_argument('--keyring', dest='keyring', default=C.GALAXY_GPG_KEYRING,
                                   help='The keyring used during signature verification')  # Eventually default to ~/.distronode/pubring.kbx?
    ...
    def add_install_options(self, parser, parents=None):
        ...
        install_parser = parser.add_parser('install', parents=parents,
                                           help='Install {0}(s) from file(s), URL(s) or Distronode '
    ...
    def add_build_options(self, parser, parents=None):
        build_parser = parser.add_parser('build', parents=parents,
                                         help='Build an Distronode collection artifact that can be published to Distronode '
                                              'Galaxy.')
    ...
    def add_publish_options(self, parser, parents=None):
        publish_parser = parser.add_parser('publish', parents=parents,
                                           help='Publish a collection artifact to Distronode Galaxy.')
    ...
    def run(self):
        ...
        def server_config_def(section, key, required, option_type):
            config_def = {
                ...
                'env': [
                    {'name': 'DISTRONODE_GALAXY_SERVER_%s_%s' % (section.upper(), key.upper())},
                ],
                'required': required,
                'type': option_type,
            }
        ...
        for server_priority, server_key in enumerate(server_list, start=1):
            ...
            defs = DistronodeLoader(yaml_dump(config_dict)).get_single_data()
            ...
            # against pulp_distronode and I'm not sure it has a practical purpose
    ...
    def _parse_requirements_file(self, requirements_file, allow_old_format=True, artifacts_manager=None, validate_signature_options=True):
        """
        Parses an Distronode requirement.yml file and returns all the roles and/or collections defined in it. There are 2
        ...
        if not os.path.exists(b_requirements_file):
            raise DistronodeError("The requirements file '%s' does not exist." % to_native(requirements_file))
        ...
        if file_requirements is None:
            raise DistronodeError("No requirements found in file '%s'" % to_native(requirements_file))
        ...
        def parse_role_req(requirement):
            if "include" not in requirement:
                ...
                if "name" not in role and "src" not in role:
                    raise DistronodeError("Must specify name or src for role")
                return [GalaxyRole(self.galaxy, self.lazy_role_api, **role)]
            else:
                b_include_path = to_bytes(requirement["include"], errors="surrogate_or_strict")
                if not os.path.isfile(b_include_path):
                    raise DistronodeError("Failed to find include requirements file '%s' in '%s'"
                ...
                with open(b_include_path, 'rb') as f_include:
                    try:
                        ...
                        raise DistronodeError("Unable to load data from include requirements file: %s %s"
                                           % (to_native(requirements_file), to_native(e)))

        if isinstance(file_requirements, list):
            # Older format that contains only roles
            if not allow_old_format:
                raise DistronodeError("Expecting requirements file to be a dict with the key 'collections' that contains "
            ...
            if extra_keys:
                raise DistronodeError("Expecting only 'roles' and/or 'collections' as base keys in the requirements "
            ...
            raise DistronodeError(f"Expecting requirements yaml to be a list or dictionary but got {type(file_requirements).__name__}")
    ...
    def exit_without_ignore(rc=1):
        ...
        if not context.CLIARGS['ignore_errors']:
            raise DistronodeError('- you can use --ignore-errors to skip failed roles and finish processing the list.')
    ...
    def _require_one_of_collections_requirements(
        ...
        if collections and requirements_file:
            raise DistronodeError("The positional collection_name arg and --requirements-file are mutually exclusive.")
        elif not collections and not requirements_file:
            raise DistronodeError("You must specify a collection name or a requirements file.")
    ...
    def execute_role(self):
        """
        Perform the action on an Distronode Galaxy role. Must be combined with a further action like delete/install/init
        as listed below.
        """
    ...
    def execute_collection(self):
        """
        Perform the action on an Distronode Galaxy collection. Must be combined with a further action like init/install as
        listed below.
        """
        # To satisfy doc build
        pass
    ...
    def execute_build(self):
        """
        Build an Distronode Galaxy collection artifact that can be stored in a central repository like Distronode Galaxy.
        ...
        if not os.path.exists(b_output_path):
            ...
            raise DistronodeError("- the output collection directory %s is a file - aborting" % to_native(output_path))
    ...
    def execute_init(self):
        ...
        inject_data = dict(
            ...
            distronode_plugin_list_dir=get_versioned_doclink('plugins/plugins.html'),
        )
        if galaxy_type == 'role':
            inject_data.update(dict(
                ...
                min_distronode_version=distronode_version[:3],  # x.y
                dependencies=[],
            ))
        ...
        if os.path.exists(b_obj_path):
            if os.path.isfile(obj_path):
                raise DistronodeError("- the path %s already exists, but is a file - aborting" % to_native(obj_path))
            elif not force:
                raise DistronodeError("- the directory %s already exists. "
        ...
        if not os.path.exists(obj_skeleton):
            raise DistronodeError("- the skeleton path '{0}' does not exist, cannot init {1}".format(
            ...
            if not context.CLIARGS['offline']:
                ...
                try:
                    ...
                    if e.http_code == 400 and 'Bad Request' in e.message:
                        # Role does not exist in Distronode Galaxy
                        data = u"- the role %s was not found" % role
                        break

                    raise DistronodeError("Unable to find info about '%s': %s" % (role, e))
    ...
    def execute_verify(self, artifacts_manager=None):
        ...
        collections = context.CLIARGS['args']
        search_paths = DistronodeCollectionConfig.collection_paths
    ...
    def execute_install(self, artifacts_manager=None):
        """
        Install one or more roles(``distronode-galaxy role install``), or one or more collections(``distronode-galaxy collection install``).
        ...
        two_type_warning = "The requirements file '%s' contains {0}s which will be ignored. To install these {0}s " \
                           "run 'distronode-galaxy {0} install -r' or to install both at the same time run " \
                           "'distronode-galaxy install -r' without a custom install path." % to_text(requirements_file)
        ...
        if context.CLIARGS['type'] == 'collection':
            ...
            if not install_items and requirements_file is None:
                raise DistronodeOptionsError("- you must specify a user/role name or a roles file")

            if requirements_file:
                if not (requirements_file.endswith('.yaml') or requirements_file.endswith('.yml')):
                    raise DistronodeError("Invalid role requirements file, it must end with a .yml or .yaml extension")
                ...
                if requirements['collections'] and (not self._implicit_role or '-p' in galaxy_args or
                    ...
                    # We only want to display a warning if 'distronode-galaxy install -r ... -p ...'. Other cases the user
        ...
        if collection_requirements:
            ...
            # Collections can technically be installed even when distronode-galaxy is in role mode so we need to pass in
    ...
    def _execute_install_collection(
        ...
        try:
            ...
            if self._implicit_role:
                raise DistronodeError(
                    'Unable to properly parse command line arguments. Please use "distronode-galaxy collection install" '
                    'instead of "distronode-galaxy install".'
                )
            raise

        # If `distronode-galaxy install` is used, collection-only options aren't available to the user and won't be in context.CLIARGS
        ...
        read_req_paths = set(validate_collection_path(p) for p in DistronodeCollectionConfig.collection_paths)
        ...
        if unexpected_path and any(p.startswith(path) for p in read_req_paths):
            display.warning(
                f"The specified collections path '{path}' appears to be part of the pip Distronode package. "
                "Managing these directly with distronode-galaxy could break the Distronode package. "
            ...
            display.warning("The specified collections path '%s' is not part of the configured Distronode "
                            "collections paths '%s'. The installed collection will not be picked up in an Distronode "
                ...
                if not role.metadata:
                    ...
                    for dep in role_dependencies:
                        ...
                        if '.' not in dep_role.name and '.' not in dep_role.src and dep_role.scm is None:
                            ...
                            # be found on galaxy.distronode.com
                            continue
    ...
    def execute_remove(self):
        ...
        if not context.CLIARGS['args']:
            raise DistronodeOptionsError('- you must specify at least one role to remove.')
        ...
        for role_name in context.CLIARGS['args']:
            ...
            try:
                ...
                raise DistronodeError("Failed to remove role %s: %s" % (role_name, to_native(e)))
    ...
    def execute_list_collection(self, artifacts_manager=None):
        ...
        collections_search_paths = (
            set(context.CLIARGS['collections_path'] or []) | default_collections_path | set(DistronodeCollectionConfig.collection_paths)
        )
    ...
    def execute_publish(self):
        """
        Publish a collection into Distronode Galaxy. Requires the path to the collection tarball to publish.
        """
    ...
    def execute_search(self):
        ''' searches for roles on the Distronode Galaxy server'''
        page_size = 1000
        search = None
        ...
        if not search and not context.CLIARGS['platforms'] and not context.CLIARGS['galaxy_tags'] and not context.CLIARGS['author']:
            raise DistronodeError("Invalid query. At least one search term, platform, galaxy tag or author must be provided.")
    ...
    def execute_import(self):
        """ used to import a role into Distronode Galaxy """
    ...
    def execute_setup(self):
        """ Setup an integration from Github or Travis for Distronode Galaxy roles"""
    ...
    def execute_delete(self):
        """ Delete a role from Distronode Galaxy. """
