...
from distronode.collections.list import list_collection_dirs
from distronode.errors import DistronodeError, DistronodeOptionsError, DistronodeParserError, DistronodePluginNotFound
...
from distronode.plugins.list import list_plugins
from distronode.plugins.loader import action_loader, fragment_loader
...
from distronode.utils.plugin_docs import get_plugin_docs, get_docstring, get_versioned_doclink
...
def jdump(text):
    try:
        ...
        raise DistronodeError('We could not convert all the documentation into JSON as there was a conversion issue: %s' % to_native(e))
...
class RoleMixin(object):
    ...
    def _load_argspec(self, role_name, collection_path=None, role_path=None):
        ...
        if collection_path:
            ...
            raise DistronodeError("A path is required to load argument specs for role '%s'" % role_name)
        ...
        try:
            ...
            raise DistronodeParserError("An error occurred while trying to read the file '%s': %s" % (path, to_native(e)), orig_exc=e)
...
class DocCLI(CLI, RoleMixin):
    ''' displays information on modules installed in Distronode libraries.
    ...
    # distronode-community/antsibull repo)
    ...
    def display_plugin_list(self, results):
        ...
        if context.CLIARGS['list_files']:
            ...
            for plugin in sorted(results.keys()):
                ...
                pbreak = plugin.split('.')
                if pbreak[-1].startswith('_') and pbreak[0] == 'distronode' and pbreak[1] in ('builtin', 'legacy'):
                    pbreak[-1] = pbreak[-1][1:]
                    plugin = '.'.join(pbreak)
            ...
            for plugin in sorted(results.keys()):
                ...
                if pbreak[-1].startswith('_') and plugin.startswith(('distronode.builtin.', 'distronode.legacy.')):
                    # Handle deprecated distronode.builtin plugins
                    pbreak[-1] = pbreak[-1][1:]
                    plugin = '.'.join(pbreak)
    ...
    @staticmethod
    def _list_keywords():
        return from_yaml(pkgutil.get_data('distronode', 'keyword_desc.yml'))
            ...
            try:
                ...
                for pobj in PB_OBJECTS:
                    if pobj not in PB_LOADED:
                        obj_class = 'distronode.playbook.%s' % pobj.lower()
    ...
    def _get_collection_filter(self):
        ...
        if len(context.CLIARGS['args']) >= 1:
            ...
            for coll_name in coll_filter:
                if not DistronodeCollectionRef.is_valid_collection_name(coll_name):
                    raise DistronodeError('Invalid collection name (must be of the form namespace.collection): {0}'.format(coll_name))
    ...
    def _get_plugins_docs(self, plugin_type, names, fail_ok=False, fail_on_errors=True):
        ...
        for plugin in names:
            ...
            except DistronodePluginNotFound as e:
    ...
    def run(self):
        ...
        if plugin_type not in TARGET_OPTIONS:
            raise DistronodeOptionsError("Unknown or undocumentable plugin type: %s" % plugin_type)
        ...
        if context.CLIARGS['dump']:
            ...
            if len(context.CLIARGS['args']) == 0:
                raise DistronodeOptionsError("Missing name(s), incorrect options passed for detailed documentation.")
            ...
            if plugin_type in C.DOCUMENTABLE_PLUGINS:
                if listing and docs:
                    ...
                    if plugin_type not in SNIPPETS:
                        raise DistronodeError('Snippets are only available for the following plugin'
    ...
    def get_plugin_metadata(plugin_type, plugin_name):
        ...
        if not result.resolved:
            raise DistronodeError("unable to load {0} plugin named {1} ".format(plugin_type, plugin_name))
        ...
        try:
            ...
            display.vvv(traceback.format_exc())
            raise DistronodeError("%s %s at %s has a documentation formatting error or is missing documentation." % (plugin_type, plugin_name, filename))
    ...
    def format_plugin_doc(plugin, plugin_type, doc, plainexamples, returndocs, metadata):
        ...
        try:
            ...
            display.vvv(traceback.format_exc())
            raise DistronodeError("Unable to retrieve documentation from '%s' due to: %s" % (plugin, to_native(e)), orig_exc=e)
    ...
    def _dump_yaml(struct, flow_style=False):
        return yaml_dump(struct, default_flow_style=flow_style, default_style="''", Dumper=DistronodeDumper).rstrip('\n')
    ...
    def _format_version_added(version_added, version_added_collection=None):
        if version_added_collection == 'distronode.builtin':
            version_added_collection = 'distronode-core'
            # In distronode-core, version_added can be 'historical'
    ...
    def add_fields(text, fields, limit, opt_indent, return_values=False, base_indent=''):
        ...
        for o in sorted(fields):
            ...
            if not isinstance(required, bool):
                raise DistronodeError("Incorrect value for 'Required', a boolean is needed.: %s" % required)
            if required:
                opt_leadin = "="
            else:
                opt_leadin = "-"
            ...
            if 'description' not in opt:
                raise DistronodeError("All (sub-)options and return values must have a 'description' field")
            if is_sequence(opt['description']):
                for entry_idx, entry in enumerate(opt['description'], 1):
                    if not isinstance(entry, string_types):
                        raise DistronodeError("Expected string in description of %s at index %s, got %s" % (o, entry_idx, type(entry)))
                    text.append(textwrap.fill(DocCLI.tty_ify(entry), limit, initial_indent=opt_indent, subsequent_indent=opt_indent))
            else:
                if not isinstance(opt['description'], string_types):
                    raise DistronodeError("Expected string in description of %s, got %s" % (o, type(opt['description'])))
        ...
        if doc.get('deprecated', False):
            ...
            if isinstance(doc['deprecated'], dict):
                if 'removed_at_date' in doc['deprecated']:
                    ...
                    text.append("\tReason: %(why)s\n\tWill be removed in: Distronode %(removed_in)s\n\tAlternatives: %(alternative)s" % doc.pop('deprecated'))
            else:
        ...
        if doc.get('seealso', False):
            ...
            for item in doc['seealso']:
                if 'module' in item:
                    ...
                    if description is None and item['module'].startswith('distronode.builtin.'):
                    ...
                    if item['module'].startswith('distronode.builtin.'):
                    ...
                    text.append(textwrap.fill(DocCLI.tty_ify('%s%s %s' % (item['plugin_type'].title(), plugin_suffix, item['plugin'])),
                                limit - 6, initial_indent=opt_indent[:-2] + "* ", subsequent_indent=opt_indent))
                    description = item.get('description')
                    if description is None and item['plugin'].startswith('distronode.builtin.'):
                        description = 'The official documentation on the %s %s%s.' % (item['plugin'], item['plugin_type'], plugin_suffix)
                    if description is not None:
                        text.append(textwrap.fill(DocCLI.tty_ify(description),
                                    limit - 6, initial_indent=opt_indent + '   ', subsequent_indent=opt_indent + '   '))
                    if item['plugin'].startswith('distronode.builtin.'):
                    ...
                    text.append(textwrap.fill(DocCLI.tty_ify('Distronode documentation [%s]' % item['ref']),
                                limit - 6, initial_indent=opt_indent[:-2] + "* ", subsequent_indent=opt_indent))
                    text.append(textwrap.fill(DocCLI.tty_ify(item['description']),
                                limit - 6, initial_indent=opt_indent + '   ', subsequent_indent=opt_indent + '   '))
        ...
        if doc.get('plainexamples', False):
            ...
            if isinstance(doc['plainexamples'], string_types):
                ...
                try:
                    ...
                    raise DistronodeParserError("Unable to parse examples section", orig_exc=e)
            text.append('')
            text.append('')
