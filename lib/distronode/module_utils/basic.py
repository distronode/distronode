...
HAVE_SELINUX = False
try:
    from distronode.module_utils.compat import selinux
    HAVE_SELINUX = True
except ImportError:
    pass
...
from distronode.module_utils.common.arg_spec import ModuleArgumentSpecValidator

from distronode.module_utils.common.text.formatters import (
    lenient_lowercase,
    bytes_to_human,
    human_to_bytes,
    SIZE_RANGES,
...
from distronode.module_utils.common.process import get_bin_path
from distronode.module_utils.common.file import (
...
from distronode.module_utils.common.sys_info import (
    get_distribution,
...
from distronode.module_utils.errors import DistronodeFallbackNotFound, DistronodeValidationErrorMultiple, UnsupportedError
from distronode.module_utils.six import (
    PY2,
    PY3,
    b,
    binary_type,
...
from distronode.module_utils.six.moves import map, reduce, shlex_quote
from distronode.module_utils.common.validation import (
    check_missing_parameters,
    safe_eval,
)
from distronode.module_utils.common._utils import get_all_subclasses as _get_all_subclasses
from distronode.module_utils.parsing.convert_bool import BOOLEANS, BOOLEANS_FALSE, BOOLEANS_TRUE, boolean
from distronode.module_utils.common.warnings import (
    deprecate,
...
# multiple DistronodeModules are created.  Otherwise each DistronodeModule would
# attempt to read from stdin.  Other code should not use this directly as it
# is an internal implementation detail
_DISTRONODE_ARGS = None
...
FILE_COMMON_ARGUMENTS = dict(
    ...
    unsafe_writes=dict(type='bool', default=False, fallback=(env_fallback, ['DISTRONODE_UNSAFE_WRITES'])),  # should be available to any module using atomic_move
)
...
def load_platform_subclass(cls, *args, **kwargs):
    """**Deprecated**: Use distronode.module_utils.common.sys_info.get_platform_subclass instead"""
    platform_cls = get_platform_subclass(cls)
    return super(cls, platform_cls).__new__(platform_cls)


def get_all_subclasses(cls):
    """**Deprecated**: Use distronode.module_utils.common._utils.get_all_subclasses instead"""
...
def _load_params():
    ...
    '''
    global _DISTRONODE_ARGS
    if _DISTRONODE_ARGS is not None:
        buffer = _DISTRONODE_ARGS
    else:
    ...
    try:
        return params['DISTRONODE_MODULE_ARGS']
    except KeyError:
...
def missing_required_lib(library, reason=None, url=None):
    ...
    msg += (" Please read the module documentation and install it in the appropriate location."
            " If the required library is installed, but Distronode is using the wrong Python interpreter,"
            " please consult the documentation on distronode_python_interpreter")
    return msg


class DistronodeModule(object):
    def __init__(self, argument_spec, bypass_checks=False, no_log=False,
        ...
        '''
        Common code for quickly building an distronode module in Python
    ...
    @property
    def tmpdir(self):
        # if _distronode_tmpdir was not set and we have a remote_tmp,
        ...
        if self._tmpdir is None:
            ...
            basefile = "distronode-moduletmp-%s-" % time.time()
            try:
    ...
    def _set_internal_properties(self, argument_spec=None, module_parameters=None):
        ...
        for k in PASS_VARS:
            # handle setting internal properties from internal distronode vars
            param_key = '_distronode_%s' % k
    ...
    def _log_to_syslog(self, msg):
        if HAS_SYSLOG:
            try:
                module = 'distronode-%s' % self._name
                ...
                self.fail_json(
                    msg='Failed to log to syslog (%s). To proceed anyway, '
                        ...
                        'to True in your Distronode config.' % to_native(e),
                    exception=traceback.format_exc(),
                    msg_to_log=msg,
                )
    ...
    def _log_invocation(self):
        ''' log that distronode ran the module '''
            ...
            if isinstance(kwargs['deprecations'], list):
                for d in kwargs['deprecations']:
                    if isinstance(d, SEQUENCETYPE) and len(d) == 2:
                        ...
                        self.deprecate(d)  # pylint: disable=distronode-deprecated-no-version
            else:
                self.deprecate(kwargs['deprecations'])  # pylint: disable=distronode-deprecated-no-version
    ...
    def atomic_move(self, src, dest, unsafe_writes=False):
        ...
        try:
            ...
            if e.errno not in [errno.EPERM, errno.EXDEV, errno.EACCES, errno.ETXTBSY, errno.EBUSY]:
                ...
                tmp_dest_name = None
                try:
                    tmp_dest_fd, tmp_dest_name = tempfile.mkstemp(prefix=b'.distronode_tmp', dir=b_dest_dir, suffix=b_suffix)
                except (OSError, IOError) as e:
    ...
    def run_command(self, args, check_rc=False, close_fds=True, executable=None, data=None, binary_data=False, path_prefix=None, cwd=None,
        ...
        #   /tmp/test_module_scratch/debug_dir/distronode/module_utils/basic.py
        ...
        if 'PYTHONPATH' in env:
            pypaths = [x for x in env['PYTHONPATH'].split(':')
                       if x and
                       not x.endswith('/distronode_modlib.zip') and
