...
from distronode.module_utils.six.moves.collections_abc import Set
    ...
    :kwarg errors: The error handler to use if the text string is not
        ...
        The last error handler is:
            ...
            :surrogate_then_replace: Will use ``surrogateescape`` if it is a valid
                ...
                not present it will simply use ``replace``.  (Added in Distronode 2.3)
                This strategy is designed to never traceback when it attempts
                to encode a string.

        The default until Distronode-2.2 was ``surrogate_or_replace``
        From Distronode-2.3 onwards, the default is ``surrogate_then_replace``.
    ...
    :kwarg errors: The error handler to use if the byte string is not
        ...
        specifically aimed at helping people to port code:
            ...
            :surrogate_then_replace: Does the same as surrogate_or_replace but
                ...
                :func:`distronode.module_utils.common.text.converters.to_bytes` (Added in Distronode 2.3)
        ...
        The default until Distronode-2.2 was `surrogate_or_replace`
        In Distronode-2.3 this defaults to `surrogate_then_replace` for symmetry
        with :func:`distronode.module_utils.common.text.converters.to_bytes` .
    :kwarg nonstring: The strategy to use if a nonstring is specified in
        ``obj``.  Default is 'simplerepr'.  Valid values are:
...
#:      :func:`~distronode.module_utils.to_bytes`.  On Python3 it is an alias for
#:      :func:`~distronode.module_utils.to_text`.  It makes it easier to
#:      transform a variable into the native str type for the python version
