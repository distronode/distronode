...
from distronode.errors import DistronodeError, DistronodeAssertionError
...
from distronode.utils.path import makedirs_safe, unfrackpath

display = Display()


b_HEADER = b'$DISTRONODE_VAULT'
CIPHER_WHITELIST = frozenset((u'AES256',))
CIPHER_WRITE_WHITELIST = frozenset((u'AES256',))
# See also CIPHER_MAPPING at the bottom of the file which maps cipher strings
# (used in VaultFile header) to a cipher class

NEED_CRYPTO_LIBRARY = "distronode-vault requires the cryptography library in order to function"


class DistronodeVaultError(DistronodeError):
    pass


class DistronodeVaultPasswordError(DistronodeVaultError):
    pass


class DistronodeVaultFormatError(DistronodeError):
    pass
...
def parse_vaulttext_envelope(b_vaulttext_envelope, default_vault_id=None, filename=None):
    ...
    :raises: DistronodeVaultFormatError: if the vaulttext_envelope format is invalid
    """
    # used by decrypt
    ...
    try:
        ...
        if filename:
            msg += ' in %s' % (filename)
        msg += ': %s' % exc
        raise DistronodeVaultFormatError(msg)
...
def format_vaulttext_envelope(b_ciphertext, cipher_name, version=None, vault_id=None):
    ...
    if not cipher_name:
        raise DistronodeError("the cipher must be set before adding a header")
...
def _unhexlify(b_data):
    try:
        ...
        raise DistronodeVaultFormatError('Vault format unhexlify error: %s' % exc)
...
def parse_vaulttext(b_vaulttext):
    ...
    :raises: DistronodeVaultFormatError: if the vaulttext format is invalid
    """
    # SPLIT SALT, DIGEST, AND DATA
    try:
        return _parse_vaulttext(b_vaulttext)
    except DistronodeVaultFormatError:
        raise
    except Exception as exc:
...
def verify_secret_is_not_empty(secret, msg=None):
    ...
    Raises: DistronodeVaultPasswordError if the password does not meet requirements.
    ...
    if not secret:
        raise DistronodeVaultPasswordError(msg)
...
class PromptVaultSecret(VaultSecret):
    ...
    def ask_vault_passwords(self):
        ...
        for prompt_format in self.prompt_formats:
            ...
            try:
                ...
                raise DistronodeVaultError('EOFError (ctrl-d) on prompt for (%s)' % self.vault_id)
    ...
    def confirm(self, b_vault_pass_1, b_vault_pass_2):
        ...
        if b_vault_pass_1 != b_vault_pass_2:
            ...
            raise DistronodeError("Passwords do not match")
...
def get_file_vault_secret(filename=None, vault_id=None, encoding=None, loader=None):
    ...
    if not os.path.exists(this_path):
        raise DistronodeError("The vault password file %s was not found" % this_path)
...
class FileVaultSecret(VaultSecret):
    ...
    def _read_file(self, filename):
        ...
        try:
            ...
            raise DistronodeError("Could not read vault password file %s: %s" % (filename, e))
...
class ScriptVaultSecret(FileVaultSecret):
    def _read_file(self, filename):
        if not self.loader.is_executable(filename):
            raise DistronodeVaultError("The vault password script %s was not executable" % filename)
    ...
    def _check_results(self, stdout, stderr, popen):
        if popen.returncode != 0:
            raise DistronodeError("Vault password script %s returned non-zero (%s): %s" %
...
class ClientScriptVaultSecret(ScriptVaultSecret):
    ...
    def _check_results(self, stdout, stderr, popen):
        if popen.returncode == self.VAULT_ID_UNKNOWN_RC:
            raise DistronodeError('Vault password client script %s did not find a secret for vault-id=%s: %s' %
                               (self.filename, self._vault_id, stderr))

        if popen.returncode != 0:
            raise DistronodeError("Vault password client script %s returned non-zero (%s) when getting secret for vault-id=%s: %s" %
...
def match_encrypt_vault_id_secret(secrets, encrypt_vault_id=None):
    ...
    if encrypt_vault_id is None:
        raise DistronodeError('match_encrypt_vault_id_secret requires a non None encrypt_vault_id')
    ...
    raise DistronodeVaultError('Did not find a match for --encrypt-vault-id=%s in the known vault-ids %s' % (encrypt_vault_id,
...
class VaultLib:
    ...
    def encrypt(self, plaintext, secret=None, vault_id=None, salt=None):
        ...
        if secret is None:
            if self.secrets:
                ...
                raise DistronodeVaultError("A vault password must be specified to encrypt data")

        b_plaintext = to_bytes(plaintext, errors='surrogate_or_strict')

        if is_encrypted(b_plaintext):
            raise DistronodeError("input is already encrypted")
        ...
        try:
            ...
            raise DistronodeError(u"{0} cipher could not be found".format(self.cipher_name))
    ...
    def decrypt_and_get_vault_id(self, vaulttext, filename=None, obj=None):
        ...
        if self.secrets is None:
            ...
            raise DistronodeVaultError(msg)
        ...
        if cipher_name in CIPHER_WHITELIST:
            ...
            raise DistronodeError("{0} cipher could not be found".format(cipher_name))

        b_plaintext = None

        if not self.secrets:
            raise DistronodeVaultError('Attempting to decrypt but no vault secrets found')
        ...
        for vault_secret_id, vault_secret in matched_secrets:
            ...
            try:
                ...
                if b_plaintext is not None:
                    ...
                    break
            except DistronodeVaultFormatError as exc:
                exc.obj = obj
...
class VaultEditor:
    ...
    def _shred_file_custom(self, tmp_path):
        ...
        See https://github.com/distronode/distronode/pull/13700 .
        """
        ...
        if file_len > 0:  # avoid work when file was empty
            ...
            with open(tmp_path, "wb") as fh:
                for dummy in range(passes):
                    ...
                    if fh.tell() != file_len:
                        raise DistronodeAssertionError()
    ...
    def _edit_file_helper(self, filename, secret, existing_data=None, force_save=False, vault_id=None):
        ...
        try:
            ...
            # if an error happens, destroy the decrypted file
            self._shred_file(tmp_path)
            raise DistronodeError('Unable to execute the command "%s": %s' % (' '.join(cmd), to_native(e)))
    ...
    def decrypt_file(self, filename, output_file=None):
        ...
        try:
            ...
            raise DistronodeError("%s for %s" % (to_native(e), to_native(filename)))
    ...
    def create_file(self, filename, secret, vault_id=None):
        ...
        if os.path.isfile(filename):
            raise DistronodeError("%s exists, please use 'edit' instead" % filename)
    ...
    def plaintext(self, filename):
        ...
        try:
            ...
            raise DistronodeVaultError("%s for %s" % (to_native(e), to_native(filename)))
    ...
    def rekey_file(self, filename, new_vault_secret, new_vault_id=None):
        ...
        if new_vault_secret is None:
            raise DistronodeError('The value for the new_password to rekey %s with is not valid' % filename)
    ...
    def read_data(self, filename):
        ...
        try:
            ...
            if not msg:
                msg = repr(e)
            raise DistronodeError('Unable to read source file (%s): %s' % (to_native(filename), msg))
    ...
    def write_data(self, data, thefile, shred=True, mode=0o600):
        ...
        if is_fd:
            ...
            if not os.access(os.path.dirname(thefile), os.W_OK):
                raise DistronodeError("Destination '%s' not writable" % (os.path.dirname(thefile)))
            ...
            try:
                try:
                    ...
                    if ose.errno == errno.EEXIST:
                        raise DistronodeError('Vault file got recreated while we were operating on it: %s' % to_native(ose))

                    raise DistronodeError('Problem creating temporary vault file: %s' % to_native(ose))
                ...
                try:
                    ...
                    raise DistronodeError('Unable to write to temporary vault file: %s' % to_native(e))
                finally:
...
class VaultAES256:
    ...
    def __init__(self):
        if not HAS_CRYPTOGRAPHY:
            raise DistronodeError(NEED_CRYPTO_LIBRARY)
    ...
    def _gen_key_initctr(cls, b_password, b_salt):
        ...
        if HAS_CRYPTOGRAPHY:
            ...
            raise DistronodeError(NEED_CRYPTO_LIBRARY + '(Detected in initctr)')
    ...
    def encrypt(cls, b_plaintext, secret, salt=None):
        ...
        if secret is None:
            raise DistronodeVaultError('The secret passed to encrypt() was None')

        if salt is None:
            b_salt = cls._get_salt()
        elif not salt:
            raise DistronodeVaultError('Empty or invalid salt passed to encrypt()')
        else:
        ...
        if HAS_CRYPTOGRAPHY:
            ...
            raise DistronodeError(NEED_CRYPTO_LIBRARY + '(Detected in encrypt)')
    ...
    def _decrypt_cryptography(cls, b_ciphertext, b_crypted_hmac, b_key1, b_key2, b_iv):
        ...
        try:
            ...
            raise DistronodeVaultError('HMAC verification failed: %s' % e)
    ...
    def decrypt(cls, b_vaulttext, secret):
        ...
        if HAS_CRYPTOGRAPHY:
            ...
            raise DistronodeError(NEED_CRYPTO_LIBRARY + '(Detected in decrypt)')
