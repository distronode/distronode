...
from distronode.errors import DistronodeParserError, DistronodeError, DistronodeAssertionError
from distronode.module_utils.six import string_types
from distronode.module_utils.common.text.converters import to_text
from distronode.parsing.splitter import parse_kv, split_args
from distronode.plugins.loader import module_loader, action_loader
...
class ModuleArgsParser:
    ...
    def __init__(self, task_ds=None, collection_list=None):
        ...
        if not isinstance(task_ds, dict):
            raise DistronodeAssertionError("the type of 'task_ds' should be a dict, but is a %s" % type(task_ds))
    ...
    def _normalize_parameters(self, thing, action=None, additional_args=None):
        ...
        if additional_args:
            if isinstance(additional_args, string_types):
                ...
                if templar.is_template(additional_args):
                    ...
                    raise DistronodeParserError("Complex args containing variables cannot use bare variables (without Jinja2 delimiters), "
                                             "and must use the full variable style ('{{var_name}}')")
            elif isinstance(additional_args, dict):
                final_args.update(additional_args)
            else:
                raise DistronodeParserError('Complex args must be a dictionary or variable string ("{{var}}").')
        ...
        if args and action not in FREEFORM_ACTIONS:
            for arg in args:
                arg = to_text(arg)
                if arg.startswith('_distronode_'):
                    raise DistronodeError("invalid parameter specified for action '%s': '%s'" % (action, arg))
    ...
    def _normalize_new_style_args(self, thing, action):
        ...
        if isinstance(thing, dict):
            ...
            raise DistronodeParserError("unexpected parameter type in action: %s" % type(thing), obj=self._task_ds)
        return args
    ...
    def parse(self, skip_action_validation=False):
        ...
        if 'local_action' in self._task_ds:
            ...
            if action is not None:
                raise DistronodeParserError("action and local_action are mutually exclusive", obj=self._task_ds)
        ...
        for item, value in non_task_ds.items():
            ...
            if is_action_candidate:
                ...
                if action is not None:
                    raise DistronodeParserError("conflicting action statements: %s, %s" % (action, item), obj=self._task_ds)
        ...
        if action is None:
            if non_task_ds:  # there was one non-task action, but we couldn't find it
                ...
                raise DistronodeParserError("couldn't resolve module/action '{0}'. This often indicates a "
                                         "misspelling, missing collection, or incorrect module path.".format(bad_action),
                                         obj=self._task_ds)
            else:
                raise DistronodeParserError("no module/action detected in task.",
                                         obj=self._task_ds)
            ...
            if templar.is_template(raw_params):
                ...
                raise DistronodeParserError("this task '%s' has extra params, which is only allowed in the following modules: %s" % (action,
