...
from distronode.errors import DistronodeError, DistronodeParserError, DistronodeUndefinedVariable, DistronodeConnectionFailure, DistronodeActionFail, DistronodeActionSkip
from distronode.executor.task_result import TaskResult
...
from distronode.module_utils.six import binary_type
from distronode.module_utils.common.text.converters import to_text, to_native
from distronode.module_utils.connection import write_to_file_descriptor
from distronode.playbook.conditional import Conditional
from distronode.playbook.task import Task
from distronode.plugins import get_plugin_class
from distronode.plugins.loader import become_loader, cliconf_loader, connection_loader, httpapi_loader, netconf_loader, terminal_loader
from distronode.template import Templar
from distronode.utils.collection_loader import DistronodeCollectionConfig
from distronode.utils.listify import listify_lookup_plugin_terms
from distronode.utils.unsafe_proxy import to_unsafe_text, wrap_var
from distronode.vars.clean import namespace_facts, clean_facts
from distronode.utils.display import Display
from distronode.utils.vars import combine_vars, isidentifier
...
class TaskExecutor:
    ...
    def run(self):
        ...
        try:
                ...
                if len(items) > 0:
                    ...
                    for item in item_results:
                        ...
                        if 'failed' in item and item['failed']:
                            item_ignore = item.pop('_distronode_ignore_errors')
                        ...
                        if 'unreachable' in item and item['unreachable']:
                            item_ignore_unreachable = item.pop('_distronode_ignore_unreachable')
            ...
            return dict(failed=True, msg=wrap_var(to_text(e, nonstring='simplerepr')), _distronode_no_log=self._play_context.no_log)
        except Exception as e:
            return dict(failed=True, msg=wrap_var('Unexpected failure during module execution: %s' % (to_native(e, nonstring='simplerepr'))),
                        exception=to_text(traceback.format_exc()), stdout='', _distronode_no_log=self._play_context.no_log)
        finally:
            try:
    ...
    def _get_loop_items(self):
        ...
        self._job_vars['distronode_search_path'] = self._task.get_search_path()

        # ensure basedir is always in (dwim already searches here but we need to display it)
        if self._loader.get_basedir() not in self._job_vars['distronode_search_path']:
            self._job_vars['distronode_search_path'].append(self._loader.get_basedir())
        ...
        if self._task.loop_with:
            if self._task.loop_with in self._shared_loader_obj.lookup_loader:
                ...
                raise DistronodeError("Unexpected failure in finding the lookup named '%s' in the available lookup plugins" % self._task.loop_with)
    ...
    def _run_loop(self, items):
        ...
        for item_index, item in enumerate(items):
            task_vars['distronode_loop_var'] = loop_var

            task_vars[loop_var] = item
            if index_var:
                task_vars['distronode_index_var'] = index_var
                task_vars[index_var] = item_index

            if extended:
                task_vars['distronode_loop'] = {
                    'index': item_index + 1,
                    'index0': item_index,
                    'first': item_index == 0,
                ...
                if extended_allitems:
                    task_vars['distronode_loop']['allitems'] = items
                try:
                    task_vars['distronode_loop']['nextitem'] = items[item_index + 1]
                except IndexError:
                    pass
                if item_index - 1 >= 0:
                    task_vars['distronode_loop']['previtem'] = items[item_index - 1]
            ...
            except DistronodeParserError as e:
            ...
            res[loop_var] = item
            res['distronode_loop_var'] = loop_var
            if index_var:
                res[index_var] = item_index
                res['distronode_index_var'] = index_var
            if extended:
                res['distronode_loop'] = task_vars['distronode_loop']

            res['_distronode_item_result'] = True
            res['_distronode_ignore_errors'] = task_fields.get('ignore_errors')
            res['_distronode_ignore_unreachable'] = task_fields.get('ignore_unreachable')

            # gets templated here unlike rest of loop_control fields, depends on loop_var above
            try:
                res['_distronode_item_label'] = templar.template(label)
            except DistronodeUndefinedVariable as e:
                res.update({
                    'failed': True,
    ...
    def _execute(self, variables=None):
        ...
        try:
            ...
            if not conditional_result:
                ...
                return dict(changed=False, skipped=True, skip_reason='Conditional result was False',
                            false_condition=false_condition, _distronode_no_log=no_log)
        except DistronodeError as e:
        ...
        if context_validation_error is not None:
            ...
            if self._task.delegate_to:
                if isinstance(context_validation_error, DistronodeUndefinedVariable):
                    raiseit = False
                elif isinstance(context_validation_error, DistronodeParserError):
                    # parser error, might be cause by undef too
                    orig_exc = getattr(context_validation_error, 'orig_exc', None)
                    if isinstance(orig_exc, DistronodeUndefinedVariable):
                        raiseit = False
            if raiseit:
        ...
        try:
            ...
            return dict(changed=False, failed=True, _distronode_no_log=no_log, exception=to_text(traceback.format_exc()))
        if '_variable_params' in self._task.args:
            variable_params = self._task.args.pop('_variable_params')
            if isinstance(variable_params, dict):
                if C.INJECT_FACTS_AS_VARS:
                    display.warning("Using a variable for a task's 'args' is unsafe in some situations "
                                    "(see https://docs.distronode.github.io/distronode/devel/reference_appendices/faq.html#argsplat-unsafe)")
                variable_params.update(self._task.args)
                self._task.args = variable_params
            else:
                # if we didn't get a dict, it means there's garbage remaining after k=v parsing, just give up
                # see https://github.com/distronode/distronode/issues/79862
                raise DistronodeError(f"invalid or malformed argument: '{variable_params}'")
        ...
        if self._task.delegate_to:
            ...
            cvars = variables.get('distronode_delegated_vars', {}).get(self._task.delegate_to, {})
        else:
        ...
        if cvars.get('distronode_connection') is not None:
            current_connection = templar.template(cvars['distronode_connection'])
        else:
        ...
        # update with connection info (i.e distronode_host/distronode_user)
        ...
        # special handling for python interpreter for network_os, default to distronode python unless overridden
        if 'distronode_python_interpreter' not in cvars and 'distronode_network_os' in cvars and getattr(self._connection, '_remote_is_local', False):
            # this also avoids 'python discovery'
            cvars['distronode_python_interpreter'] = sys.executable
        ...
        for attempt in range(1, retries + 1):
            ...
            except (DistronodeActionFail, DistronodeActionSkip) as e:
                return e.result
            except DistronodeConnectionFailure as e:
            ...
            # preserve no log
            result["_distronode_no_log"] = no_log
            ...
            if self._task.register:
                if not isidentifier(self._task.register):
                    raise DistronodeError("Invalid variable name in 'register' specified: '%s'" % self._task.register)
            ...
            if 'distronode_facts' in result and self._task.action not in C._ACTION_DEBUG:
                if self._task.action in C._ACTION_WITH_CLEAN_FACTS:
                    if self._task.delegate_to and self._task.delegate_facts:
                        if '_distronode_delegated_vars' in vars_copy:
                            vars_copy['_distronode_delegated_vars'].update(result['distronode_facts'])
                        else:
                            vars_copy['_distronode_delegated_vars'] = result['distronode_facts']
                    else:
                        vars_copy.update(result['distronode_facts'])
                else:
                    # TODO: cleaning of facts should eventually become part of taskresults instead of vars
                    af = wrap_var(result['distronode_facts'])
                    vars_copy['distronode_facts'] = combine_vars(vars_copy.get('distronode_facts', {}), namespace_facts(af))
                    if C.INJECT_FACTS_AS_VARS:
        ...
        if 'distronode_facts' in result and self._task.action not in C._ACTION_DEBUG:
            if self._task.action in C._ACTION_WITH_CLEAN_FACTS:
                variables.update(result['distronode_facts'])
            else:
                # TODO: cleaning of facts should eventually become part of taskresults instead of vars
                af = wrap_var(result['distronode_facts'])
                variables['distronode_facts'] = combine_vars(variables.get('distronode_facts', {}), namespace_facts(af))
                if C.INJECT_FACTS_AS_VARS:
        ...
        if self._task.notify is not None:
            result['_distronode_notify'] = self._task.notify

        # add the delegated vars to the result, so we can reference them
        # on the results side without having to do any further templating
        # also now add connection vars results when delegating
        if self._task.delegate_to:
            result["_distronode_delegated_vars"] = {'distronode_delegated_host': self._task.delegate_to}
            for k in plugin_vars:
                result["_distronode_delegated_vars"][k] = cvars.get(k)

            # note: here for callbacks that rely on this info to display delegation
            for requireshed in ('distronode_host', 'distronode_port', 'distronode_user', 'distronode_connection'):
                if requireshed not in result["_distronode_delegated_vars"] and requireshed in cvars:
                    result["_distronode_delegated_vars"][requireshed] = cvars.get(requireshed)
    ...
    def _poll_async_result(self, result, templar, task_vars=None):
        ...
        async_jid = result.get('distronode_job_id')
        ...
        async_handler = self._shared_loader_obj.action_loader.get(
            'distronode.legacy.async_status',
            task=async_task,
        ...
        while time_left > 0:
            ...
            try:
                ...
                if (int(async_result.get('finished', 0)) == 1 or
                        ('failed' in async_result and async_result.get('_distronode_parsed', False)) or
        ...
        if int(async_result.get('finished', 0)) != 1:
            if async_result.get('_distronode_parsed'):
    ...
    def _get_become(self, name):
        ...
        if not become:
            raise DistronodeError("Invalid become method specified, could not find matching plugin: '%s'. "
                               "Use `distronode-doc -t become -l` to list available plugins." % name)
        return become
    ...
    def _get_connection(self, cvars, templar, current_connection):
        ...
        connection, plugin_load_context = self._shared_loader_obj.connection_loader.get_with_context(
            ...
            distronode_playbook_pid=to_text(os.getppid())
        )

        if not connection:
            raise DistronodeError("the connection plugin '%s' was not found" % conn_type)
    ...
    def _set_become_plugin(self, cvars, templar, connection):
        ...
        if cvars.get('distronode_become') is not None:
            become = boolean(templar.template(cvars['distronode_become']))
        else:
            become = self._task.become

        if become:
            if cvars.get('distronode_become_method'):
                become_plugin = self._get_become(templar.template(cvars['distronode_become_method']))
            else:
            ...
            # https://github.com/distronode/distronode/issues/78425
            become_plugin = None
    ...
    def _set_connection_options(self, variables, templar):
        ...
        if getattr(self._connection, 'allow_extras', False):
            for k in variables:
                if k.startswith('distronode_%s_' % self._connection._load_name) and k not in options:
    ...
    def _get_action_handler_with_module_context(self, templar):
        ...
        # let action plugin override module, fallback to 'normal' action plugin otherwise
            ...
            # use distronode.legacy.normal to allow (historic) local action_plugins/ override without collections search
            handler_name = 'distronode.legacy.normal'
        ...
        if not handler:
            raise DistronodeError("the handler '%s' was not found" % handler_name)
...
def start_connection(play_context, options, task_uuid):
    ...
    '''
    candidate_paths = [C.DISTRONODE_CONNECTION_PATH or os.path.dirname(sys.argv[0])]
    candidate_paths.extend(os.environ.get('PATH', '').split(os.pathsep))
    for dirname in candidate_paths:
        distronode_connection = os.path.join(dirname, 'distronode-connection')
        if os.path.isfile(distronode_connection):
            display.vvvv("Found distronode-connection at path {0}".format(distronode_connection))
            break
    else:
        raise DistronodeError("Unable to find location of 'distronode-connection'. "
                           "Please set or check the value of DISTRONODE_CONNECTION_PATH")
    ...
    env.update({
        ...
        # to invalidate/update, distronode-connection won't always see the same plugins the controller
        # can.
        'DISTRONODE_BECOME_PLUGINS': become_loader.print_paths(),
        'DISTRONODE_CLICONF_PLUGINS': cliconf_loader.print_paths(),
        'DISTRONODE_COLLECTIONS_PATH': to_native(os.pathsep.join(DistronodeCollectionConfig.collection_paths)),
        'DISTRONODE_CONNECTION_PLUGINS': connection_loader.print_paths(),
        'DISTRONODE_HTTPAPI_PLUGINS': httpapi_loader.print_paths(),
        'DISTRONODE_NETCONF_PLUGINS': netconf_loader.print_paths(),
        'DISTRONODE_TERMINAL_PLUGINS': terminal_loader.print_paths(),
    })
    verbosity = []
    ...
    p = subprocess.Popen(
        [python, distronode_connection, *verbosity, to_text(os.getppid()), to_text(task_uuid)],
    ...
    if 'error' in result:
        ...
        raise DistronodeError(result['error'])
