...
description:
    ...
    - These variables will be available to subsequent plays during an distronode-playbook run via the host they were set on.
    ...
    - Per the standard Distronode variable precedence rules, other types of variables have a higher priority, so this value may be overridden.
options:
  key_value:
    description:
      - "The M(distronode.builtin.set_fact) module takes C(key=value) pairs or C(key: value) (YAML notation) as variables to set in the playbook scope.
  ...
  cacheable:
    description:
      ...
      - Normally this module creates 'host level variables' and has much higher precedence, this option changes the nature and precedence
        ...
        U(https://docs.distronode.com/distronode/latest/user_guide/playbooks_variables.html#variable-precedence-where-should-i-put-a-variable)
...
seealso:
- module: distronode.builtin.include_vars
- ref: distronode_variable_precedence
...
- name: Setting host facts using key=value pairs, this format can only create strings or booleans
  distronode.builtin.set_fact: one_fact="something" other_fact="{{ local_var }}"

- name: Setting host facts using complex arguments
  distronode.builtin.set_fact:
    one_fact: something
    other_fact: "{{ local_var * 2 }}"
    another_fact: "{{ some_registered_var.results | map(attribute='distronode_facts.some_fact') | list }}"
...
- name: Creating list and dictionary variables
  distronode.builtin.set_fact:
    ...
    one_list:
        ...
        - b
        - c
# As of Distronode 1.8, Distronode will convert boolean strings ('true', 'false', 'yes', 'no')
