...
syslog.openlog('distronode-%s' % os.path.basename(__file__))
...
def _run_module(wrapped_cmd, jid):
    ...
    jwrite({"started": 1, "finished": 0, "distronode_job_id": jid})
...
def main():
    ...
    async_dir = os.environ.get('DISTRONODE_ASYNC_DIR', '~/.distronode_async')
    ...
    try:
        ...
        if pid:
            ...
            # to initialize PRIOR to distronode trying to clean up the launch directory (and argsfile)
            ...
            end({"failed": 0, "started": 1, "finished": 0, "distronode_job_id": jid, "results_file": job_path,
                 "_distronode_suppress_tmpdir_delete": (not preserve_tmp)}, 0)
        else:
