...
description:
     ...
     - From Distronode 2.4 when run with C(--check), it will do a HEAD request to validate the URL but
       will not download the entire file or verify it against hashes and will report incorrect changed status.
     - For Windows targets, use the M(distronode.windows.win_get_url) module instead.
version_added: '0.6'
options:
  ciphers:
    description:
  ...
  tmp_dest:
    description:
      ...
      - When run on Distronode 2.5 or greater, path defaults to distronode's remote_tmp setting
      - When run on Distronode prior to 2.5, it defaults to E(TMPDIR), E(TEMP) or E(TMP) env variables or a platform specific value.
  ...
  headers:
    description:
        ...
        - The hash/dict format was added in Distronode 2.6.
...
- module: distronode.windows.win_get_url
author:
...
from distronode.module_utils.six.moves.urllib.parse import urlsplit
...
from distronode.module_utils.urls import fetch_url, url_argument_spec
