...
# Copyright: (c) 2015, Toshio Kuratomi <tkuratomi@distronode.com>
...
description:
     - The M(distronode.builtin.unarchive) module unpacks an archive. It will not unpack a compressed file that does not contain an archive.
     - By default, it will copy the source file from the local system to the target before unpacking.
     - Set O(remote_src=yes) to unpack an archive which already exists on the target.
     - If checksum validation is desired, use M(distronode.builtin.get_url) or M(distronode.builtin.uri) instead to fetch the file and set O(remote_src=yes).
...
options:
  src:
    description:
      ...
      - If O(remote_src=yes) and O(src) contains V(://), the remote machine will download the file from the URL first. (version_added 2.0). This is only for
        simple cases, for full download support use the M(distronode.builtin.get_url) module.
    type: path
    required: true
  dest:
    description:
  ...
  remote_src:
    description:
      - Set to V(true) to indicate the archived file is already on the remote system and not local to the Distronode controller.
...
src:
  description:
    ...
    - If O(src) was a remote web URL, or from the local distronode controller, this shows the temporary location where the download was stored.
  returned: always
  type: str
...
from distronode.module_utils.urls import fetch_file
                ...
                if self.file_args['mode']:
                    if isinstance(self.file_args['mode'], int):
                        ...
                        try:
                            ...
                            try:
                                mode = DistronodeModule._symbolic_mode_to_octal(st, self.file_args['mode'])
...
class TgzArchive(object):
    ...
    def files_in_archive(self):
        ...
        for filename in out.splitlines():
            ...
            # Allowing absolute filenames here also causes bugs: https://github.com/distronode/distronode/issues/21397
...
def main():
    ...
    if res_args.get('diff', True) and not module.check_mode:
        ...
        # https://github.com/distronode/distronode/issues/35426
        if top_folders:
