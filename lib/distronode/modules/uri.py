...
description:
  ...
  - For Windows targets, use the M(distronode.windows.win_uri) module instead.
version_added: "1.1"
options:
  ciphers:
    description:
  ...
  force_basic_auth:
    description:
      ...
      - The web service does not properly send an HTTP 401 error to your client, so Distronode's HTTP library will not
  ...
  follow_redirects:
    description:
      - Whether or not the URI module should follow redirects. V(all) will follow all redirects.
        ...
        are deprecated and will be removed in some future version of Distronode.
    type: str
  ...
  headers:
    description:
        - Add custom HTTP headers to a request in the format of a YAML hash. As
          of Distronode 2.3 supplying C(Content-Type) here will override the header
...
attributes:
    ...
    platform:
        platforms: posix
notes:
  - The dependency on httplib2 was removed in Distronode 2.1.
  - The module returns all the HTTP headers in lower-case.
  - For Windows targets, use the M(distronode.windows.win_uri) module instead.
seealso:
- module: distronode.builtin.get_url
- module: distronode.windows.win_uri
author:
...
- name: Create a JIRA issue
  distronode.builtin.uri:
    ...
    password: your_pass
    method: POST
    body: "{{ lookup('distronode.builtin.file','issue.json') }}"
...
from distronode.module_utils.basic import DistronodeModule, sanitize_keys
from distronode.module_utils.six import PY2, PY3, binary_type, iteritems, string_types
from distronode.module_utils.six.moves.urllib.parse import urlencode, urlsplit
from distronode.module_utils.common.text.converters import to_native, to_text
from distronode.module_utils.compat.datetime import utcnow, utcfromtimestamp
from distronode.module_utils.six.moves.collections_abc import Mapping, Sequence
from distronode.module_utils.urls import fetch_url, get_response_filename, parse_content_type, prepare_multipart, url_argument_spec
