...
description:
     - The M(distronode.builtin.command) module takes the command name followed by a list of space-delimited arguments.
...
notes:
    -  If you want to run a command through the shell (say you are using C(<), C(>), C(|), and so on),
       you actually want the M(distronode.builtin.shell) module instead.
       Parsing shell metacharacters can lead to unexpected commands being executed if quoting is not done correctly so it is more secure to
       use the M(distronode.builtin.command) module when possible.
    ...
    -  The O(ignore:executable) parameter is removed since version 2.4. If you have a need for this parameter, use the M(distronode.builtin.shell) module instead.
    -  For Windows targets, use the M(distronode.windows.win_command) module instead.
    -  For rebooting systems, use the M(distronode.builtin.reboot) or M(distronode.windows.win_reboot) module.
    -  If the command returns non UTF-8 data, it must be encoded to avoid issues. This may necessitate using M(distronode.builtin.shell) so the output
       can be piped through C(base64).
seealso:
- module: distronode.builtin.raw
- module: distronode.builtin.script
- module: distronode.builtin.shell
- module: distronode.windows.win_command
author:
...
- name: Return motd to registered var
  distronode.builtin.command: cat /etc/motd
  register: mymotd

# free-form (string) arguments, all arguments on one line
- name: Run command if /path/to/database does not exist (without 'args')
  distronode.builtin.command: /usr/bin/make_database.sh db_user db_name creates=/path/to/database

# free-form (string) arguments, some arguments on separate lines with the 'args' keyword
# 'args' is a task keyword, passed at the same level as the module
- name: Run command if /path/to/database does not exist (with 'args' keyword)
  distronode.builtin.command: /usr/bin/make_database.sh db_user db_name
  args:
...
- name: Safely use templated variable to run command. Always use the quote filter to avoid injection issues
  distronode.builtin.command: cat {{ myfile|quote }}
  register: myoutput
'''
...
from distronode.module_utils.common.text.converters import to_native, to_bytes, to_text
from distronode.module_utils.common.collections import is_iterable


def main():

    # the command module is the one distronode module that does not take key=value args
    ...
    if not shell and executable:
        module.warn("As of Distronode 2.4, the parameter 'executable' is no longer supported with the 'command' module. Not using '%s'." % executable)
        executable = None
