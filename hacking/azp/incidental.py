...
def parse_args():
    ...
    parser.add_argument('--source',
                        type=optional_directory,
                        default=source,
                        help='path to git repository containing Distronode source')
...
def incidental_report(args):
    ...
    exclude_path = '^(test/support/|lib/distronode/module_utils/six/)'
...
def get_target_name_from_plugin_path(path):  # type: (str) -> str
    ...
    if path.startswith('lib/distronode/modules/'):
        plugin_type = None
    elif path.startswith('lib/distronode/plugins/'):
        plugin_type = parts[3]
    elif path.startswith('lib/distronode/module_utils/'):
        plugin_type = parts[2]
...
class CoverageData:
    def __init__(self, result_path):
        ...
        self.github_base_url = 'https://github.com/distronode/distronode/blob/%s/' % self.result_sha
...
class CoverageTool:
    def __init__(self):
        self.analyze_cmd = ['distronode-test', 'coverage', 'analyze', 'targets']
