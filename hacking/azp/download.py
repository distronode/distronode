...
def run_id_arg(arg):
    m = re.fullmatch(r"(?:https:\/\/dev\.azure\.com\/distronode\/distronode\/_build\/results\?buildId=)?(\d+)", arg)
    if not m:
...
def download_run(args):
    ...
    if args.run_metadata:
        run_url = 'https://dev.azure.com/distronode/distronode/_apis/pipelines/%s/runs/%s?api-version=6.0-preview.1' % (args.pipeline_id, args.run)
    ...
    timeline_response = requests.get('https://dev.azure.com/distronode/distronode/_apis/build/builds/%s/timeline?api-version=6.0' % args.run)
    ...
    if args.artifacts:
        artifact_list_url = 'https://dev.azure.com/distronode/distronode/_apis/build/builds/%s/artifacts?api-version=6.0' % args.run
