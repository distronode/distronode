...
from distronode.utils.color import stringc
import requests
import sys
import datetime
...
def get_coverage_runs():
    list_response = requests.get("https://dev.azure.com/distronode/distronode/_apis/pipelines/%s/runs?api-version=6.0-preview.1" % PIPELINE_ID)
    ...
    for run_summary in runs["value"][0:1000]:
        ...
        if run_response.status_code == 500 and 'Cannot serialize type Microsoft.Azure.Pipelines.WebApi.ContainerResource' in run_response.json()['message']:
            ...
            # A work-around for this issue was applied in: https://github.com/distronode/distronode/pull/80299
            break
        ...
        artifact_response = requests.get("https://dev.azure.com/distronode/distronode/_apis/build/builds/%s/artifacts?api-version=6.0" % run['id'])
...
def pretty_coverage_runs(runs):
    ...
    for run in sorted(ended, key=lambda x: x['finishedDate']):
        if run['result'] == "succeeded":
            print('🙂 [%s] https://dev.azure.com/distronode/distronode/_build/results?buildId=%s (%s)' % (
                stringc('PASS', 'green'),
                run['id'],
                run['finishedDate']))
        else:
    ...
    if in_progress:
        ...
        for run in in_progress:
            print('🤔 [%s] https://dev.azure.com/distronode/distronode/_build/results?buildId=%s' % (
                stringc('FATE', 'yellow'),
                run['id']))
