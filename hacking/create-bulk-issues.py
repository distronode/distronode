...
PROJECT = f'distronode-core {MAJOR_MINOR_VERSION}'
...
def run_sanity_test(test_name: str) -> list[str]:
    cmd = ['distronode-test', 'sanity', '--test', test_name, '--lint', '--failure-ok']
    skip_path = 'test/sanity/code-smell/skip.txt'
    skip_temp_path = skip_path + '.tmp'

    os.rename(skip_path, skip_temp_path)  # make sure distronode-test isn't configured to skip any tests
