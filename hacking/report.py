...
DATABASE_PATH = os.path.expanduser('~/.distronode/report.db')
BASE_PATH = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..')) + '/'
DISTRONODE_PATH = os.path.join(BASE_PATH, 'lib')
DISTRONODE_TEST_PATH = os.path.join(BASE_PATH, 'test/lib')

if DISTRONODE_PATH not in sys.path:
    sys.path.insert(0, DISTRONODE_PATH)

if DISTRONODE_TEST_PATH not in sys.path:
    sys.path.insert(0, DISTRONODE_TEST_PATH)

from distronode.module_utils.urls import open_url
from distronode.parsing.plugin_docs import read_docstring

from distronode_test._internal.target import walk_integration_targets
...
def populate_modules():
    module_dir = os.path.join(BASE_PATH, 'lib/distronode/modules/')
...
def populate_coverage():
    response = open_url('https://codecov.io/api/gh/distronode/distronode/tree/devel/?src=extension')
