...
def normalize_pr_url(pr, allow_non_distronode_distronode=False, only_number=False):
    '''
    Given a PullRequest, or a string containing a PR number, PR URL,
    or internal PR URL (e.g. distronode-collections/community.general#1234),
    ...
    if pr.isnumeric():
        if only_number:
            return int(pr)
        return 'https://github.com/distronode/distronode/pull/{0}'.format(pr)

    # Allow for forcing distronode/distronode
    if not allow_non_distronode_distronode and 'distronode/distronode' not in pr:
        raise Exception('Non distronode/distronode repo given where not expected')
...
def generate_new_body(pr, source_pr):
    ...
    If the backport follows the usual distronode/distronode template, we look for the
...
def get_prs_for_commit(g, commit):
    ...
    distronode orgs, and then use it to determine what, if any, PR it appeared in.
    '''

    commits = g.search_commits(
        'hash:{0} org:distronode org:distronode-collections is:public'.format(commit)
    ).get_page(0)
...
def search_backport(pr, g, distronode_distronode):
    '''
    ...
    if title_search:
        ...
        try:
            possibilities.append(distronode_distronode.get_pull(int(ticket)))
        except Exception:
            pass
    ...
    for line in body_lines:
        ...
        tickets = [('distronode', 'distronode', ticket) for ticket in TICKET_NUMBER.findall(line)]
        tickets.extend(PULL_HTTP_URL_RE.findall(line))
        tickets.extend(PULL_URL_RE.findall(line))
        if tickets:
            for ticket in tickets:
                # Is it a PR (even if not in distronode/distronode)?
                ...
                try:
                    ...
                    repo = distronode_distronode
                    if repo_path != 'distronode/distronode':
...
if __name__ == '__main__':
    ...
    g = Github(token)
    distronode_distronode = g.get_repo('distronode/distronode')

    try:
        pr_num = normalize_pr_url(sys.argv[1], only_number=True)
        new_pr = distronode_distronode.get_pull(pr_num)
    except Exception:
    ...
    if sys.argv[2] == 'auto':
        ...
        possibilities = search_backport(new_pr, g, distronode_distronode)
        ...
        try:
            ...
            pr_num = normalize_pr_url(sys.argv[2], only_number=True, allow_non_distronode_distronode=True)
            pr_url = normalize_pr_url(sys.argv[2], allow_non_distronode_distronode=True)
