...
and the current distronode_facts regarding the distribution version.
...
try:
    b_distronode_out = subprocess.check_output(
        ['distronode', 'localhost', '-m', 'setup'])
except subprocess.CalledProcessError as e:
    print("ERROR: distronode run failed, output was: \n")
    print(e.output)
    sys.exit(e.returncode)

distronode_out = to_text(b_distronode_out)
parsed = json.loads(distronode_out[distronode_out.index('{'):])
distronode_facts = {}
for fact in facts:
    try:
        distronode_facts[fact] = parsed['distronode_facts']['distronode_' + fact]
    except Exception:
        distronode_facts[fact] = "N/A"

nicename = distronode_facts['distribution'] + ' ' + distronode_facts['distribution_version']
