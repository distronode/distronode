...
from distronode.executor import module_common as amc
...
class TestSlurp:
    def test_slurp_nonexistent(self, mocker):
        ...
        with pytest.raises(distronode.errors.DistronodeError):
            amc._slurp('no_file')
...
class TestGetShebang:
    ...
    def test_interpreter_set_in_task_vars(self, templar):
        assert amc._get_shebang(u'/usr/bin/python', {u'distronode_python_interpreter': u'/usr/bin/pypy'}, templar) == \
            (u'#!/usr/bin/pypy', u'/usr/bin/pypy')

    def test_non_python_interpreter_in_task_vars(self, templar):
        assert amc._get_shebang(u'/usr/bin/ruby', {u'distronode_ruby_interpreter': u'/usr/local/bin/ruby'}, templar) == \
            (u'#!/usr/local/bin/ruby', u'/usr/local/bin/ruby')

    def test_with_args(self, templar):
        assert amc._get_shebang(u'/usr/bin/python', {u'distronode_python_interpreter': u'/usr/bin/python3'}, templar, args=('-tt', '-OO')) == \
            (u'#!/usr/bin/python3 -tt -OO', u'/usr/bin/python3')

    def test_python_via_env(self, templar):
        assert amc._get_shebang(u'/usr/bin/python', {u'distronode_python_interpreter': u'/usr/bin/env python'}, templar) == \
            (u'#!/usr/bin/env python', u'/usr/bin/env python')


class TestDetectionRegexes:
    DISTRONODE_MODULE_UTIL_STRINGS = (
        # Absolute collection imports
        b'import distronode_collections.my_ns.my_col.plugins.module_utils.my_util',
        b'from distronode_collections.my_ns.my_col.plugins.module_utils import my_util',
        b'from distronode_collections.my_ns.my_col.plugins.module_utils.my_util import my_func',
        # Absolute core imports
        b'import distronode.module_utils.basic',
        b'from distronode.module_utils import basic',
        b'from distronode.module_utils.basic import DistronodeModule',
        # Relative imports
        b'from ..module_utils import basic',
        b'from .. module_utils import basic',
        b'from ....module_utils import basic',
        b'from ..module_utils.basic import DistronodeModule',
    )
    NOT_DISTRONODE_MODULE_UTIL_STRINGS = (
        b'from distronode import release',
        b'from ..release import __version__',
        b'from .. import release',
        b'from distronode.modules.system import ping',
        b'from distronode_collecitons.my_ns.my_col.plugins.modules import function',
    )

    OFFSET = os.path.dirname(os.path.dirname(amc.__file__))
    CORE_PATHS = (
        ('%s/modules/from_role.py' % OFFSET, 'distronode/modules/from_role'),
        ('%s/modules/system/ping.py' % OFFSET, 'distronode/modules/system/ping'),
        ('%s/modules/cloud/amazon/s3.py' % OFFSET, 'distronode/modules/cloud/amazon/s3'),
    )

    COLLECTION_PATHS = (
        ('/root/distronode_collections/ns/col/plugins/modules/ping.py',
         'distronode_collections/ns/col/plugins/modules/ping'),
        ('/root/distronode_collections/ns/col/plugins/modules/subdir/ping.py',
         'distronode_collections/ns/col/plugins/modules/subdir/ping'),
    )

    @pytest.mark.parametrize('testcase', DISTRONODE_MODULE_UTIL_STRINGS)
    def test_detect_new_style_python_module_re(self, testcase):
        assert amc.NEW_STYLE_PYTHON_MODULE_RE.search(testcase)

    @pytest.mark.parametrize('testcase', NOT_DISTRONODE_MODULE_UTIL_STRINGS)
