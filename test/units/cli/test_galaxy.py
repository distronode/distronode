...
from distronode.cli.galaxy import GalaxyCLI
from distronode.galaxy import collection
...
class TestGalaxy(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.temp_dir = tempfile.mkdtemp(prefix='distronode-test_galaxy-')
        os.chdir(cls.temp_dir)

        shutil.rmtree("./delete_me", ignore_errors=True)

        # creating framework for a role
        gc = GalaxyCLI(args=["distronode-galaxy", "init", "--offline", "delete_me"])
        gc.run()
    ...
    def setUp(self):
        ...
        co.GlobalCLIArgs._Singleton__instance = None
        self.default_args = ['distronode-galaxy']
    ...
    def test_run(self):
        ...
        gc = GalaxyCLI(args=["distronode-galaxy", "install", "--ignore-errors", "imaginary_role"])
        gc.parse()
        with patch.object(distronode.cli.CLI, "run", return_value=None) as mock_run:
            gc.run()
            # testing
            self.assertIsInstance(gc.galaxy, distronode.galaxy.Galaxy)
            self.assertEqual(mock_run.call_count, 1)
            self.assertTrue(isinstance(gc.api, distronode.galaxy.api.GalaxyAPI))

    def test_execute_remove(self):
        # installing role
        gc = GalaxyCLI(args=["distronode-galaxy", "install", "-p", self.role_path, "-r", self.role_req, '--force'])
        gc.run()
        ...
        co.GlobalCLIArgs._Singleton__instance = None
        gc = GalaxyCLI(args=["distronode-galaxy", "remove", role_file, self.role_name])
        gc.run()
    ...
    def test_exit_without_ignore_without_flag(self):
        ...
        gc = GalaxyCLI(args=["distronode-galaxy", "install", "--server=None", "fake_role_name"])
        with patch.object(distronode.utils.display.Display, "display", return_value=None) as mocked_display:
            # testing that error expected is raised
            self.assertRaises(DistronodeError, gc.run)
    ...
    def test_exit_without_ignore_with_flag(self):
        ...
        gc = GalaxyCLI(args=["distronode-galaxy", "install", "--server=None", "fake_role_name", "--ignore-errors"])
        with patch.object(distronode.utils.display.Display, "display", return_value=None) as mocked_display:
            gc.run()
            assert mocked_display.call_count == 2
            assert mocked_display.mock_calls[0].args[0] == "Starting galaxy role install process"
    ...
    def test_parse_install(self):
        ...
        gc = GalaxyCLI(args=["distronode-galaxy", "install"])
        gc.parse()
    ...
    def test_parse_list(self):
        ...
        gc = GalaxyCLI(args=["distronode-galaxy", "list"])
        gc.parse()
        self.assertEqual(context.CLIARGS['verbosity'], 0)

    def test_parse_remove(self):
        ''' testing the options parser when the action 'remove' is given '''
        gc = GalaxyCLI(args=["distronode-galaxy", "remove", "foo"])
        gc.parse()
        self.assertEqual(context.CLIARGS['verbosity'], 0)

    def test_parse_search(self):
        ''' testing the options parswer when the action 'search' is given '''
        gc = GalaxyCLI(args=["distronode-galaxy", "search"])
        gc.parse()
    ...
    def test_parse_setup(self):
        ...
        gc = GalaxyCLI(args=["distronode-galaxy", "setup", "source", "github_user", "github_repo", "secret"])
        gc.parse()
        self.assertEqual(context.CLIARGS['verbosity'], 0)
...
@pytest.mark.parametrize('cli_args, expected', [
    (['distronode-galaxy', 'collection', 'init', 'abc._def'], 0),
    (['distronode-galaxy', 'collection', 'init', 'abc._def', '-vvv'], 3),
    (['distronode-galaxy', 'collection', 'init', 'abc._def', '-vv'], 2),
])
...
def collection_skeleton(request, tmp_path_factory):
    ...
    galaxy_args = ['distronode-galaxy', 'collection', 'init', '-c']
...
@pytest.mark.parametrize('collection_skeleton', [
    ('distronode_test.my_collection', None),
], indirect=True)
def test_collection_default(collection_skeleton):
    ...
    assert metadata['namespace'] == 'distronode_test'
...
@pytest.mark.parametrize('collection_skeleton', [
    ('distronode_test.delete_me_skeleton', os.path.join(os.path.split(__file__)[0], 'test_data', 'collection_skeleton')),
], indirect=True)
def test_collection_skeleton(collection_skeleton):
    ...
    assert metadata['authors'] == ['Distronode Cow <acow@bovineuniversity.edu>', 'Tu Cow <tucow@bovineuniversity.edu>']
...
def collection_artifact(collection_skeleton, tmp_path_factory):
    ...
    try:
        co.GlobalCLIArgs._Singleton__instance = None
        galaxy_args = ['distronode-galaxy', 'collection', 'build', collection_skeleton, '--output-path', output_dir]
...
def test_invalid_skeleton_path():
    ...
    gc = GalaxyCLI(args=['distronode-galaxy', 'collection', 'init', 'my.collection', '--collection-skeleton',
                         '/fake/path'])
...
def test_invalid_collection_name_init(name):
    ...
    gc = GalaxyCLI(args=['distronode-galaxy', 'collection', 'init', name])
    with pytest.raises(DistronodeError, match=expected):
        gc.run()
...
def test_invalid_collection_name_install(name, expected, tmp_path_factory):
    ...
    gc = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', name, '-p', os.path.join(install_path, 'install')])
    with pytest.raises(DistronodeError, match=expected):
        gc.run()


@pytest.mark.parametrize('collection_skeleton', [
    ('distronode_test.build_collection', None),
], indirect=True)
def test_collection_build(collection_artifact):
    tar_path = os.path.join(collection_artifact, 'distronode_test-build_collection-1.0.0.tar.gz')
    ...
    with tarfile.open(tar_path, mode='r') as tar:
        ...
        assert coll_info['namespace'] == 'distronode_test'
...
def collection_install(reset_cli_args, tmp_path_factory, monkeypatch):
    ...
    monkeypatch.setattr(distronode.cli.galaxy, 'install_collections', mock_install)

    mock_warning = MagicMock()
    monkeypatch.setattr(distronode.utils.display.Display, 'warning', mock_warning)
...
def test_collection_install_with_names(collection_install):
    ...
    galaxy_args = ['distronode-galaxy', 'collection', 'install', 'namespace.collection', 'namespace2.collection:1.0.1',
                   '--collections-path', output_dir]
    GalaxyCLI(args=galaxy_args).run()

    collection_path = os.path.join(output_dir, 'distronode_collections')
    assert os.path.isdir(collection_path)

    assert mock_warning.call_count == 1
    assert "The specified collections path '%s' is not part of the configured Distronode collections path" % output_dir \
...
def test_collection_install_with_invalid_requirements_format(collection_install):
    ...
    galaxy_args = ['distronode-galaxy', 'collection', 'install', '--requirements-file', requirements_file,
                   '--collections-path', output_dir]

    with pytest.raises(DistronodeError, match="Expecting requirements yaml to be a list or dictionary but got str"):
        GalaxyCLI(args=galaxy_args).run()
...
def test_collection_install_with_relative_path(collection_install, monkeypatch):
    ...
    monkeypatch.setattr(distronode.cli.galaxy.GalaxyCLI, '_parse_requirements_file', mock_req)

    monkeypatch.setattr(os, 'makedirs', MagicMock())

    requirements_file = './requirements.myl'
    collections_path = './distronode_collections'
    galaxy_args = ['distronode-galaxy', 'collection', 'install', '--requirements-file', requirements_file,
...
def test_collection_install_with_url(monkeypatch, collection_install):
    ...
    galaxy_args = ['distronode-galaxy', 'collection', 'install', 'https://foo/bar/foo-bar-v1.0.0.tar.gz',
                   '--collections-path', output_dir]
    GalaxyCLI(args=galaxy_args).run()
...
def test_collection_install_name_and_requirements_fail(collection_install):
    ...
    with pytest.raises(DistronodeError, match=expected):
        GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection', '--collections-path',
...
def test_collection_install_no_name_and_requirements_fail(collection_install):
    ...
    with pytest.raises(DistronodeError, match=expected):
        GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', '--collections-path', test_path]).run()


def test_collection_install_path_with_distronode_collections(collection_install):
    mock_install, mock_warning, output_dir = collection_install
    ...
    assert mock_warning.call_count == 1
    assert "The specified collections path '%s' is not part of the configured Distronode collections path" \
...
def requirements_cli(monkeypatch):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'install'])
    cli.run()
    return cli
...
def test_parse_requirements_roles_with_include(requirements_cli, requirements_file):
    ...
    assert actual['roles'][0].name == 'distronode.role'
...
def test_install_implicit_role_with_collections(requirements_file, monkeypatch):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'install', '-r', requirements_file])
    cli.run()
...
def test_install_explicit_role_with_collections(requirements_file, monkeypatch):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'role', 'install', '-r', requirements_file])
    cli.run()
...
def test_install_role_with_collections_and_path(requirements_file, monkeypatch):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'install', '-p', 'path', '-r', requirements_file])
    cli.run()
...
def test_install_collection_with_roles(requirements_file, monkeypatch):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', '-r', requirements_file])
    cli.run()
