...
from distronode.module_utils.api import basic_auth_argument_spec, rate_limit_argument_spec, retry_argument_spec
from distronode.module_utils.common import warnings
from distronode.module_utils.common.warnings import get_deprecation_messages, get_warning_messages
from distronode.module_utils.six import integer_types, string_types
...
INVALID_SPECS = (
    ...
    # unknown parameter
    ({'arg': {'type': 'int'}}, {'other': 'bad', '_distronode_module_name': 'distronode_unittest'},
     'Unsupported parameters for (distronode_unittest) module: other. Supported parameters include: arg.'),
    ({'arg': {'type': 'int', 'aliases': ['argument']}}, {'other': 'bad', '_distronode_module_name': 'distronode_unittest'},
     'Unsupported parameters for (distronode_unittest) module: other. Supported parameters include: arg (argument).'),
    # parameter is required
...
def test_validator_basic_types(argspec, expected, stdin):
    ...
    am = basic.DistronodeModule(argspec)
...
def test_validate_basic_auth_arg(mocker, stdin):
    ...
    am = basic.DistronodeModule(**kwargs)
...
def test_validator_fail(stdin, capfd, argspec, expected):
    with pytest.raises(SystemExit):
        basic.DistronodeModule(argument_spec=argspec)
...
class TestComplexArgSpecs:
    ...
    def test_complex_required(self, stdin, complex_argspec):
        ...
        am = basic.DistronodeModule(**complex_argspec)
    ...
    def test_complex_type_fallback(self, mocker, stdin, complex_argspec):
        ...
        mocker.patch('distronode.module_utils.basic.os.environ', environ)
    ...
    def test_fail_list_with_choices(self, capfd, mocker, stdin, complex_argspec):
        ...
        with pytest.raises(SystemExit):
            basic.DistronodeModule(**complex_argspec)
...
class TestComplexOptions:
    ...
    def test_options_type_dict(self, stdin, options_argspec_dict, expected):
        ...
        am = basic.DistronodeModule(**options_argspec_dict)
    ...
    def test_options_type_list(self, stdin, options_argspec_list, expected):
        """Test that a basic creation with required and required_if works"""
        # should test ok, tests basic foo requirement and required_if
        am = basic.DistronodeModule(**options_argspec_list)
    ...
    def test_subspec_not_required_defaults(self, stdin, spec, expected):
        ...
        am = basic.DistronodeModule(spec)
...
def test_no_log_true(stdin, capfd):
    ...
    am = basic.DistronodeModule(arg_spec)
...
def test_no_log_none(stdin, capfd):
    """Allow Distronode to make the decision by matching the argument name
