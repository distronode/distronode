...
def no_args_module_exec():
    with patch.object(basic, '_DISTRONODE_ARGS', b'{"DISTRONODE_MODULE_ARGS": {}}'):
...
# test DistronodeModule selinux wrapper methods
...
class TestSELinuxMU:
    ...
    def test_is_special_selinux_path(self):
        args = to_bytes(json.dumps(dict(DISTRONODE_MODULE_ARGS={'_distronode_selinux_special_fs': "nfs,nfsd,foos",
                                                             '_distronode_remote_tmp': "/tmp",
                                                             '_distronode_keep_remote_files': False})))

        with patch.object(basic, '_DISTRONODE_ARGS', args):
