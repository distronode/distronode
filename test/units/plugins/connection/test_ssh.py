...
from distronode.errors import DistronodeAuthenticationFailure
...
from distronode.plugins.connection import ssh
from distronode.plugins.loader import connection_loader, become_loader
...
class TestConnectionBaseClass(unittest.TestCase):
    ...
    def test_plugins_connection_ssh_put_file(self, mock_ospe, mock_sleep):
        ...
        self.assertRaises(DistronodeError, conn.put_file, '/path/to/bad/file', '/remote/path/to/file')

        # test that a not-found path raises an error
        mock_ospe.return_value = False
        conn._bare_run.return_value = (0, 'stdout', '')
        self.assertRaises(DistronodeFileNotFound, conn.put_file, '/path/to/bad/file', '/remote/path/to/file')
    ...
    def test_plugins_connection_ssh_fetch_file(self, mock_sleep):
        ...
        # test that a non-zero rc raises an error
        conn._bare_run.return_value = (1, 'stdout', 'some errors')
        self.assertRaises(DistronodeError, conn.fetch_file, '/path/to/bad/file', '/remote/path/to/file')
...
class TestSSHConnectionRetries(object):
    def test_incorrect_password(self, monkeypatch):
        ...
        exception_info = pytest.raises(DistronodeAuthenticationFailure, self.conn.exec_command, 'sshpass', 'some data')
    ...
    def test_multiple_failures(self, monkeypatch):
        ...
        pytest.raises(DistronodeConnectionFailure, self.conn.exec_command, 'ssh', 'some data')
    ...
    def test_put_file_retries(self, monkeypatch):
        ...
        monkeypatch.setattr('time.sleep', lambda x: None)
        monkeypatch.setattr('distronode.plugins.connection.ssh.os.path.exists', lambda x: True)
