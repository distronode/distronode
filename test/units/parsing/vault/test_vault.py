...
class TestUnhexlify(unittest.TestCase):
    ...
    def test_odd_length(self):
        ...
        self.assertRaisesRegex(vault.DistronodeVaultFormatError,
...
class TestParseVaulttext(unittest.TestCase):
    def test(self):
        vaulttext_envelope = u'''$DISTRONODE_VAULT;1.1;AES256
...
class TestPromptVaultSecret(unittest.TestCase):
    ...
    @patch('distronode.parsing.vault.display.prompt', return_value='the_password')
    ...
    @patch('distronode.parsing.vault.display.prompt', side_effect=EOFError)
    def test_prompt_eoferror(self, mock_display_prompt):
        secret = vault.PromptVaultSecret(vault_id='test_id')
        self.assertRaisesRegex(vault.DistronodeVaultError,
                               'EOFError.*test_id',
                               secret.load)

    @patch('distronode.parsing.vault.display.prompt', side_effect=['first_password', 'second_password'])
    def test_prompt_passwords_dont_match(self, mock_display_prompt):
        secret = vault.PromptVaultSecret(vault_id='test_id',
                                         prompt_formats=['Vault password: ',
                                                         'Confirm Vault password: '])
        self.assertRaisesRegex(errors.DistronodeError,
...
class TestFileVaultSecret(unittest.TestCase):
    ...
    def test_file_empty(self):
        ...
        secret = vault.FileVaultSecret(loader=fake_loader, filename=tmp_file.name)
        self.assertRaisesRegex(vault.DistronodeVaultPasswordError,
    ...
    def test_file_encrypted(self):
        ...
        password = 'some password'
        # 'some password' encrypted with 'test-distronode-password'

        password_file_content = '''$DISTRONODE_VAULT;1.1;AES256
...
class TestScriptVaultSecret(unittest.TestCase):
    ...
    @patch('distronode.parsing.vault.subprocess.Popen')
...
class TestVaultIsEncrypted(unittest.TestCase):
    ...
    def test_bytes_encrypted(self):
        b_data = b"$DISTRONODE_VAULT;9.9;TEST\n%s" % hexlify(b"distronode")
    ...
    def test_text_encrypted(self):
        b_data = to_text(b"$DISTRONODE_VAULT;9.9;TEST\n%s" % hexlify(b"distronode"))
        self.assertTrue(vault.is_encrypted(b_data))

    def test_invalid_text_not_ascii(self):
        data = u"$DISTRONODE_VAULT;9.9;TEST\n%s" % u"ァ ア ィ イ ゥ ウ ェ エ ォ オ カ ガ キ ギ ク グ ケ "
...
class TestVaultIsEncryptedFile(unittest.TestCase):
    ...
    def test_text_file_handle_encrypted(self):
        data = u"$DISTRONODE_VAULT;9.9;TEST\n%s" % to_text(hexlify(b"distronode"))
        data_fo = io.StringIO(data)
    ...
    def test_file_with_offset(self):
        b_data = b"JUNK$DISTRONODE_VAULT;9.9;TEST\n%s" % hexlify(b"distronode\ntesting\nfile pos")
        b_data_fo = io.BytesIO(b_data)
...
class TestVaultLib(unittest.TestCase):
    ...
    def test_encrypt(self):
        ...
        b_header = b'$DISTRONODE_VAULT;1.1;AES256\n'
    ...
    def test_encrypt_vault_id(self):
        ...
        b_header = b'$DISTRONODE_VAULT;1.2;AES256;test_id\n'
        self.assertEqual(b_vaulttext[:len(b_header)], b_header)
    ...
    def test_format_vaulttext_envelope(self):
        ...
        self.assertEqual(b_header_parts[0], b'$DISTRONODE_VAULT', msg="header does not start with $DISTRONODE_VAULT")
    ...
    def test_parse_vaulttext_envelope(self):
        b_vaulttext = b"$DISTRONODE_VAULT;9.9;TEST\ndistronode"
        b_ciphertext, b_version, cipher_name, vault_id = vault.parse_vaulttext_envelope(b_vaulttext)
        b_lines = b_ciphertext.split(b'\n')
        self.assertEqual(b_lines[0], b"distronode", msg="Payload was not properly split from the header")
    ...
    def test_encrypt_decrypt_aes256_none_secrets(self):
        vault_secrets = self._vault_secrets_from_password('default', 'distronode')
        v = vault.VaultLib(vault_secrets)
    ...
    def test_encrypt_decrypt_aes256_existing_vault(self):
        ...
        vaulttext = u'''$DISTRONODE_VAULT;1.1;AES256

3138'''
    ...
    def test_decrypt_and_get_vault_id(self):
        ...
        vaulttext = '''$DISTRONODE_VAULT;1.2;AES256;distronode_devel
...
6264'''
        ...
        vault_secrets = self._vault_secrets_from_password('distronode_devel', 'distronode')
        v = vault.VaultLib(vault_secrets)
        ...
        self.assertEqual(vault_id_used, 'distronode_devel')
        self.assertEqual(vault_secret_used.text, 'distronode')
...
6264'''
        ...
        b_ciphertext, b_version, cipher_name, vault_id = vault.parse_vaulttext_envelope(b_vaulttext)
        self.assertEqual('distronode_devel', vault_id)
        self.assertEqual(b'1.2', b_version)

    def test_decrypt_decrypted(self):
        plaintext = u"distronode"
        self.assertRaises(errors.DistronodeError, self.v.decrypt, plaintext)

        b_plaintext = b"distronode"
        self.assertRaises(errors.DistronodeError, self.v.decrypt, b_plaintext)
