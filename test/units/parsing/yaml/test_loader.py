...
class TestDistronodeLoaderBasic(unittest.TestCase):

    def test_parse_number(self):
        stream = StringIO(u"""
                1
                """)
        loader = DistronodeLoader(stream, 'myfile.yml')
    ...
    def test_parse_string(self):
        ...
        loader = DistronodeLoader(stream, 'myfile.yml')
        data = loader.get_single_data()
        self.assertEqual(data, u'Distronode')
        self.assertIsInstance(data, text_type)

        self.assertEqual(data.distronode_pos, ('myfile.yml', 2, 17))
    ...
    def test_parse_dict(self):
        ...
        self.assertEqual(data[u'webster'].distronode_pos, ('myfile.yml', 2, 26))
        self.assertEqual(data[u'oed'].distronode_pos, ('myfile.yml', 3, 22))
    ...
    def test_parse_list(self):
        ...
        self.assertEqual(data[0].distronode_pos, ('myfile.yml', 2, 19))
        self.assertEqual(data[1].distronode_pos, ('myfile.yml', 3, 19))
    ...
    def test_parse_short_dict(self):
        ...
        self.assertEqual(data.distronode_pos, ('myfile.yml', 1, 1))
        self.assertEqual(data[u'foo'].distronode_pos, ('myfile.yml', 1, 9))
        ...
        self.assertEqual(data.distronode_pos, ('myfile.yml', 1, 1))
        self.assertEqual(data[u'foo'].distronode_pos, ('myfile.yml', 1, 6))
    ...
    def test_front_matter(self):
        ...
        self.assertEqual(data.distronode_pos, ('myfile.yml', 2, 1))
        self.assertEqual(data[u'foo'].distronode_pos, ('myfile.yml', 2, 6))
        ...
        self.assertEqual(data.distronode_pos, ('myfile.yml', 1, 2))
        self.assertEqual(data[0].distronode_pos, ('myfile.yml', 1, 4))
        self.assertEqual(data[0][u'foo'].distronode_pos, ('myfile.yml', 1, 9))
        self.assertEqual(data[0][u'baz'].distronode_pos, ('myfile.yml', 2, 9))


class TestDistronodeLoaderVault(unittest.TestCase, YamlTestUtils):
    def setUp(self):
    ...
    def test_wrong_password(self):
        ...
        try:
            ...
            self.assertIsInstance(e, errors.DistronodeError)
    ...
    def _loader(self, stream):
        return DistronodeLoader(stream, vault_secrets=self.vault.secrets)
    ...
    def test_dump_load_cycle(self):
        avu = DistronodeVaultEncryptedUnicode.from_plaintext('The plaintext for test_dump_load_cycle.', self.vault, self.vault_secret)
        self._dump_load_cycle(avu)

    def test_embedded_vault_from_dump(self):
        avu = DistronodeVaultEncryptedUnicode.from_plaintext('setec astronomy', self.vault, self.vault_secret)
        ...
        self._dump_stream(blip, stream, dumper=DistronodeDumper)
        ...
        self._dump_stream(data_from_yaml, stream2, dumper=DistronodeDumper)
...
class TestDistronodeLoaderPlay(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.loader = DistronodeLoader(stream)
    ...
    def test_data_complete(self):
        ...
        self.assertEqual(self.data[0][u'vars'][u'string'], u'Distronode')
    ...
    def check_vars(self):
        ...
        # self.assertEqual(self.data[0][u'vars'][u'number'].distronode_pos, (self.play_filename, 4, 21))

        self.assertEqual(self.data[0][u'vars'][u'string'].distronode_pos, (self.play_filename, 5, 29))
        self.assertEqual(self.data[0][u'vars'][u'utf8_string'].distronode_pos, (self.play_filename, 6, 34))

        self.assertEqual(self.data[0][u'vars'][u'dictionary'].distronode_pos, (self.play_filename, 8, 23))
        self.assertEqual(self.data[0][u'vars'][u'dictionary'][u'webster'].distronode_pos, (self.play_filename, 8, 32))
        self.assertEqual(self.data[0][u'vars'][u'dictionary'][u'oed'].distronode_pos, (self.play_filename, 9, 28))

        self.assertEqual(self.data[0][u'vars'][u'list'].distronode_pos, (self.play_filename, 11, 23))
        self.assertEqual(self.data[0][u'vars'][u'list'][0].distronode_pos, (self.play_filename, 11, 25))
        self.assertEqual(self.data[0][u'vars'][u'list'][1].distronode_pos, (self.play_filename, 12, 25))
        # Numbers don't have line/col info yet
        # self.assertEqual(self.data[0][u'vars'][u'list'][2].distronode_pos, (self.play_filename, 13, 25))
        # self.assertEqual(self.data[0][u'vars'][u'list'][3].distronode_pos, (self.play_filename, 14, 25))

    def check_tasks(self):
        #
        # First Task
        #
        self.assertEqual(self.data[0][u'tasks'][0].distronode_pos, (self.play_filename, 16, 23))
        self.assertEqual(self.data[0][u'tasks'][0][u'name'].distronode_pos, (self.play_filename, 16, 29))
        self.assertEqual(self.data[0][u'tasks'][0][u'ping'].distronode_pos, (self.play_filename, 18, 25))
        self.assertEqual(self.data[0][u'tasks'][0][u'ping'][u'data'].distronode_pos, (self.play_filename, 18, 31))

        #
        # Second Task
        #
        self.assertEqual(self.data[0][u'tasks'][1].distronode_pos, (self.play_filename, 20, 23))
        self.assertEqual(self.data[0][u'tasks'][1][u'name'].distronode_pos, (self.play_filename, 20, 29))
        self.assertEqual(self.data[0][u'tasks'][1][u'ping'].distronode_pos, (self.play_filename, 22, 25))
        self.assertEqual(self.data[0][u'tasks'][1][u'ping'][u'data'].distronode_pos, (self.play_filename, 22, 31))

        #
        # Third Task
        #
        self.assertEqual(self.data[0][u'tasks'][2].distronode_pos, (self.play_filename, 24, 23))
        self.assertEqual(self.data[0][u'tasks'][2][u'name'].distronode_pos, (self.play_filename, 24, 29))
        self.assertEqual(self.data[0][u'tasks'][2][u'command'].distronode_pos, (self.play_filename, 25, 32))

    def test_line_numbers(self):
        # Check the line/column numbers are correct
        # Note: Remember, currently dicts begin at the start of their first entry
        self.assertEqual(self.data[0].distronode_pos, (self.play_filename, 2, 19))
        self.assertEqual(self.data[0][u'hosts'].distronode_pos, (self.play_filename, 2, 26))
        self.assertEqual(self.data[0][u'vars'].distronode_pos, (self.play_filename, 4, 21))

        self.check_vars()

        self.assertEqual(self.data[0][u'tasks'].distronode_pos, (self.play_filename, 16, 21))
