...
from distronode.utils.hashing import secure_hash_s
...
def collection_input(tmp_path_factory, collection_path_suffix):
    ...
    galaxy_args = ['distronode-galaxy', 'collection', 'init', '%s.%s' % (namespace, collection),
...
def manifest_template():
    def get_manifest_info(namespace='distronode_namespace', name='collection', version='0.1.0'):
        return {
            "collection_info": {
                "namespace": namespace,
                "name": name,
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                'url': 'https://galaxy.distronode.com',
...
def test_validate_certs_server_config(ignore_certs_cfg, ignore_certs_cli, expected_server2_validate_certs, expected_server3_validate_certs, monkeypatch):
    ...
    cfg_lines = [
        ...
        "[galaxy_server.server1]",
        "url=https://galaxy.distronode.com/api/",
        "validate_certs=False",
...
def test_timeout_server_config(timeout_cli, timeout_cfg, timeout_fallback, expected_timeout, monkeypatch):
    ...
    cfg_lines.extend(["[galaxy_server.server1]", "url=https://galaxy.distronode.com/api/"])
...
def test_build_existing_output_file(collection_input):
    ...
    existing_output_dir = os.path.join(output_dir, 'distronode_namespace-collection-0.1.0.tar.gz')
...
def test_build_existing_output_without_force(collection_input):
    ...
    existing_output = os.path.join(output_dir, 'distronode_namespace-collection-0.1.0.tar.gz')
...
def test_build_with_existing_files_and_manifest(collection_input):
    ...
    output_artifact = os.path.join(output_dir, 'distronode_namespace-collection-0.1.0.tar.gz')
...
def test_build_ignore_files_and_folders(collection_input, monkeypatch):
    ...
    retry_file = os.path.join(input_dir, 'distronode.retry')
    ...
    for manifest_entry in actual['files']:
        assert manifest_entry['name'] not in ['.git', 'distronode.retry', 'galaxy.yml', 'tests/output', 'tests/output/result.txt']
...
def test_require_one_of_collections_requirements_with_both():
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'verify', 'namespace.collection', '-r', 'requirements.yml'])

    with pytest.raises(DistronodeError) as req_err:
        cli._require_one_of_collections_requirements(('namespace.collection',), 'requirements.yml')

    with pytest.raises(DistronodeError) as cli_err:
        cli.run()
...
def test_require_one_of_collections_requirements_with_neither():
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'verify'])
...
def test_require_one_of_collections_requirements_with_collections():
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'verify', 'namespace1.collection1', 'namespace2.collection1:1.0.0'])
...
@patch('distronode.cli.galaxy.GalaxyCLI._parse_requirements_file')
def test_require_one_of_collections_requirements_with_requirements(mock_parse_requirements_file, galaxy_server):
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'verify', '-r', 'requirements.yml', 'namespace.collection'])
...
@patch('distronode.cli.galaxy.GalaxyCLI.execute_verify', spec=True)
def test_call_GalaxyCLI(execute_verify):
    galaxy_args = ['distronode-galaxy', 'collection', 'verify', 'namespace.collection']
...
@patch('distronode.cli.galaxy.GalaxyCLI.execute_verify')
def test_call_GalaxyCLI_with_implicit_role(execute_verify):
    galaxy_args = ['distronode-galaxy', 'verify', 'namespace.implicit_role']
...
def test_call_GalaxyCLI_with_role(execute_verify):
    galaxy_args = ['distronode-galaxy', 'role', 'verify', 'namespace.role']
...
@patch('distronode.cli.galaxy.verify_collections', spec=True)
def test_execute_verify_with_defaults(mock_verify_collections):
    galaxy_args = ['distronode-galaxy', 'collection', 'verify', 'namespace.collection:1.0.4']
    GalaxyCLI(args=galaxy_args).run()
    ...
    assert galaxy_apis[0].api_server == 'https://galaxy.distronode.com'
...
def test_execute_verify(mock_verify_collections):
    GalaxyCLI(args=[
        'distronode-galaxy', 'collection', 'verify', 'namespace.collection:1.0.4', '--ignore-certs',
        '-p', '~/.distronode', '--ignore-errors', '--server', 'http://galaxy-dev.com',
    ]).run()
...
def test_verify_file_hash_deleted_file(manifest_info):
    ...
    server = 'http://galaxy.distronode.com'
...
def test_get_nonexistent_tar_file_member(tmp_tarfile):
    ...
    with pytest.raises(DistronodeError) as err:
