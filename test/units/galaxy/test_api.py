...
from distronode.galaxy import api as galaxy_api
from distronode.galaxy.api import CollectionVersionMetadata, GalaxyAPI, GalaxyError
from distronode.galaxy.token import BasicAuthToken, GalaxyToken, KeycloakToken
from distronode.module_utils.common.text.converters import to_native, to_text
from distronode.module_utils.six.moves.urllib import error as urllib_error
    ...
    responses = [
        {
            ...
            "namespace": {
                "id": 30000,
                "href": "https://galaxy.distronode.com/api/v1/namespaces/30000/",
                "name": namespace,
            },
...
def test_api_no_auth():
    api = GalaxyAPI(None, "test", "https://galaxy.distronode.com/api/")
    actual = {}
...
def test_api_no_auth_but_required():
    ...
    with pytest.raises(DistronodeError, match=expected):
        GalaxyAPI(None, "test", "https://galaxy.distronode.com/api/")._add_auth_token({}, "", required=True)


def test_api_token_auth():
    token = GalaxyToken(token=u"my_token")
    api = GalaxyAPI(None, "test", "https://galaxy.distronode.com/api/", token=token)
    actual = {}
...
def test_api_token_auth_with_v3_url(monkeypatch):
    ...
    api = GalaxyAPI(None, "test", "https://galaxy.distronode.com/api/", token=token)
    actual = {}
    api._add_auth_token(actual, "https://galaxy.distronode.com/api/v3/resource/name", required=True)
    assert actual == {'Authorization': 'Bearer my_token'}
...
def test_api_token_auth_with_v2_url():
    ...
    api._add_auth_token(actual, "https://galaxy.distronode.com/api/v2/resourcev3/name", required=True)
    assert actual == {'Authorization': 'Token my_token'}
...
def test_initialise_galaxy(monkeypatch):
    ...
    assert mock_open.mock_calls[0][1][0] == 'https://galaxy.distronode.com/api/'
    assert 'distronode-galaxy' in mock_open.mock_calls[0][2]['http_agent']
    assert mock_open.mock_calls[1][1][0] == 'https://galaxy.distronode.com/api/v1/tokens/'
...
def test_initialise_galaxy_with_auth(monkeypatch):
    ...
    api = GalaxyAPI(None, "test", "https://galaxy.distronode.com/api/", token=GalaxyToken(token='my_token'))
    actual = api.authenticate("github_token")
...
def test_initialise_unknown(monkeypatch):
    mock_open = MagicMock()
    mock_open.side_effect = [
        urllib_error.HTTPError('https://galaxy.distronode.com/api/', 500, 'msg', {}, StringIO(u'{"msg":"raw error"}')),
        urllib_error.HTTPError('https://galaxy.distronode.com/api/api/', 500, 'msg', {}, StringIO(u'{"msg":"raw error"}')),
    ]
    monkeypatch.setattr(galaxy_api, 'open_url', mock_open)
...
def test_publish_collection_missing_file():
    ...
    api = get_test_galaxy_api("https://galaxy.distronode.com/api/", "v2")
    with pytest.raises(DistronodeError, match=expected):
...
def test_publish_collection_not_a_tarball():
    ...
    with tempfile.NamedTemporaryFile(prefix=u'ÅÑŚÌβŁÈ') as temp_file:
        ...
        temp_file.flush()
        with pytest.raises(DistronodeError, match=expected.format(to_native(temp_file.name))):
...
def test_publish_collection_unsupported_version():
    expected = "Galaxy action publish_collection requires API versions 'v2, v3' but only 'v1' are available on test " \
               "https://galaxy.distronode.com/api/"

    api = get_test_galaxy_api("https://galaxy.distronode.com/api/", "v1")
    with pytest.raises(DistronodeError, match=expected):
...
def test_publish_collection(api_version, collection_url, collection_artifact, monkeypatch):
    api = get_test_galaxy_api("https://galaxy.distronode.com/api/", api_version)
    ...
    assert mock_call.mock_calls[0][1][0] == 'https://galaxy.distronode.com/api/%s/%s/' % (api_version, collection_url)
            ...
            'signatures': [
                {
                    ...
                    "signing_service": "distronode-default",
...
def test_missing_cache_dir(cache_dir):
    os.rmdir(cache_dir)
    GalaxyAPI(None, "test", 'https://galaxy.distronode.com/', no_cache=False)
...
def test_world_writable_cache(cache_dir, monkeypatch):
    ...
    api = GalaxyAPI(None, "test", 'https://galaxy.distronode.com/', no_cache=False)
...
def test_no_cache(cache_dir):
    ...
    api = GalaxyAPI(None, "test", 'https://galaxy.distronode.com/')
    assert api._cache is None
...
def test_clear_cache_with_no_cache(cache_dir):
    ...
    GalaxyAPI(None, "test", 'https://galaxy.distronode.com/', clear_response_cache=True)
...
def test_clear_cache(cache_dir):
    ...
    GalaxyAPI(None, "test", 'https://galaxy.distronode.com/', clear_response_cache=True, no_cache=False)
