...
import distronode.module_utils.six.moves.urllib.error as urllib_error

from distronode import context
from distronode.cli.galaxy import GalaxyCLI
from distronode.errors import DistronodeError
from distronode.galaxy import collection, api, dependency_resolution
from distronode.galaxy.dependency_resolution.dataclasses import Candidate, Requirement
...
def call_galaxy_cli(args):
    ...
    try:
        GalaxyCLI(args=['distronode-galaxy', 'collection'] + args).run()
    finally:
...
def test_concrete_artifact_manager_scm_no_executable(monkeypatch):
    ...
    with pytest.raises(DistronodeError, match=error):
...
def test_build_requirement_from_path(collection_artifact):
    ...
    assert actual.namespace == u'distronode_namespace'
...
def test_build_requirement_from_path_with_manifest(version, collection_artifact):
    ...
    manifest_value = json.dumps({
        'collection_info': {
            ...
            'version': version,
            'dependencies': {
                'distronode_namespace.collection': '*'
            }
        }
    })
...
def test_build_requirement_from_tar_no_manifest(tmp_path_factory):
    ...
    tar_path = os.path.join(test_dir, b'distronode-collections.tar.gz')
...
def test_build_requirement_from_name(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', collections[0]])
...
def test_build_requirement_from_name_with_prerelease(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_build_requirment_from_name_with_prerelease_explicit(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection:2.0.1-beta.1'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_build_requirement_from_name_second_server(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection:>1.0.1'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_build_requirement_from_name_missing(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    with pytest.raises(DistronodeError, match=re.escape(expected)):
...
def test_build_requirement_from_name_single_version(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection:==2.0.0'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_build_requirement_from_name_multiple_versions_one_match(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection:>=2.0.1,<2.0.2'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_build_requirement_from_name_multiple_version_results(galaxy_server, monkeypatch, tmp_path_factory):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection:!=2.0.2'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_candidate_with_conflict(monkeypatch, tmp_path_factory, galaxy_server):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'namespace.collection:!=2.0.5'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_dep_candidate_with_conflict(monkeypatch, tmp_path_factory, galaxy_server):
    ...
    cli = GalaxyCLI(args=['distronode-galaxy', 'collection', 'install', 'parent.collection:2.0.5'])
    requirements = cli._require_one_of_collections_requirements(
...
def test_install_collection(collection_artifact, monkeypatch):
    ...
    collection_path = os.path.join(output_path, b'distronode_namespace', b'collection')
    os.makedirs(os.path.join(collection_path, b'delete_me'))  # Create a folder to verify the install cleans out the dir

    candidate = Candidate('distronode_namespace.collection', '0.1.0', to_text(collection_tar), 'file', None)
    ...
    assert mock_display.mock_calls[0][1][0] == "Installing 'distronode_namespace.collection:0.1.0' to '%s'" \
        % to_text(collection_path)
    assert mock_display.mock_calls[1][1][0] == "distronode_namespace.collection:0.1.0 was installed successfully"
...
def test_install_collection_with_download(galaxy_server, collection_artifact, monkeypatch):
    ...
    req = Candidate('distronode_namespace.collection', '0.1.0', 'https://downloadme.com', 'galaxy', None)
...
def test_install_collections_from_tar(collection_artifact, monkeypatch):
    ...
    requirements = [Requirement('distronode_namespace.collection', '0.1.0', to_text(collection_tar), 'file', None)]
    ...
    assert actual_manifest['collection_info']['namespace'] == 'distronode_namespace'
    ...
    assert display_msgs[2] == "Installing 'distronode_namespace.collection:0.1.0' to '%s'" % to_text(collection_path)


# Makes sure we don't get stuck in some recursive loop
@pytest.mark.parametrize('collection_artifact', [
    {'distronode_namespace.collection': '>=0.0.1'},
], indirect=True)
def test_install_collection_with_circular_dependency(collection_artifact, monkeypatch):
    ...
    assert actual_manifest['collection_info']['name'] == 'collection'
    assert actual_manifest['collection_info']['version'] == '0.1.0'
    assert actual_manifest['collection_info']['dependencies'] == {'distronode_namespace.collection': '>=0.0.1'}
    ...
    assert display_msgs[1] == "Starting collection install process"
    assert display_msgs[2] == "Installing 'distronode_namespace.collection:0.1.0' to '%s'" % to_text(collection_path)
    assert display_msgs[3] == "distronode_namespace.collection:0.1.0 was installed successfully"
...
def test_verify_file_signatures(signatures, required_successful_count, ignore_errors, expected_success):
    ...
    keyring = '~/.distronode/pubring.kbx'
