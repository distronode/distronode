...
# (c) 2015, Toshio Kuraotmi <tkuratomi@distronode.com>
#
...
class TestVariableUtils(unittest.TestCase):
    ...
    def test_combine_vars_improper_args(self):
        with mock.patch('distronode.constants.DEFAULT_HASH_BEHAVIOUR', 'replace'):
            with self.assertRaises(DistronodeError):
                combine_vars([1, 2, 3], dict(a=1))
            with self.assertRaises(DistronodeError):
                combine_vars(dict(a=1), [1, 2, 3])

        with mock.patch('distronode.constants.DEFAULT_HASH_BEHAVIOUR', 'merge'):
            with self.assertRaises(DistronodeError):
                combine_vars([1, 2, 3], dict(a=1))
            with self.assertRaises(DistronodeError):
                combine_vars(dict(a=1), [1, 2, 3])
