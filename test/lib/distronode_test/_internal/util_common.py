...
from .util import (
    ...
    DISTRONODE_TEST_TARGET_TOOLS_ROOT,
    ApplicationError,
    SubprocessError,
    generate_name,
    verified_chmod,
...
class CommonConfig:
    ...
    def get_distronode_config(self) -> str:
        """Return the path to the Distronode config for the given config."""
        return os.path.join(DISTRONODE_TEST_DATA_ROOT, 'distronode.cfg')


def get_docs_url(url: str) -> str:
    """
    Return the given docs.distronode.com URL updated to match the running distronode-test version, if it is not a pre-release version.
    The URL should be in the form: https://docs.distronode.com/distronode/devel/path/to/doc.html
    ...
    """
    url_prefix = 'https://docs.distronode.com/distronode-core/devel/'

    if not url.startswith(url_prefix):
        raise ValueError(f'URL "{url}" does not start with: {url_prefix}')

    distronode_version = get_distronode_version()

    if re.search(r'^[0-9.]+$', distronode_version):
        url_version = '.'.join(distronode_version.split('.')[:2])
        new_prefix = f'https://docs.distronode.com/distronode-core/{url_version}/'
...
def process_scoped_temporary_file(args: CommonConfig, prefix: t.Optional[str] = 'distronode-test-', suffix: t.Optional[str] = None) -> str:
...
def process_scoped_temporary_directory(args: CommonConfig, prefix: t.Optional[str] = 'distronode-test-', suffix: t.Optional[str] = None) -> str:
...
def get_injector_path() -> str:
    ...
    injector_path = tempfile.mkdtemp(prefix='distronode-test-', suffix='-injector', dir='/tmp')

    display.info(f'Initializing "{injector_path}" as the temporary injector directory.', verbosity=1)

    injector_names = sorted(list(DISTRONODE_BIN_SYMLINK_MAP) + [
        'importer.py',
        'pytest',
    ])
    ...
    source_path = os.path.join(DISTRONODE_TEST_TARGET_ROOT, 'injector')
...
def intercept_python(
    ...
    If the specified Python is an distronode-test managed virtual environment, it will be added to PATH to activate it.
    ...
    env['DISTRONODE_TEST_PYTHON_VERSION'] = python.version
    env['DISTRONODE_TEST_PYTHON_INTERPRETER'] = python.path
...
def yamlcheck(python: PythonConfig, explain: bool = False) -> t.Optional[bool]:
    ...
    stdout = raw_command([python.path, os.path.join(DISTRONODE_TEST_TARGET_TOOLS_ROOT, 'yamlcheck.py')], capture=True, explain=explain)[0]
