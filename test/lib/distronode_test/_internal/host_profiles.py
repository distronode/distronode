...
from .distronode_util import (
    distronode_environment,
    get_hosts,
    parse_inventory,
)
...
class Inventory:
    """Simple representation of an Distronode inventory."""
...
class RemoteProfile(SshTargetHostProfile[TRemoteConfig], metaclass=abc.ABCMeta):
    ...
    def core_ci_state(self) -> t.Optional[dict[str, str]]:
        """The saved Distronode Core CI state."""
    ...
    @property
    def core_ci(self) -> t.Optional[DistronodeCoreCI]:
        """Return the cached DistronodeCoreCI instance, if any, otherwise None."""
        return self.cache.get('core_ci')

    @core_ci.setter
    def core_ci(self, value: DistronodeCoreCI) -> None:
        """Cache the given DistronodeCoreCI instance."""
        self.cache['core_ci'] = value

    def get_instance(self) -> t.Optional[DistronodeCoreCI]:
        """Return the current DistronodeCoreCI instance, loading it if not already loaded."""
    ...
    def delete_instance(self) -> None:
        """Delete the DistronodeCoreCI VM instance."""
    ...
    def wait_for_instance(self) -> DistronodeCoreCI:
        """Wait for an DistronodeCoreCI VM instance to become ready."""
        core_ci = self.get_instance()
        core_ci.wait()

        return core_ci

    def create_core_ci(self, load: bool) -> DistronodeCoreCI:
        """Create and return an DistronodeCoreCI instance."""
        if not self.config.arch:
            raise InternalError(f'No arch specified for config: {self.config}')

        return DistronodeCoreCI(
            args=self.args,
            resource=VmResource(
...
class DockerProfile(ControllerHostProfile[DockerConfig], SshTargetHostProfile[DockerConfig]):
    ...
    def provision(self) -> None:
        ...
        container = run_support_container(
            ...
            name=f'distronode-test-{self.label}',
            ports=[22],
        ...
        try:
            ...
            if init_config.command and init_config.command_privileged:
                ...
                run_utility_container(self.args, f'distronode-test-init-{self.label}', cmd, options)
            ...
            if init_probe:
                ...
                run_utility_container(self.args, f'distronode-test-wake-{self.label}', cmd, options)
    ...
    def get_podman_init_config(self) -> InitConfig:
        ...
        # The most common source of a mismatch is the use of sudo to run distronode-test, which changes the uid but cannot change the loginuid.
        ...
        # The warning serves as notice to the user that their usage of distronode-test is responsible for the additional capability requirement.
        if (loginuid := detect_host_properties(self.args).loginuid) not in (0, LOGINUID_NOT_SET, None):
            display.warning(f'Running containers with capability AUDIT_CONTROL since the container loginuid ({loginuid}) is incorrect. '
                            'This is most likely due to use of sudo to run distronode-test when loginuid is already set.', unique=True)
        ...
        if self.config.cgroup == CGroupVersion.NONE:
            ...
            options.extend((
                ...
                # Provide a read-only cgroup v1 systemd hierarchy under which the dedicated distronode-test cgroup will be mounted read-write.
                ...
                # Provide the container access to the cgroup v1 systemd hierarchy created by distronode-test.
    ...
    def check_systemd_cgroup_v1(self, options: list[str]) -> None:
        ...
        probe_script = (read_text_file(os.path.join(DISTRONODE_TEST_TARGET_ROOT, 'setup', 'check_systemd_cgroup_v1.sh'))
        ...
        try:
            run_utility_container(self.args, f'distronode-test-cgroup-check-{self.label}', cmd, options, data=probe_script)
    ...
    def create_systemd_cgroup_v1(self) -> str:
        """Create a unique distronode-test cgroup in the v1 systemd hierarchy and return its path."""
        self.cgroup_path = f'/sys/fs/cgroup/systemd/distronode-test-{self.label}-{self.args.session_name}'
        ...
        try:
            run_utility_container(self.args, f'distronode-test-cgroup-create-{self.label}', cmd, options)
        except SubprocessError as ex:
            if error := self.extract_error(ex.stderr):
    ...
    def delete_systemd_cgroup_v1_command(self) -> list[str]:
        """The command used to remove the previously created distronode-test cgroup in the v1 systemd hierarchy."""
        return ['find', self.cgroup_path, '-type', 'd', '-delete']

    def delete_systemd_cgroup_v1(self) -> None:
        """Delete a previously created distronode-test cgroup in the v1 systemd hierarchy."""
        ...
        try:
            run_utility_container(self.args, f'distronode-test-cgroup-delete-{self.label}', cmd, options)
        except SubprocessError as ex:
            if error := self.extract_error(ex.stderr):
    ...
    def extract_error(self, value: str) -> t.Optional[str]:
        """
        Extract the distronode-test portion of the error message from the given value and return it.
        Returns None if no distronode-test marker was found.
        """
    ...
    def check_cgroup_requirements(self) -> None:
        ...
        # None of the containers included with distronode-test currently use this configuration.
    ...
    def get_controller_target_connections(self) -> list[SshConnection]:
        ...
        settings = SshConnectionDetail(
            ...
            # Since only CentOS 6 is affected, and it is only supported by distronode-core 2.12, support for RSA SHA-1 is simply hard-coded here.
            # A substring is used to allow custom containers to work, not just the one provided with distronode-test.
    ...
    def get_common_run_options(self) -> list[str]:
        ...
        options = [
            ...
            # Of special note is the "exec" option for "/tmp", which is required by distronode-test for path injection of executables using temporary directories.
            '--tmpfs', '/tmp:exec',
            '--tmpfs', '/run:exec',
...
class NetworkRemoteProfile(RemoteProfile[NetworkRemoteConfig]):
    ...
    def get_inventory_variables(self) -> dict[str, t.Optional[t.Union[str, int]]]:
        ...
        variables: dict[str, t.Optional[t.Union[str, int]]] = dict(
            distronode_connection=self.config.connection,
            distronode_pipelining='yes',
            distronode_host=connection.hostname,
            distronode_port=connection.port,
            distronode_user=connection.username,
            distronode_ssh_private_key_file=core_ci.ssh_key.key,
            ...
            # That means all network platforms currently offered by distronode-core-ci require support for RSA SHA-1, so it is simply hard-coded here.
            # NOTE: This option only exists in distronode-core 2.14 and later. For older distronode-core versions, use of Paramiko 2.8.x or earlier is required.
            #       See: https://github.com/distronode/distronode/pull/78789
            #       See: https://github.com/distronode/distronode/pull/78842
            distronode_paramiko_use_rsa_sha2_algorithms='no',
            distronode_network_os=f'{self.config.collection}.{self.config.platform}' if self.config.collection else self.config.platform,
        )

        return variables

    def wait_until_ready(self) -> None:
        """Wait for the host to respond to an Distronode module request."""
        core_ci = self.wait_for_instance()
        ...
        env = distronode_environment(self.args)
        ...
        with tempfile.NamedTemporaryFile() as inventory_file:
            ...
            cmd = ['distronode', '-m', module_name, '-a', 'commands=?', '-i', inventory_file.name, 'all']
...
class WindowsInventoryProfile(SshTargetHostProfile[WindowsInventoryConfig]):
    ...
    def get_controller_target_connections(self) -> list[SshConnection]:
        ...
        settings = [SshConnectionDetail(
            name=name,
            host=config['distronode_host'],
            port=22,
            user=config['distronode_user'],
...
class WindowsRemoteProfile(RemoteProfile[WindowsRemoteConfig]):
    ...
    def get_inventory_variables(self) -> dict[str, t.Optional[t.Union[str, int]]]:
        ...
        variables: dict[str, t.Optional[t.Union[str, int]]] = dict(
            ...
            distronode_port=connection.port,
            distronode_user=connection.username,
            distronode_password=connection.password,
            distronode_ssh_private_key_file=core_ci.ssh_key.key,
        )
        ...
        if self.config.version == '2016':
            variables.update(
                distronode_winrm_transport='ntlm',
                distronode_winrm_scheme='http',
                distronode_port='5985',
            )
    ...
    def wait_until_ready(self) -> None:
        ...
        inventory = Inventory.create_single_host(sanitize_host_name(self.config.name), self.get_inventory_variables())
        env = distronode_environment(self.args)
        module_name = 'distronode.windows.win_ping'

        with tempfile.NamedTemporaryFile() as inventory_file:
            inventory.write(self.args, inventory_file.name)

            cmd = ['distronode', '-m', module_name, '-i', inventory_file.name, 'all']
