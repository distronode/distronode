...
def get_coverage_platform(config: HostConfig) -> str:
    ...
    if isinstance(config, PosixRemoteConfig):
        ...
        platform = 'origin'  # previous versions of distronode-test used "local-{python_version}"
    else:
...
def get_coverage_environment(
    ...
    # Enable code coverage collection on local Python programs (this does not include Distronode modules).
...
def generate_coverage_config(args: TestConfig) -> str:
    ...
    if data_context().content.collection:
        ...
        coverage_config = generate_distronode_coverage_config()

    return coverage_config


def generate_distronode_coverage_config() -> str:
    """Generate code coverage configuration for Distronode tests."""
    coverage_config = '''
[run]
branch = True
concurrency =
...
def generate_collection_coverage_config(args: TestConfig) -> str:
    """Generate code coverage configuration for Distronode Collection tests."""
    coverage_config = '''
[run]
branch = True
concurrency =
