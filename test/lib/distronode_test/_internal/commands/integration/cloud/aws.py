...
class AwsCloudProvider(CloudProvider):
    ...
    def filter(self, targets: tuple[IntegrationTarget, ...], exclude: list[str]) -> None:
        ...
        aci = self._create_distronode_core_ci()
    ...
    def _setup_dynamic(self) -> None:
        """Request AWS credentials through the Distronode Core CI service."""
    ...
    def _create_distronode_core_ci(self) -> DistronodeCoreCI:
        """Return an AWS instance of DistronodeCoreCI."""
        return DistronodeCoreCI(self.args, CloudResource(platform='aws'))
...
class AwsCloudEnvironment(CloudEnvironment):
    ...
    def get_environment_config(self) -> CloudEnvironmentConfig:
        ...
        distronode_vars: dict[str, t.Any] = dict(
            resource_prefix=self.resource_prefix,
            tiny_prefix=uuid.uuid4().hex[0:12]
        )

        distronode_vars.update(dict(parser.items('default')))

        display.sensitive.add(distronode_vars.get('aws_secret_key'))
        display.sensitive.add(distronode_vars.get('security_token'))

        if 'aws_cleanup' not in distronode_vars:
            distronode_vars['aws_cleanup'] = not self.managed

        env_vars = {'DISTRONODE_DEBUG_BOTOCORE_LOGS': 'True'}
    ...
    def on_failure(self, target: IntegrationTarget, tries: int) -> None:
        ...
        if not tries and self.managed:
            display.notice('If %s failed due to permissions, the IAM test policy may need to be updated. '
                           'https://docs.distronode.github.io/distronode/devel/collections/amazon/aws/docsite/dev_guidelines.html#aws-permissions-for-integration-tests'
                           % target.name)
