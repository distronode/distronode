...
def main():
    ...
    # preload an empty distronode._vendor module to prevent use of any embedded modules during the import test
    vendor_module_name = 'distronode._vendor'

    vendor_module = types.ModuleType(vendor_module_name)
    vendor_module.__file__ = os.path.join(os.path.sep.join(os.path.abspath(__file__).split(os.path.sep)[:-8]), 'lib/distronode/_vendor/__init__.py')
    ...
    distronode_path = os.path.dirname(os.path.dirname(distronode.__file__))
    ...
    collection_root = os.environ.get('DISTRONODE_COLLECTIONS_PATH')
    ...
    if collection_full_name:
        ...
        from distronode.module_utils.common.text.converters import to_bytes, to_text, to_native, text_type

        # noinspection PyProtectedMember
        from distronode.utils.collection_loader._collection_finder import _DistronodeCollectionFinder
        from distronode.utils.collection_loader import _collection_finder
        ...
        collection_loader = _DistronodeCollectionFinder(paths=[collection_root])
        # noinspection PyProtectedMember
    ...
    if collection_loader and import_type == 'plugin':
        # do not unload distronode code for collection plugin (not module) tests
        # doing so could result in the collection loader being initialized multiple times
        pass
    else:
        # remove all modules under the distronode package, except the preloaded vendor module
        list(map(sys.modules.pop, [m for m in sys.modules if m.partition('.')[0] == distronode.__name__ and m != vendor_module_name]))
    ...
    if import_type == 'module':
        ...
        distronode_module.__path__ = distronode.__path__
        distronode_module.__package__ = distronode.__package__

        sys.modules[distronode.__name__] = distronode_module

    class ImporterDistronodeModuleException(Exception):
        """Exception thrown during initialization of ImporterDistronodeModule."""

    class ImporterDistronodeModule:
        """Replacement for DistronodeModule to support import testing."""
        def __init__(self, *args, **kwargs):
            raise ImporterDistronodeModuleException()
    ...
    class RestrictedModuleLoader:
        ...
        def find_spec(self, fullname, path=None, target=None):  # pylint: disable=unused-argument
            ...
            if loader is not None:
                ...
                raise ImportError("Failed to import '%s' due to a bug in distronode-test. Check importlib imports for typos." % fullname)
            return None
        ...
        def _get_loader(self, fullname, path=None):
            ...
            if is_name_in_namepace(fullname, ['distronode']):
                if not self.restrict_to_module_paths:
                    return None  # for non-modules, everything in the distronode namespace is allowed

                if fullname in ('distronode.module_utils.basic',):
                    return self  # intercept loading so we can modify the result

                if is_name_in_namepace(fullname, ['distronode.module_utils', self.name]):
                    return None  # module_utils and module under test are always allowed

                if any(os.path.exists(candidate_path) for candidate_path in convert_distronode_name_to_absolute_paths(fullname)):
                    return self  # restrict access to distronode files that exist

                return None  # distronode file does not exist, do not restrict access

            if is_name_in_namepace(fullname, ['distronode_collections']):
                ...
                if is_name_in_namepace(fullname, ['distronode_collections...plugins.module_utils', self.name]):
                    return None  # module_utils and module under test are always allowed
        ...
        def exec_module(self, module):
            ...
            """Execute the module if the name is distronode.module_utils.basic and otherwise raise an ImportError"""
            fullname = module.__spec__.name
            if fullname == 'distronode.module_utils.basic':
                self.loaded_modules.add(fullname)
                for path in convert_distronode_name_to_absolute_paths(fullname):
                    ...
                    real_module.DistronodeModule = ImporterDistronodeModule  # type: ignore[attr-defined]
        ...
        def load_module(self, fullname):
            ...
            """Return the module if the name is distronode.module_utils.basic and otherwise raise an ImportError."""
            if fullname == 'distronode.module_utils.basic':
                module = self.__load_module(fullname)

                # stop Distronode module execution during DistronodeModule instantiation
                module.DistronodeModule = ImporterDistronodeModule  # type: ignore[attr-defined]
                # no-op for _load_params since it may be called before instantiating DistronodeModule
    ...
    def test_python_module(path, name, base_dir, messages, restrict_to_module_paths):
        ...
        is_distronode_module = (path.startswith('lib/distronode/modules/') or path.startswith('plugins/modules/')) and os.path.basename(path) != '__init__.py'
        run_main = is_distronode_module

        if path == 'lib/distronode/modules/async_wrapper.py':
            # async_wrapper is a non-standard Distronode module (does not use DistronodeModule) so we cannot test the main function
            run_main = False
        ...
        try:
            ...
            if run_main:
                run_module_ok = is_distronode_module

                with monitor_sys_modules(path, messages):
                    with restrict_imports(path, name, messages, restrict_to_module_paths):
                        with capture_output(capture_main):
                            runpy.run_module(name, run_name='__main__', alter_sys=True)
        except ImporterDistronodeModuleException:
            # module instantiated DistronodeModule without raising an exception
            if not run_module_ok:
                if is_distronode_module:
                    report_message(path, 0, 0, 'module-guard', "DistronodeModule instantiation not guarded by `if __name__ == '__main__'`", messages)
                else:
                    report_message(path, 0, 0, 'non-module', "DistronodeModule instantiated by import of non-module", messages)
    ...
    def is_name_in_namepace(name, namespaces):
        ...
        for namespace in namespaces:
            ...
            # example: name=distronode, allowed_name=distronode.module_utils
            # example: name=distronode.module_utils.system.ping, allowed_name=distronode.module_utils
    ...
    def convert_distronode_name_to_absolute_paths(name):
        """Calculate the module path from the given name.
        :type name: str
        :rtype: list[str]
        """
        return [
            os.path.join(distronode_path, name.replace('.', os.path.sep)),
            os.path.join(distronode_path, name.replace('.', os.path.sep)) + '.py',
        ]
    ...
    def convert_relative_path_to_name(path):
        ...
        if collection_loader:
            ...
            name = 'distronode_collections.%s.%s' % (collection_full_name, name)
        else:
            # when testing distronode all files being imported reside under the lib directory
            name = name[len('lib/'):]
    ...
    class Capture:
        ...
        def __init__(self):
            # use buffered IO to simulate StringIO; allows Distronode's stream patching to behave without warnings
    ...
    def restrict_imports(path, name, messages, restrict_to_module_paths):
        ...
        try:
            ...
            if import_type == 'plugin' and not collection_loader:
                from distronode.utils.collection_loader._collection_finder import _DistronodeCollectionFinder
                _DistronodeCollectionFinder._remove()  # pylint: disable=protected-access
    ...
    def monitor_sys_modules(path, messages):
        ...
        try:
            ...
            for key in set(sys.modules.keys()) - set(snapshot.keys()):
                if is_name_in_namepace(key, ('distronode', 'distronode_collections')):
