...
try:
    ...
    from urllib2 import urlopen  # pylint: disable=distronode-bad-import-from
...
def main():  # type: () -> None
    ...
    with tempfile.NamedTemporaryFile(prefix='distronode-test-', suffix='-pip.py') as pip:
...
https://github.com/distronode/distronode/issues/77304
''' % (ex, cache_path)).strip())
...
def install(pip, options):  # type: (str, t.Dict[str, t.Any]) -> None
    ...
    tempdir = tempfile.mkdtemp(prefix='distronode-test-', suffix='-requirements')
    ...
    try:
        ...
        for path, content in requirements:
            if path.split(os.sep)[0] in ('test', 'requirements'):
                # Support for pre-build is currently limited to requirements embedded in distronode-test and those used by distronode-core.
                # Requirements from distronode-core can be found in the 'test' and 'requirements' directories.
...
class PreBuild:
    ...
    def execute(self, pip):  # type: (str) -> None
        ...
        tempdir = tempfile.mkdtemp(prefix='distronode-test-', suffix='-pre-build')
...
def parse_pre_build_instructions(requirements):  # type: (str) -> list[PreBuild]
    ...
    #          test/lib/distronode_test/_internal/commands/sanity/__init__.py
