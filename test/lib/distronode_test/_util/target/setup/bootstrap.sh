...
install_pip() {
    if ! "${python_interpreter}" -m pip.__main__ --version --disable-pip-version-check 2>/dev/null; then
        case "${python_version}" in
            *)
                pip_bootstrap_url="https://ci-files.testing.distronode.com/distronode-test/get-pip-23.1.2.py"
                ;;
        esac
...
bootstrap_remote_freebsd()
{
    ...
    if [ "${controller}" ]; then
        ...
        # For these combinations distronode-test will use pip to install the requirements instead.
    ...
    echo "# generated by distronode-test
[global]
extra-index-url = https://spare-tire.testing.distronode.com/simple/
prefer-binary = yes
" > /etc/pip.conf
    ...
    # enable sudo without a password for the wheel group, allowing distronode to use the sudo become plugin
    echo '%wheel ALL=(ALL:ALL) NOPASSWD: ALL' > /usr/local/etc/sudoers.d/distronode-test
}
...
bootstrap_remote_macos()
{
    ...
    # Make sure ~/distronode/ is the starting directory for interactive shells on the control node.
    # The root home directory is under a symlink. Without this the real path will be displayed instead.
    if [ "${controller}" ]; then
        echo "cd ~/distronode/" >> ~/.bashrc
    fi
...
bootstrap_remote_rhel_9()
{
    ...
    # Instead, distronode-test will install it using pip.
    # packaging and resolvelib are missing for Python 3.11 (and possible later) so we just
    # skip them and let distronode-test install them from PyPI.
    if [ "${controller}" ]; then
        packages="
            ${packages}
            ${py_pkg_prefix}-cryptography
...
bootstrap_remote_ubuntu()
{
    ...
    if [ "${controller}" ]; then
        ...
        # For these distronode-test will use pip to install the requirements instead.
...
bootstrap()
{
    ...
    # allow tests to detect distronode-test bootstrapped instances, as well as the bootstrap type
    echo "${bootstrap_type}" > /etc/distronode-test.bootstrap
