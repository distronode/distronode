...
def setup_collection_loader():
    ...
    collections_paths = os.environ.get('DISTRONODE_COLLECTIONS_PATH', '').split(os.pathsep)
    collection_loader = _DistronodeCollectionFinder(collections_paths)
    # noinspection PyProtectedMember
...
from distronode.executor.module_common import REPLACER_WINDOWS, NEW_STYLE_PYTHON_MODULE_RE
...
from .module_args import DistronodeModuleImportError, DistronodeModuleNotInitialized, get_argument_spec

from .schema import (
    distronode_module_kwargs_schema,
    doc_schema,
    return_schema,
)
    ...
    'requests': {
        ...
        'error': {
            ...
            'msg': ('requests import found, should use '
                    'distronode.module_utils.urls instead')
        }
    },
    r'boto(?:\.|$)': {
        'new_only': True,
...
LOOSE_DISTRONODE_VERSION = LooseVersion('.'.join(distronode_version.split('.')[:3]))
...
class ModuleValidator(Validator):
    ...
    def __init__(self, path, git_cache: GitCache, analyze_arg_spec=False, collection=None, collection_version=None,
        ...
        self.collection_name = 'distronode.builtin'
        if self.collection:
            ...
            if len([line for line in header
                ...
                self.reporter.error(
                    ...
                    msg='Found old style GPLv3 license header: '
                        'https://docs.distronode.com/distronode-core/devel/dev_guide/developing_modules_documenting.html#copyright'
                )
    ...
    def _find_module_utils(self):
        ...
        for child in self.ast.body:
            if isinstance(child, (ast.Import, ast.ImportFrom)):
                ...
                if [n for n in names if n.startswith('distronode.module_utils')]:
        ...
        if not found_basic:
            self.reporter.warning(
                ...
                msg='Did not find "distronode.module_utils.basic" import'
            )
    ...
    def _validate_ps_replacers(self):
        ...
        module_requires = r'(?im)^#\s*requires\s+\-module(?:s?)\s*(Distronode\.ModuleUtils\..+)'
        csharp_requires = r'(?im)^#\s*distronoderequires\s+\-csharputil\s*(Distronode\..+)'
        ...
        for req_stmt in re.finditer(module_requires, self.text):
            ...
            if len(module_list) > 1:
                self.reporter.error(
                    ...
                    msg='Distronode.ModuleUtils requirements do not support multiple modules per statement: "%s"' % req_stmt.group(0)
                )
                continue
        ...
        for req_stmt in re.finditer(csharp_requires, self.text):
            ...
            if len(module_list) > 1:
                self.reporter.error(
                    ...
                    msg='Distronode C# util requirements do not support multiple utils per statement: "%s"' % req_stmt.group(0)
                )
                continue
            ...
            if module_name.lower().endswith('.cs'):
                self.reporter.error(
                    ...
                    msg='Module #DistronodeRequires -CSharpUtil should not end in .cs: "%s"' % module_name
                )
        ...
        if not found_requires and REPLACER_WINDOWS not in self.text:
            self.reporter.error(
                ...
                msg='No Distronode.ModuleUtils or C# Distronode util requirements/imports found'
            )
    ...
    def _validate_docs_schema(self, doc, schema, name, error_code):
        ...
        for error in errors:
            ...
            local_error_code = getattr(error, 'distronode_error_code', error_code)
    ...
    def _validate_docs(self):
        ...
        if not self.collection:
            ...
            # In the future we should error if DISTRONODE_METADATA exists in a collection
    ...
    def _validate_semantic_markup(self, object) -> None:
        ...
        if self.collection:
            ...
            fqcn = f'distronode.builtin.{self.name}'
    ...
    def _check_version_added(self, doc, existing_doc):
        ...
        should_be = '.'.join(distronode_version.split('.')[:2])
        strict_distronode_version = self._create_strict_version(should_be, collection_name='distronode.builtin')

        if (version_added < strict_distronode_version or
                strict_distronode_version < version_added):
            self.reporter.error(
                path=self.object_path,
                code='module-incorrect-version-added',
    ...
    def _validate_distronode_module_call(self, docs):
        try:
            spec, kwargs = get_argument_spec(self.path, self.collection)
        except DistronodeModuleNotInitialized:
            self.reporter.error(
                path=self.object_path,
                code='distronode-module-not-initialized',
                msg="Execution of the module did not result in initialization of DistronodeModule",
            )
            return
        except DistronodeModuleImportError as e:
            self.reporter.error(
                path=self.object_path,
                code='import-error',
        ...
        schema = distronode_module_kwargs_schema(self.object_name.split('.')[0], for_collection=bool(self.collection))
        self._validate_docs_schema(kwargs, schema, 'DistronodeModule', 'invalid-distronodemodule-schema')
        ...
        for arg, data in spec.items():
            ...
            if arg.lower() in restricted_argument_names:
                ...
                msg += "must not be one of %s as it is used " \
                       "internally by Distronode Core Engine" % (",".join(restricted_argument_names))
                self.reporter.error(
                    path=self.object_path,
            ...
            if self.collection and self.collection_version is not None:
                ...
                compare_version = LOOSE_DISTRONODE_VERSION
                version_of_what = "Distronode (%s)" % distronode_version
                code_prefix = 'distronode'
                has_version = True
            ...
            if arg == 'provider' and self.object_path.startswith('lib/distronode/modules/network/'):
    ...
    def _check_for_new_args(self, doc):
        ...
        for option, details in options.items():
            ...
            builtin = self.collection_name == 'distronode.builtin' and collection_name in ('distronode.builtin', None)
            if not builtin and collection_name != self.collection_name:
                continue
            if (strict_distronode_version != mod_version_added and
                    (version_added < strict_distronode_version or
                     strict_distronode_version < version_added)):
                self.reporter.error(
                    path=self.object_path,
                    code='option-incorrect-version-added',
    ...
    def validate(self):
        ...
        if not self._python_module() and not self._powershell_module() and not self._sidecar_doc():
            self.reporter.error(
                ...
                msg=('Official Distronode modules must have a .py '
        ...
        if self._python_module() or self._sidecar_doc():
            ...
            if docs and docs.get('deprecated', False):
                ...
                if 'removed_in' in docs['deprecated']:
                    ...
                    if removed_in:
                        if not self.collection:
                            strict_distronode_version = self._create_strict_version(
                                '.'.join(distronode_version.split('.')[:2]), self.collection_name)
                            end_of_deprecation_should_be_removed_only = strict_distronode_version >= removed_in

                            if end_of_deprecation_should_be_removed_only:
                                self.reporter.error(
                                    path=self.object_path,
                                    code='distronode-deprecated-module',
                                    msg='Module is marked for removal in version %s of Distronode when the current version is %s' % (
                                        version, distronode_version),
                                )
                        elif self.collection_version:
                            strict_distronode_version = self.collection_version
                            end_of_deprecation_should_be_removed_only = strict_distronode_version >= removed_in
        ...
        if self._python_module() and not self._just_docs() and not end_of_deprecation_should_be_removed_only:
            if self.plugin_type == 'module':
                self._validate_distronode_module_call(docs)
        ...
        if self._powershell_module():
            ...
            # We can only validate PowerShell arg spec if it is using the new Distronode.Basic.DistronodeModule util
            pattern = r'(?im)^#\s*distronoderequires\s+\-csharputil\s*Distronode\.Basic'
    ...
    def validate(self):
        ...
        if not os.path.exists(init_file):
            self.reporter.error(
                ...
                msg='Distronode module subdirectories must contain an __init__.py'
            )
...
class CoreGitCache(GitCache):
    ...
    def __init__(self, original_plugins: str | None, plugin_type: str) -> None:
        ...
        rel_path = 'lib/distronode/modules/' if plugin_type == 'module' else f'lib/distronode/plugins/{plugin_type}/'
...
def run():
    ...
    parser.add_argument('--collection',
                        help='Specifies the path to the collection, when '
                             ...
                             'that DISTRONODE_COLLECTIONS_PATH is set so the '
