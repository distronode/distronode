"""Schema validation of distronode-core's distronode_builtin_runtime.yml and collection's meta/runtime.yml"""
...
def fqcr(value):
    ...
    if not DistronodeCollectionRef.is_valid_fqcr(value):
...
def removal_version(value, is_distronode, current_version=None, is_tombstone=False):
    """Validate a removal version string."""
    msg = (
        'Removal version must be a string' if is_distronode else
    ...
    try:
        if is_distronode:
            ...
            version.parse(value)
            version = LooseVersion(value)  # We're storing Distronode's version as a LooseVersion
        else:
...
def get_distronode_version():
    """Return current distronode-core version"""
...
def validate_metadata_file(path, is_distronode, check_deprecation_dates=False):
    ...
    deprecation_schema = All(
        ...
        Schema(
            {
                'removal_version': partial(removal_version, is_distronode=is_distronode,
    ...
    schema = Schema({
        ...
        # requires_distronode: In the future we should validate this with SpecifierSet
        ('requires_distronode'): Any(*string_types),
        ('action_groups'): dict,
    }, extra=PREVENT_EXTRA)
...
def main():
    ...
    for path in paths:
        ...
        validate_metadata_file(
            path,
            is_distronode=path not in (collection_legacy_file, collection_runtime_file),
