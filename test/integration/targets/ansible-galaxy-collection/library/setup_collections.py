...
- name: Build test collections
  setup_collections:
    path: ~/distronode/collections/distronode_collections
    collections:
...
def publish_collection(module, collection):
    ...
    with open(os.path.join(b_collection_dir, b'meta/runtime.yml'), mode='wb') as fd:
        fd.write(b'requires_distronode: ">=1.0.0"')
    ...
    with tempfile.NamedTemporaryFile(mode='wb') as temp_fd:
        ...
        rc, stdout, stderr = module.run_command(['distronode-galaxy', 'collection', 'build'], cwd=collection_dir)
        result['build'] = {
            'rc': rc,
            'stdout': stdout,
            'stderr': stderr,
    ...
    publish_args = ['distronode-galaxy', 'collection', 'publish', collection_path, '--server', module.params['server']]
