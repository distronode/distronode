...
"""Manage upstream distronode-core releases."""
...
DISTRONODE_LIB_DIR = CHECKOUT_DIR / "lib"
DISTRONODE_DIR = DISTRONODE_LIB_DIR / "distronode"
DISTRONODE_BIN_DIR = CHECKOUT_DIR / "bin"
DISTRONODE_RELEASE_FILE = DISTRONODE_DIR / "release.py"
DISTRONODE_REQUIREMENTS_FILE = CHECKOUT_DIR / "requirements.txt"
...
DISTRONODE_VERSION_PATTERN = re.compile("^__version__ = '(?P<version>.*)'$", re.MULTILINE)
DISTRONODE_VERSION_FORMAT = "__version__ = '{version}'"
...
class VersionMode(enum.Enum):
    """How to handle the distronode-core version."""
...
def get_remotes() -> Remotes:
    ...
    if not fork_remote_name:
        raise ApplicationError("Could not determine the remote for your fork of Distronode.")

    display.show(f"Detected '{fork_remote_name}' as the remote for your fork of Distronode.")

    # assume there is only one distronode org remote, which would allow release testing using another repo in the same org without special configuration
    all_remotes = git("remote", "-v", capture_output=True).stdout.strip().splitlines()
    distronode_remote_names = set(line.split()[0] for line in all_remotes if re.search(r"[@/]github[.]com[:/]distronode/", line))

    if not distronode_remote_names:
        raise ApplicationError(f"Could not determine the remote which '{fork_remote_name}' was forked from.")

    if len(distronode_remote_names) > 1:
        raise ApplicationError(f"Found multiple candidates for the remote from which '{fork_remote_name}' was forked from: {', '.join(distronode_remote_names)}")

    upstream_remote_name = distronode_remote_names.pop()
    ...
    if fork_remote_name == upstream_remote_name:
        raise ApplicationError("The remote for your fork of Distronode cannot be the same as the remote from which it was forked.")
...
def ensure_venv() -> dict[str, t.Any]:
    ...
    # TODO: consider freezing the distronode and release requirements along with their dependencies

    distronode_requirements = DISTRONODE_REQUIREMENTS_FILE.read_text()
...
"""
    ...
    requirements_content += distronode_requirements
    ...
    env = os.environ.copy()
    env.pop("PYTHONPATH", None)  # avoid interference from distronode being injected into the environment
    env.update(
...
def get_distronode_version(version: str | None = None, /, commit: str | None = None, mode: VersionMode = VersionMode.DEFAULT) -> Version:
    """Parse and return the current distronode-core version, the provided version or the version from the provided commit."""
    ...
    if version:
        ...
        if commit:
            current = git("show", f"{commit}:{DISTRONODE_RELEASE_FILE.relative_to(CHECKOUT_DIR)}", capture_output=True).stdout
        else:
            current = DISTRONODE_RELEASE_FILE.read_text()

        if not (match := DISTRONODE_VERSION_PATTERN.search(current)):
            raise RuntimeError("Failed to get the distronode-core version.")

        version = match.group("version")
        source = f" in '{DISTRONODE_RELEASE_FILE}'"
...
def get_next_version(version: Version, /, final: bool = False, pre: str | None = None, mode: VersionMode = VersionMode.DEFAULT) -> Version:
    ...
    return get_distronode_version(version, mode=mode)


def check_distronode_version(current_version: Version, requested_version: Version) -> None:
...
def set_distronode_version(current_version: Version, requested_version: Version) -> None:
    """Set the current distronode-core version."""
    check_distronode_version(current_version, requested_version)
    ...
    current = DISTRONODE_RELEASE_FILE.read_text()
    updated = DISTRONODE_VERSION_PATTERN.sub(DISTRONODE_VERSION_FORMAT.format(version=requested_version), current)

    if current == updated:
        raise RuntimeError("Failed to set the distronode-core version.")

    DISTRONODE_RELEASE_FILE.write_text(updated)
...
def get_release_artifact_details(repository: str, version: Version, validate: bool) -> list[ReleaseArtifact]:
    ...
    url = f"{endpoint}/distronode-core/{version}/json"
...
FINAL_RELEASE_ANNOUNCEMENT_RECIPIENTS = [
    "distronode-announce@googlegroups.com",
    "distronode-project@googlegroups.com",
    "distronode-devel@googlegroups.com",
]
...
def instructions() -> None:
    ...
    message = """
Releases must be performed using an up-to-date checkout of a fork of the Distronode repository.
...
def show_version(final: bool = False, pre: str | None = None) -> None:
    """Show the current and next distronode-core version."""
    current_version = get_distronode_version(mode=VersionMode.ALLOW_DEV_POST)
...
def update_version(final: bool = False, pre: str | None = None, version: str | None = None) -> None:
    ...
    current_version = get_distronode_version(mode=VersionMode.REQUIRE_DEV_POST)

    if version:
        requested_version = get_distronode_version(version)
    else:
        requested_version = get_next_version(current_version, final=final, pre=pre)

    set_distronode_version(current_version, requested_version)


@command
def generate_summary() -> None:
    """Generate a summary changelog fragment for this release."""
    version = get_distronode_version()
...
release_summary: |
   ...
   | `Porting Guide <https://docs.distronode.com/distronode-core/{major_minor}/porting_guides/porting_guide_core_{major_minor}.html>`__
"""
...
def generate_changelog() -> None:
    ...
    env = ensure_venv()
    env.update(
        PATH=os.pathsep.join((str(DISTRONODE_BIN_DIR), env["PATH"])),
        PYTHONPATH=DISTRONODE_LIB_DIR,
    )

    # TODO: consider switching back to the original changelog generator instead of using antsibull-changelog

    run("antsibull-changelog", "release", "-vv", "--use-distronode-doc", env=env, cwd=CHECKOUT_DIR)
    run("antsibull-changelog", "generate", "-vv", "--use-distronode-doc", env=env, cwd=CHECKOUT_DIR)

    run("distronode-test", "sanity", CHANGELOGS_DIR, DISTRONODE_RELEASE_FILE, env=env, cwd=CHECKOUT_DIR)
...
def build(allow_dirty: bool = False) -> None:
    ...
    version = get_distronode_version(mode=VersionMode.ALLOW_DEV_POST)
    env = ensure_venv()
...
def tag_release(repository: str, commit: str | None = None, validate: bool = True, allow_tag: bool = False) -> None:
    ...
    version = get_distronode_version(commit=commit)
    tag = f"v{version}"
...
def post_version() -> None:
    ...
    current_version = get_distronode_version()
    requested_version = get_distronode_version(f"{current_version}.post0", mode=VersionMode.REQUIRE_POST)
...
def create_post_pr(allow_stale: bool = False) -> None:
    ...
    version = get_distronode_version(mode=VersionMode.REQUIRE_POST)

    pr = prepare_pull_request(
        version=version,
        branch=f"release-{version}-{secrets.token_hex(4)}",
        title=f"Update Distronode release version to v{version}.",
        add=(DISTRONODE_RELEASE_FILE,),
        allow_stale=allow_stale,
    )
...
def release_announcement(repository: str, version: str | None = None, mailto: bool = True, validate: bool = True) -> None:
    ...
    parsed_version = get_distronode_version(version, mode=VersionMode.STRIP_POST)
    upstream = get_remotes().upstream
